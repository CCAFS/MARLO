package org.cgiar.ccafs.marlo.data.model;
// Generated Jan 11, 2019 9:52:41 AM by Hibernate Tools 5.3.6.Final

import org.cgiar.ccafs.marlo.data.IAuditLog;

import com.google.gson.annotations.Expose;

/**
 * ProjectPolicyCrossCuttingMarkers generated by hbm2java
 */
public class ProjectPolicyCrossCuttingMarker extends MarloBaseEntity implements java.io.Serializable, IAuditLog {

  private static final long serialVersionUID = -5739071488591041675L;

  @Expose
  private CgiarCrossCuttingMarker cgiarCrossCuttingMarker;
  @Expose
  private Phase phase;
  @Expose
  private ProjectPolicy projectPolicy;
  @Expose
  private RepIndGenderYouthFocusLevel repIndGenderYouthFocusLevel;


  public ProjectPolicyCrossCuttingMarker() {
  }

  @Override
  public boolean equals(Object obj) {
    if (this == obj) {
      return true;
    }
    if (obj == null) {
      return false;
    }
    if (this.getClass() != obj.getClass()) {
      return false;
    }
    ProjectPolicyCrossCuttingMarker other = (ProjectPolicyCrossCuttingMarker) obj;
    if (this.getId() == null) {
      if (other.getId() != null) {
        return false;
      }
    } else if (!this.getId().equals(other.getId())) {
      return false;
    }
    return true;
  }


  public CgiarCrossCuttingMarker getCgiarCrossCuttingMarker() {
    return cgiarCrossCuttingMarker;
  }


  @Override
  public String getLogDeatil() {
    StringBuilder sb = new StringBuilder();
    sb.append("Id : ").append(this.getId());
    return sb.toString();
  }


  @Override
  public String getModificationJustification() {
    return "";
  }


  @Override
  public User getModifiedBy() {
    User u = new User();
    u.setId(new Long(3));
    return u;
  }


  public Phase getPhase() {
    return phase;
  }


  public ProjectPolicy getProjectPolicy() {
    return projectPolicy;
  }

  public RepIndGenderYouthFocusLevel getRepIndGenderYouthFocusLevel() {
    return repIndGenderYouthFocusLevel;
  }

  @Override
  public int hashCode() {
    final int prime = 31;
    int result = 1;
    result = prime * result + ((projectPolicy == null) ? 0 : projectPolicy.hashCode());
    return result;
  }

  @Override
  public boolean isActive() {
    return true;
  }


  public void setCgiarCrossCuttingMarker(CgiarCrossCuttingMarker cgiarCrossCuttingMarker) {
    this.cgiarCrossCuttingMarker = cgiarCrossCuttingMarker;
  }


  @Override
  public void setModifiedBy(User modifiedBy) {
  }


  public void setPhase(Phase phase) {
    this.phase = phase;
  }


  public void setProjectPolicy(ProjectPolicy projectPolicy) {
    this.projectPolicy = projectPolicy;
  }


  public void setRepIndGenderYouthFocusLevel(RepIndGenderYouthFocusLevel repIndGenderYouthFocusLevel) {
    this.repIndGenderYouthFocusLevel = repIndGenderYouthFocusLevel;
  }


}

