...
public class DeliverableMySQLDAO extends AbstractMarloDAO<Deliverable, Long> implements DeliverableDAO {
  ...
  public List<Deliverable> getDeliverablesByPhase(long phase, boolean includePublications) {
    ...
    query.append("WHERE d.is_active = 1 AND d.project_id IS NOT NULL AND ");
    query.append(includePublications ? "" : "(d.is_publication IS NULL OR d.is_publication = 0) AND ");
  ...
  public List<DeliverableHomeDTO> getDeliverablesByProjectAndPhaseHome(long phaseId, long projectId) {
    ...
    createQuery.setResultTransformer(
      (ListResultTransformer) (tuple, aliases) -> new DeliverableHomeDTO(((Number) tuple[0]).longValue(),
        ...
        (String) tuple[4], (String) tuple[5], (String) tuple[6], (String) tuple[7]));
  ...
  public List<Deliverable> getDeliverableSynthesis(Long projectId, long phaseId, boolean prp) {
    ...
    query = query.append("select d from DeliverableInfo di join di.deliverable as d join di.phase as ph ")
      .append(projectId != null ? "join d.project as pr " : "").append("where di.deliverableType.id ")
      .append(prp ? "=" : "!=").append(" 63 ").append("and d.active = true and ph.id = :phaseId ")
      .append(projectId != null ? "and pr.id = :projectId " : "").append("and coalesce(di.status, -1) = 3 ")
      .append("and ((coalesce(di.newExpectedYear, -1) = ph.year) or (di.year = ph.year))");

    Query createQuery = this.getSessionFactory().getCurrentSession().createQuery(query.toString());

    createQuery.setParameter("phaseId", phaseId);
    if (projectId != null) {
      createQuery.setParameter("projectId", projectId.longValue());
    }
