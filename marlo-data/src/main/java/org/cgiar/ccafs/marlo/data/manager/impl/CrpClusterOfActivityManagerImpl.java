...
public class CrpClusterOfActivityManagerImpl implements CrpClusterOfActivityManager {
	...
	private void addCrpClusterPhase(Phase next, long crpProgramID, CrpClusterOfActivity crpCluster) {
		...
		List<CrpClusterOfActivity> clusters = phase.getClusters().stream()
				.filter(c -> c.isActive() && c.getCrpProgram().getId().longValue() == crpProgramID
		...
		if (crpClusterOfActivity.getKeyOutputs() != null) {
			for (CrpClusterKeyOutput crpClusteKeyOutput : crpClusterOfActivity.getKeyOutputs()) {
				...
				if (crpClusteKeyOutput.getKeyOutputOutcomes() != null) {
					for (CrpClusterKeyOutputOutcome crpClusterKeyOutputOutcome : crpClusteKeyOutput
						...
						CrpProgramOutcome crpProgramOutcome = crpProgramOutcomeDAO
								.find(crpClusterKeyOutputOutcome.getCrpProgramOutcome().getId());
						crpClusterKeyOutputOutcomeAdd.setCrpProgramOutcome(crpProgramOutcomeDAO.getCrpProgramOutcome(
								crpProgramOutcome.getComposeID(), crpClusterOfActivityAdd.getPhase()));
	...
	public CrpClusterOfActivity saveCrpClusterOfActivity(CrpClusterOfActivity crpClusterOfActivity) {
		...
		if (crpClusterOfActivity.getPhase().getNext() != null) {
			this.addCrpClusterPhase(crpClusterOfActivity.getPhase().getNext(),
					crpClusterOfActivity.getCrpProgram().getId(), crpClusterOfActivity);
		}
	...
	private void updateKeyOutputs(CrpClusterOfActivity crpClusterOfActivityPrev,
		...
		for (CrpClusterKeyOutput crpClusterKeyOutput : crpClusterOfActivityPrev.getCrpClusterKeyOutputs().stream()
				.filter(c -> c.isActive()).collect(Collectors.toList())) {
			if (crpClusterOfActivity.getKeyOutputs() == null || crpClusterOfActivity.getKeyOutputs().stream().filter(
					c -> c.getComposeID() != null && c.getComposeID().equals(crpClusterKeyOutput.getComposeID()))
					.collect(Collectors.toList()).isEmpty()) {
			...
			for (CrpClusterKeyOutput crpClusterKeyOutput : crpClusterOfActivity.getKeyOutputs()) {
				if (crpClusterOfActivityPrev.getCrpClusterKeyOutputs().stream()
					...
					if (crpClusterKeyOutput.getComposeID() == null) {
						crpClusterKeyOutput.setComposeID(
								crpClusterOfActivityPrev.getIdentifier() + "-" + crpClusterKeyOutputAdd.getId());
						crpClusterKeyOutputAdd.setComposeID(crpClusterKeyOutput.getComposeID());
						crpClusterKeyOutputDAO.save(crpClusterKeyOutputAdd);
					}
					...
					if (crpClusterKeyOutput.getKeyOutputOutcomes() != null) {
						for (CrpClusterKeyOutputOutcome crpClusterKeyOutputOutcome : crpClusterKeyOutput
							...
							CrpProgramOutcome crpProgramOutcome = crpProgramOutcomeDAO
									.find(crpClusterKeyOutputOutcome.getCrpProgramOutcome().getId());
							crpClusterKeyOutputOutcomeAdd.setCrpProgramOutcome(
									crpProgramOutcomeDAO.getCrpProgramOutcome(crpProgramOutcome.getComposeID(),
											crpClusterOfActivityPrev.getPhase()));
					...
					CrpClusterKeyOutput crpClusterKeyOutputtoUpdate = crpClusterOfActivityPrev.getCrpClusterKeyOutputs()
							.stream()
							.filter(c -> c.isActive() && c.getComposeID().equals(crpClusterKeyOutput.getComposeID()))
					...
					for (CrpClusterKeyOutputOutcome crpClusterKeyOutputOutcome : crpClusterKeyOutput
						...
						if (crpClusterKeyOutputOutcome != null
								&& crpClusterKeyOutputOutcome.getCrpProgramOutcome() != null
								&& crpClusterKeyOutputOutcome.getCrpProgramOutcome().getId() != null) {
							CrpProgramOutcome crpProgramOutcome = crpProgramOutcomeDAO
									.find(crpClusterKeyOutputOutcome.getCrpProgramOutcome().getId());
							crpClusterKeyOutputOutcome.setCrpProgramOutcome(crpProgramOutcomeDAO.getCrpProgramOutcome(
									crpProgramOutcome.getComposeID(), crpClusterOfActivityPrev.getPhase()));
						} else {
					...
					for (CrpClusterKeyOutputOutcome crpClusterKeyOutputOutcome : crpClusterKeyOutputtoUpdate
						...
						if (crpClusterKeyOutput.getKeyOutputOutcomes().stream()
								.filter(c -> c.getCrpProgramOutcome() != null
										&& c.getCrpProgramOutcome().getComposeID() != null
										&& c.getCrpProgramOutcome().getComposeID().equals(
												crpClusterKeyOutputOutcome.getCrpProgramOutcome().getComposeID()))
								.collect(Collectors.toList()).isEmpty()) {
							crpClusterKeyOutputOutcomeDAO
									.deleteCrpClusterKeyOutputOutcome(crpClusterKeyOutputOutcome.getId());
						}
					}
					for (CrpClusterKeyOutputOutcome crpClusterKeyOutputOutcome : crpClusterKeyOutput
							.getKeyOutputOutcomes()) {
						if (crpClusterKeyOutputOutcome != null) {
							if (crpClusterKeyOutputtoUpdate.getCrpClusterKeyOutputOutcomes().stream()
									.filter(c -> c.isActive() && crpClusterKeyOutputOutcome != null
											&& crpClusterKeyOutputOutcome.getCrpProgramOutcome() != null
											&& c.getCrpProgramOutcome() != null
											&& c.getCrpProgramOutcome().getComposeID().equals(
													crpClusterKeyOutputOutcome.getCrpProgramOutcome().getComposeID()))
									.collect(Collectors.toList()).isEmpty()) {
								CrpClusterKeyOutputOutcome crpClusterKeyOutputOutcomeAdd = new CrpClusterKeyOutputOutcome();
								crpClusterKeyOutputOutcomeAdd
										.setContribution(crpClusterKeyOutputOutcome.getContribution());
								crpClusterKeyOutputOutcomeAdd.setCrpClusterKeyOutput(crpClusterKeyOutputtoUpdate);
								crpClusterKeyOutputOutcomeAdd
										.setCrpProgramOutcome(crpClusterKeyOutputOutcome.getCrpProgramOutcome());
