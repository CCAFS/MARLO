...
public class ProjectExpectedStudyCountryMySQLDAO extends AbstractMarloDAO<ProjectExpectedStudyCountry, Long>
  ...
  public List<ProjectExpectedStudyCountry> getAllStudyCountriesByStudy(long studyId) {
    String query =
      "select pescoun from ProjectExpectedStudyCountry pescoun where pescoun.projectExpectedStudy.id = :studyId order by pescoun.phase.id";

    Query createQuery = this.getSessionFactory().getCurrentSession().createQuery(query);
    createQuery.setParameter("studyId", studyId);

    List<ProjectExpectedStudyCountry> result = super.findAll(createQuery);
  ...
  public ProjectExpectedStudyCountry getProjectExpectedStudyCountryByPhase(Long projectExpectedStudyID, Long countryID,
    Long PhaseID) {
    String query =
      "from " + ProjectExpectedStudyCountry.class.getName() + " where expected_id=" + projectExpectedStudyID.longValue()
        + " and loc_element_id=" + countryID.longValue() + " and id_phase=" + PhaseID.longValue();
    List<ProjectExpectedStudyCountry> list = super.findAll(query);
    if (list.size() > 0) {
      return list.get(0);
    }
