        ...
        success: function(data) {
          ...
          for(var i = 0; i < length; i++) {
            $('.capdevResearchProgram').append(
                '<option value=' + data[i]['rpID'] + '>' + data[i]['rpName'] + '</option>');
          }
        },
        error: function() {
        },
  ...
  if(researchProgramID > 0) {
    $.ajax({
        ...
        success: function(data) {
          ...
          for(var i = 0; i < length; i++) {
            $('.capdevProject').append(
                '<option value=' + data[i]['projectID'] + '>' + data[i]['projectName'] + '</option>');
          }
        },
        error: function() {
        },
    ...
    $.ajax({
        ...
        success: function(data) {
          ...
          for(var i = 0; i < partnersLength; i++) {
            $('.capdevPartnerSelect').append(
                '<option value=' + data[0]['partners'][i]['idPartner'] + '>' + data[0]['partners'][i]['partnerName']
                    + '</option>');
          }
          ...
          for(var i = 0; i < outputsLength; i++) {
            $('.capdevOutputSelect').append(
                '<option value=' + data[1]['outputs'][i]['idOutput'] + '>' + data[1]['outputs'][i]['outputTitle']
                    + '</option>');
          }
        },
        error: function() {
        ...
        complete: function() {
          /*
           * $(".project select option").each(function() { if($(this).val() == projectSelected){ $(this).attr(
           * "selected" , "selected"); } });
           */
        }
    })
