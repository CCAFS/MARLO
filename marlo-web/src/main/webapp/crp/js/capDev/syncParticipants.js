...
function initSync() {
  console.log('Sync Participant started');
  syncing = false;
...
function getOCSMetadata() {
  var currentCode = $('input.participant-code').val();
  ...
  $.ajax({
      ...
      'data': {
        syncParticipantCode: currentCode,
        phaseID: phaseID
      },
      beforeSend: function() {
      ...
      success: function(data) {
        if (typeof data.json != "undefined") {
          ...
          // Set MARLO Country ID
          $(".pCitizenshipcountriesList select option").each(function() {
            if($(this).text() == participant.cityOfBirth) {
              participant.countryID = this.value;
            }
          });
          
          setMetadata(participant);
       }else{
         console.log("Invalid Code for searching metadata")
         $syncButtons.animateCss('shake');
       }
      ...
      error: function() {
        $('#metadata-output').empty().append("Invalid Code for searching metadata");
      }
  });
}
