...
[#macro targetUnitMacro element name index isTemplate=false]
  <li id="targetUnit-${isTemplate?string('template',index)}" class="li-item targetUnitAdmin" style="float:left; width:48%; margin-right:5px; display:${isTemplate?string('none','block')}">
    ...
    [#-- Remove Button --]
      ...
      <span class=" pull-right" > <input [#if action.canBeDeleted(element.targetUnit.id, element.targetUnit.class.name)!][#else]disabled[/#if] type="checkbox" value="true" name="${customName}.check" id="" [#if element.check?? && element.check]checked[/#if]/></span>
