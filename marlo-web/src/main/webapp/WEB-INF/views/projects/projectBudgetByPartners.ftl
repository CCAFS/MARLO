  ...
  <div id="projectW3bilateralFund-${isTemplate?string('template', index )}" class="projectW3bilateralFund expandableBlock grayBox" style="display:${isTemplate?string('none','block')}">
    ...
    <div class="row w3bilateralFund">
      <div class="col-md-5">
        ...
        [#if editable && isYearEditable(selectedYear)]
          [@customForm.select name="${customName}.budgetType.id" i18nkey="budget.budgetType"  showTitle=false  disabled=!editable  listName="w3bilateralBudgetTypes" required=true /]
        [#else]    
          <div class="input"><p> ${(element.budgetType.id)!} </p></div>
        [/#if] 
        </div>
      </div>
      <div class="col-md-4">
