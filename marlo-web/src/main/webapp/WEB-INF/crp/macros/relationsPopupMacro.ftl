...
[#macro relationsMacro element labelText=true ]
  ...
  [/#if]
[/#macro]
[#macro relationsOutcomesMacro element labelText=true]
  [#local className = ((element.class.name)?split('.')?last)!''/]
  [#local composedID = "${className}-${(element.id)!}"]
  [#local deliverables = (action.getDeliverableRelationsProject(element.id, element.class.name,(element.project.id)!-1))! /]  
  [#local projects = (action.getProjectRelationsImpact(element.id, element.class.name))! /]
  [#-- News buttons --]
  ...
  [#local policies = (action.getPoliciesProjectOutcomes(element.id))![] /]
  [#-- News buttons --]
              ...
              <table class="table">
                <thead>
                  <tr>
                    ...
                    <th id="deliverableYear">[@s.text name="project.deliverableList.deliverySummaryYear" /]</th>
                    <th></th>
                  </tr>
                </thead>
                <tbody>
