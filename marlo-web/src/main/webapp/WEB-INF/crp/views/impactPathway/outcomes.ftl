...
[#macro milestoneMacro milestone name index isTemplate=false editable=true canEditMilestone=true]
  ...
  <div id="milestone-${isTemplate?string('template', index)}" class="milestone simpleBox" style="display:${isTemplate?string('none','block')}">
    ...
    <div class="row form-group">
      <div class="col-md-1 text-center"> <strong>${(milestone.year)!'Year Not Defined'} </strong> <br /> <small><nobr>${(milestone.milestonesStatus.name)!}</nobr></small> </div>
      <div class="col-md-9" >
        [#--  Title  --]
        <small><a href="#" data-toggle="modal" data-target="#milestone-${(milestone.id)!'template'}">${(milestone.title)!'Title Not Defined'}</a></small>
      </div>
      [#--  Projects Popup  --]
      <div class="col-md-1">[@popUps.relationsMacro element=(milestone)!{} /]</div> 
    </div>
   
    
    <!-- Modal -->
    <div class="modal fade" id="milestone-${(milestone.id)!'template'}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
