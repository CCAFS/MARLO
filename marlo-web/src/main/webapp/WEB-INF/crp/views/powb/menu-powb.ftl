...
[#assign menus= [
  { 'title': '1.  Expected Key Results', 'show': true,
    ...
    { 'slug': 'plansByFlagship',          'name': 'powb.menu.plansByFlagship',        'action': 'plansByFlagship',        'active': (flagship || PMU) && config.debug },
    { 'slug': 'crossCuttingDimensions',   'name': 'powb.menu.crossCuttingDimensions', 'action': 'crossCuttingDimensions', 'active': false  }
    ]
  },
  { 'title': '2.  Planning for CRP Effectiveness and Efficiency', 'show': true,
    'items': [
    { 'slug': 'crpStaffing',              'name': 'powb.menu.crpStaffing',                'action': 'crpStaffing',                'active': (flagship || PMU) && config.debug },
    { 'slug': 'financialPlan',            'name': 'powb.menu.financialPlan',              'action': 'financialPlan',              'active': (flagship || PMU) && config.debug  },
    { 'slug': 'collaborationIntegration', 'name': 'powb.menu.collaborationIntegration',   'action': 'collaborationIntegration',   'active': false },
    { 'slug': 'monitoringLearning',       'name': 'powb.menu.monitoringLearning',         'action': 'mel',                        'active': (flagship || PMU) && config.debug }
    ]
  },
  { 'title': '3.  CRP Management', 'show': true,
    'items': [
    { 'slug': 'managementRisks',        'name': 'powb.menu.managementRisks',        'action': 'managementRisks',        'active': (flagship || PMU) && config.debug  },
    { 'slug': 'managementGovernance',   'name': 'powb.menu.managementGovernance',   'action': 'managementGovernance',   'active': (flagship || PMU) && config.debug  }
    ]
  }
]/]
