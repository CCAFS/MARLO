    ...
    <div class="row">
      ...
      <div class="col-md-9">
        ...
        [@s.form action=actionName method="POST" enctype="multipart/form-data" cssClass=""]
          ...
          [#if editable && canEdit && (action.canAccessSuperAdmin() || action.canAcessCrpAdmin() || action.isRole("PL"))]
...
[#macro projectActivityMacro element name index=0 isTemplate=false isActive=false]
  [#assign customName = "${name}[${index}]" /]
    ...
    --]
    [#if isActive && editable && (action.canAccessSuperAdmin() || action.canAcessCrpAdmin() || action.isRole("PL"))] [#--&& (isTemplate) --]
    ...
    <div class="blockContent" style="display:none">
      ...
      [#if action.isAiccra()]
        <div class="form-group">
          [@customForm.select name="${customName}.activityTitle.id" label="" className="activityTitle" i18nkey="project.activities.inputTitle" listName="activityTitles" keyFieldName="id" displayFieldName="title" multiple=false required=true editable=(action.canAccessSuperAdmin() || action.isRole("PL")) && isActive/]
        </div>
      [/#if]
      [#-- Description --]
      <div class="form-group">
