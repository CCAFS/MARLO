      ...
      [#if reportingActive]
        ...
        <div class="${reportingActive?string('fieldFocus','')}">
          <div class="form-group">
            ...
            [@customForm.radioFlat id="hasPartnerships-yes-${index}" name="${name}.hasPartnerships" label="Yes" value="true" checked=false cssClass="hasPartnerships-yes" cssClassLabel="radio-label-yes"/]
            [@customForm.radioFlat id="hasPartnerships-no-${index}" name="${name}.hasPartnerships" label="No" value="false" checked=false cssClass="hasPartnerships-no" cssClassLabel="radio-label-no"/]
          </div>
          <div class="form-group">
            <div class="form-group row">
              <div class="col-md-6">
                [#-- Phase of research --]
                [@customForm.select name="${name}.researchPhase.id" className="setSelect2" i18nkey="projectPartners.researchPhase" listName="" keyFieldName="id"  displayFieldName="name" /]
              </div>
              <div class="col-md-6">
                [#-- Geographic Scope --]
                [@customForm.select name="${name}.geographicScope.id" className="setSelect2" i18nkey="projectPartners.geographicScope" listName="" keyFieldName="id"  displayFieldName="name" /]
              </div>
            </div>
            <div class="form-group">
              [#-- Regional scope --]
              [@customForm.select name="${name}.region.id" className="setSelect2" i18nkey="projectPartners.region" listName="" keyFieldName="id"  displayFieldName="name" /]
            </div>
            <div class="form-group">
