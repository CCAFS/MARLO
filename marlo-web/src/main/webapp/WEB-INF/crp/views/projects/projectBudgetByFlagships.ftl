            ...
            [#list budgetTypesList as budgetType]
              <td class="budgetColumn">
                ...
                [#if editable && isYearEditable(selectedYear)]
                  [@customForm.input name="${customName}.amount" i18nkey="budget.amount" showTitle=false className="percentageInput context-total  type-${budgetType.id}" required=true editable=action.hasPermission("w1w2") || action.hasPermission("w3") || action.hasPermission("bilateral") || action.hasPermission("center")/]
                [#else]
