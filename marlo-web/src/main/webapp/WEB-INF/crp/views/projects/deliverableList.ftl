    ...
    <div class="row">
      ...
      [#-- Section Messages --]
        ...
        [@s.form action=actionName method="POST" enctype="multipart/form-data" cssClass=""]
          ...
          [#-- Simple table --]
           [@deliverableList.deliverablesList deliverables=action.getDeliverables(true) canValidate=true canEdit=candit  isReportingActive=reportingActive namespace="/projects" defaultAction="${(crpSession)!}/deliverable"/]
            ...
            <div class="modal fade extended-table-modal completed-modal" tabindex="-1" role="dialog" aria-labelledby="extendedTableModal" aria-hidden="true">
              <div class="modal-dialog modal-lg">
                <div class="modal-content">
                  <h3 class="subTitle headTitle">Completed deliverables</h3>
                  <hr />
                    [@deliverableList.deliverablesListExtended deliverables=action.getDeliverables(false) canValidate=true canEdit=candit namespace="/projects" defaultAction="${(crpSession)!}/deliverable"/]
                </div>
              </div>
            </div>
            [#-- Simple table --]
            [@deliverableList.deliverablesList deliverables=action.getDeliverables(false) canValidate=true canEdit=candit isReportingActive=reportingActive namespace="/projects" defaultAction="${(crpSession)!}/deliverable"/]
          [/#if]
