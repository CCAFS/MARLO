...
[#macro relevantEvaluationMacro element name index template=false isEditable=true]
  ...
  <div id="evaluation-${template?string('template', index)}" class="evaluation borderBox form-group" style="position:relative; display:${template?string('none','block')}">
    ...
    <div class="form-group evidences-block">
      [@customForm.elementsListComponent name="${customName}.table11.evidence" elementType="projectExpectedStudy" elementList=element.projectExpectedStudies label="${customLabel}.table11.evidence" help="" helpIcon=false listName="selectedExpectedStudies" keyFieldName="id" displayFieldName="composedName" required=true /]
    </div>
    [#-- Comments --] 
    <div class="form-group">
