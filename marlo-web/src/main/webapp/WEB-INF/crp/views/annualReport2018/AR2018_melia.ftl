...
[#macro relevantEvaluationMacro element name index template=false isEditable=true]
  ...
  <div id="evaluation-${template?string('template', index)}" class="evaluation borderBox form-group" style="position:relative; display:${template?string('none','block')}">
    ...
    <div class="form-group evidences-block">
      [@customForm.elementsListComponent name="selectedExpectedStudies" elementType="projectExpectedStudy" elementList=selectedExpectedStudies label="${customLabel}.table11.evidence" help="" helpIcon=false listName="projectExpectedStudies" keyFieldName="id" displayFieldName="composedName" required=true /]
    </div>
    [#-- Comments --] 
    <div class="form-group">
