...
[#assign mainMenu= [
  ...
  { 'slug': 'synthesis', 'name': 'menu.synthesis',      'namespace': '/${reportingActive?string("synthesis","powb")}',       'action': '${(crpSession)!}/${reportingActive?string("crpIndicators","adjustmentsChanges")}',    'visible': logged && (action.canAcessCrp() || action.canAcessPOWB()), 'active': true,
    'subItems' : [
      ...
      { 'slug': 'powbReport', 'name': 'menu.synthesis.powbReport', 'namespace': '/powb',  'action': '${(crpSession)!}/adjustmentsChanges',  'visible': logged && !reportingActive, 'active':  !reportingActive },
