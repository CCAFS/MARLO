  ...
  <ul class="nav nav-tabs" role="tablist">
    ...
    [#assign link = "#area-${area.id}" /]
      <li role="areas" class="${isActive?string('active','')}">
        ...
        [#list area.researchPrograms as program][#if program_index == 0][#assign link][@s.url action='${centerSession}/projectList'][@s.param name="programID" value=program.id /][@s.param name="edit" value="true"/][/@s.url][/#assign][/#if][/#list]
    ...
    <div role="tabpanel" class="tab-pane ${isActive?string('active','')}" id="area-${area.id}">
      ...
      <ul>
        [#list area.researchPrograms as program]
          ...
          <li class="${isProgramActive?string('active','')}"> <a href="[@s.url action='${centerSession}/projectList'][@s.param name="programID" value=program.id /][@s.param name="edit" value="true"/][/@s.url]">${program.name}</a> </li>
        [/#list]
      </ul>
      [#else]
