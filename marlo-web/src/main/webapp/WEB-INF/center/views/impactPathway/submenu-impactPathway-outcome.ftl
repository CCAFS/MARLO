  ...
  <ul class="nav nav-tabs" role="tablist">
    ...
    [#assign link = "#area-${area.id}" /]
      <li role="areas" class="${isActive?string('active','')}">
        ...
        [#list area.researchPrograms as program][#if program_index == 0][#assign link][@s.url action='${centerSession}/outcomesList'][@s.param name="crpProgramID" value=program.id /][#include "/WEB-INF/global/pages/urlGlobalParams.ftl" /][/@s.url][/#assign][/#if][/#list]
    ...
    <div role="tabpanel" class="tab-pane ${isActive?string('active','')}" id="area-${area.id}">
      ...
      <ul>
        [#list area.researchPrograms as program]
          ...
          <li class="${isProgramActive?string('active','')}"> <a href="[@s.url action='${centerSession}/outcomesList'][@s.param name="crpProgramID" value=program.id /][#include "/WEB-INF/global/pages/urlGlobalParams.ftl" /][/@s.url]">${program.name}</a> </li>
        [/#list]
      </ul>
      [#else]
