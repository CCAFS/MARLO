...
[#macro textArea name editable value="-NULL" i18nkey="" disabled=false required=false errorfield="" help="" showTitle=true display=true className="-NULL" paramText="" editable=true placeholder=""]
  <div class="textArea" [#if !display]style="display: none;"[/#if]> 
    ...
    [#if showTitle]
      <label for="${name}" class="${editable?string('editable', 'readOnly')}"> [@s.text name="${customLabel}"][@s.param]${paramText}[/@s.param][/@s.text]:[#if required && editable]<span class="red">*</span>[/#if][#if help != ""]<img src="${baseUrlMedia}/images/global/icon-help3.png" title="[@s.text name="${help}"/]"/>[/#if]</label>
