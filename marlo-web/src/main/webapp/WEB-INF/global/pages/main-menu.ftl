...
[#assign mainMenu= [
  ...
  { 'slug': 'synthesis', 'name': 'menu.synthesis',      'namespace': '/synthesis',       'action': '${(crpSession)!}/crpIndicators',    'visible': logged, 'active': action.canAccessSuperAdmin() && reportingActive,
    'subItems' : [
      ...
      [#-- PHASE 1 --]
      { 'slug': 'outcomeSynthesis', 'name': 'menu.synthesis.outcomeSynthesis', 'namespace': '/synthesis',  'action': '${(crpSession)!}/outcomeSynthesisPandR',  'visible': logged && phaseOne, 'active': action.canAcessCrpAdmin() },
