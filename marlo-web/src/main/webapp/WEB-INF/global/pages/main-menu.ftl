...
[#assign mainMenu= [
  ...
  { 'slug': 'synthesis', 'name': 'menu.synthesis',      'namespace': '/annualReport${annualReport2018?string("2018", "")}',       'action': '${(crpSession)!}/crpProgress',    'visible': logged && reportingActive && !centerGlobalUnit && !upKeepActive, 'active': (actualPhase.year <= 2017),    
    'subItems' : [
      { 'slug': 'annualReport', 'name': 'menu.synthesis.annualReport', 'namespace': '/annualReport${annualReport2018?string("2018", "")}',  'action': '${(crpSession)!}/crpProgress',  'visible': logged, 'active': (actualPhase.year <= 2017), "development": true },
