...
[#assign mainMenu= [
  ...
  { 'slug': 'synthesis', 'name': 'menu.synthesis',      'namespace': '/annualReport${annualReport2018?string("2018", "")}',       'action': '${(crpSession)!}/${PMU?string("narrative","crpProgress")}',    'visible': logged && reportingActive && !centerGlobalUnit && !upKeepActive, 'active': true,    
    'subItems' : [
      { 'slug': 'annualReport', 'name': 'menu.synthesis.annualReport', 'namespace': '/annualReport${annualReport2018?string("2018", "")}',  'action': '${(crpSession)!}/${PMU?string("narrative","crpProgress")}',  'visible': logged, 'active': true },
