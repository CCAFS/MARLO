...
[#assign mainMenu= [
  ...
  { 'slug': 'publications', 'name': 'menu.publications',      'namespace': '/publications',       'action': '${(crpSession)!}/publicationsList',    'visible': logged, 'active': action.canAccessSuperAdmin() && reportingActive, 'help': true },
  { 'slug': 'synthesis', 'name': 'menu.synthesis',      'namespace': '/synthesis',       'action': '${(crpSession)!}/crpIndicators',    'visible': logged, 'active': action.canAccessSuperAdmin() && reportingActive,
    'subItems' : [
      ...
      [#-- PHASE 1 --]
      { 'slug': 'outcomeSynthesis', 'name': 'menu.synthesis.outcomeSynthesis', 'namespace': '/synthesis',  'action': '${(crpSession)!}/{crp}/outcomeSynthesisPandR',  'visible': logged && phaseOne, 'active': action.canAcessCrpAdmin() },
      { 'slug': 'synthesisByMog', 'name': 'menu.synthesis.synthesisByMog', 'namespace': '/synthesis',  'action': '${(crpSession)!}/synthesisByMog',  'visible': logged && phaseOne, 'active': action.canAcessCrpAdmin() },
      [#-- PHASE 2 --]
