...
$(document).ready(
		function() {
				...
				$(window).scroll(
						function() {
							if ($(window).scrollTop() >= mainMenuPosition) {
								$('#mainMenu .menuContent').addClass(
										'positionFixedTop');
							} else {
								$('#mainMenu .menuContent').removeClass(
										'positionFixedTop');
							}
						});
			}
			...
			if ($buttons.exists()) {
				var menuOffset = function() {
					return $(document).height()
							- ($buttons.offset().top + $buttons.height());
				}
				$buttons.find('.buttons-content').css(
						{
							right : $(document).width()
									- ($buttons.offset().left + $buttons
											.width())
						});
			...
			function setFixedElement(isFixed) {
				if (isFixed) {
					$buttons.find('.buttons-content').addClass(
							'positionFixedBot animated flipInX');
				} else {
					$buttons.find('.buttons-content').removeClass(
							'positionFixedBot animated flipInX');
				}
			}
			...
			function showNotificationMessages() {
				$('#generalMessages .messages').children("li").each(
						function(index) {
							// Validate if the notification is a warning
							// checking if the text contains --warn--
			...
			$tickBoxWp.each(function() {
				var $toggle = $(this).closest('.tickBox-wrapper').find(
						'.tickBox-toggle');
				if ($(this).is(':checked')) {
					$toggle.show();
				} else {
			...
			function toggleInputs(e) {
				$(this).parent().parent().parent().find('.tickBox-toggle')
						.slideToggle($(e.target).is(':checked'));
			}
	...
	$('[name=save], [name=next]')
			.on(
					...
					function(e) {
						...
						/*
						 * var fieldErrors = $(document).find('input.fieldError,
						 * textarea.fieldError').length; if(fieldErrors != 0) {
						 * e.preventDefault();
						 * $parent.find('.loading').fadeOut(500); var
						 * notyOptions = jQuery.extend({}, notyDefaultOptions);
						 * $('html, body').animate({ scrollTop:
						 * $('.fieldError').offset().top - 80 }, 700);
						 * notyOptions.text = 'Something is wrong in this
						 * section, please fix it then save'; noty(notyOptions); }
						 * else {
						 */
...
function setWordCounterToInputs(cssName) {
	...
	var check = cssName + "-";
	$(
			'input[type=text][class*="' + cssName + '"], textarea[class*="'
					+ cssName + '"]').each(function(i, input) {
...
$('input[name="user.email"] , input[name="user.password"]').on("keypress",
		function(event) {
