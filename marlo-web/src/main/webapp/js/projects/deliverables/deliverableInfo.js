...
function init() {
				...
				function(m) {
					for (var i = 0; i < m.deliverableSubTypes.length; i++) {
						$(".subTypeSelect").append(
								"<option value='" + m.deliverableSubTypes[i].id
										+ "' >" + m.deliverableSubTypes[i].name
										+ "</option>");
					}
				});
	}
	...
	$(".responsible").on(
			...
			function() {
				...
				var validation = $(this).parents(".fullBlock").parent().find(
						".personList")
						.find("input[value=" + option.val() + "]");
				if (option.val() != "-1") {
					if (validation.exists()) {
						...
						var text = option.html()
								+ ' was removed from contact persons list';
						notify(text);
						$(this).parents(".responsiblePartner").find(".id").val(
								option.val());
					} else {
						$(this).parents(".responsiblePartner").find(".id").val(
								option.val());
	...
	$(".partner").on(
			...
			function() {
				var option = $(this).find("option:selected");
				// validate if exists this person in contact person list
				var validation = $(this).parents(".partnerWrapper").find(
						".responsibleWrapper").find(
				...
				if (validation.exists()) {
					...
					// Show message
					var text = option.html()
							+ ' is the responsible person of this deliverable';
					notify(text);
				} else {
					$(this).parents(".deliverablePartner").find(".id").val(
							option.val());
				}
			});
...
function subTypes() {
	...
	typeSelect.on("change", function() {
		...
		subTypeSelect.empty();
		subTypeSelect
				.append("<option value='-1' >Select an option... </option>");
		...
		if (option.val() != "-1") {
			...
			$.ajax({
				...
				data : data
					...
					function(m) {
						for (var i = 0; i < m.deliverableSubTypes.length; i++) {
							subTypeSelect.append("<option value='"
									+ m.deliverableSubTypes[i].id + "' >"
									+ m.deliverableSubTypes[i].name
									+ "</option>");
						}
					});
			...
			data : data
				...
				function(m) {
					...
					for (var i = 0; i < m.keyOutputs.length; i++) {
						keyOutputSelect.append("<option value='"
								+ m.keyOutputs[i].id + "' >"
								+ m.keyOutputs[i].description + "</option>");
					}
				});
	});
}
