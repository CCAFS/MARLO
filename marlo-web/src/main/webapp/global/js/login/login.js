...
function firstForm() {
  ...
  $(
    '.selection-bar-options ul .selection-bar-image,' +
      '.selection-bar-options ul .selection-bar-acronym',
  ).addClass('hidden');
    ...
    success: function (data) {
      ...
      if (data.user == null || data.crps.length == 0) {
        ...
        $.each(data.crps, function (i) {
          ...
          $(
            '.selection-bar-options ul #crp-' + data.crps[i].acronym + ' .selection-bar-image',
          ).removeClass('hidden');
          $(
            '.selection-bar-options ul #crp-' + data.crps[i].acronym + ' .selection-bar-image',
          ).attr('tabindex', '0');
...
function loadSelectedImage(selectedImageAcronym) {
  $('#crpSelectedImage').attr(
    'src',
    baseUrl + '/data/globalUnitLogo.do?acronym=' + encodeURIComponent(selectedImageAcronym),
  );
}
...
function initJreject() {
  $.reject({
    ...
    display: ['firefox', 'chrome', 'opera', 'msie', 'safari'],
  });
}
