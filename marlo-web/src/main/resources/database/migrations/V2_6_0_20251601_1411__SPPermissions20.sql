DROP PROCEDURE IF EXISTS getPermissions;
DELIMITER ;;

CREATE PROCEDURE `getPermissions`(IN v_user_id BIGINT(20))
BEGIN
  DROP TABLE
IF EXISTS user_permission ; CREATE TEMPORARY TABLE user_permission (
  id VARCHAR (500),
  ro_acronym VARCHAR (500),
  role_id VARCHAR (500),
  permission VARCHAR (500),
  project_id VARCHAR (500),
  crp_acronym VARCHAR (500),
  permission_id VARCHAR (500)
) ENGINE = MyISAM DEFAULT CHARSET = utf8 ; INSERT INTO user_permission SELECT
  `u`.`id` AS `id`,
  `r`.`acronym` AS `ro_acronym`,
  `r`.`id` AS `role_id`,
  REPLACE (
    `p`.`permission`,
    '{0}',
    CONCAT(
      `cp`.`acronym`,
      ':',
      ph.description,
      ':',
      ph.`year`
    )
  ) AS `permission`,
  NULL AS `project_id`,
  `cp`.`acronym` AS `crp_acronym`,
  `p`.`id` AS `permission_id`
FROM
  (
    (
      (
        (
          (
            (
              `users` `u`
              LEFT JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
            )
            JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
          )
          JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
        )
        JOIN `permissions` `p` ON (
          (
            `p`.`id` = `rp`.`permission_id`
          )
        )
      )
      JOIN `crp_users` `crp` ON (
        (
          (`u`.`id` = `crp`.`user_id`)
          AND (
            `crp`.`global_unit_id` = `r`.`global_unit_id`
          )
        )
      )
    )
    JOIN `global_units` `cp` ON (
      (
        `cp`.`id` = `crp`.`global_unit_id`
      )
    )
    JOIN phases ph ON cp.id = ph.global_unit_id
  )
WHERE
  (
    (`p`.`type` = 0)
    AND (
      `r`.`id` <> 17
      AND r.acronym != 'FM'
    )
    AND (
      `cp`.`global_unit_type_id` = 1
      OR `cp`.`global_unit_type_id` = 3
      OR `cp`.`global_unit_type_id` = 4
      OR `cp`.`global_unit_type_id` = 5
    )
  )
AND ph.editable = 1
AND u.id = v_user_id
UNION
  SELECT
    `u`.`id` AS `id`,
    `r`.`acronym` AS `ro_acronym`,
    `r`.`id` AS `role_id`,
    REPLACE (
      `p`.`permission`,
      '{0}',
      `cp`.`acronym`
    ) AS `permission`,
    NULL AS `project_id`,
    `cp`.`acronym` AS `crp_acronym`,
    `p`.`id` AS `permission_id`
  FROM
    (
      (
        (
          (
            (
              `users` `u`
              LEFT JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
            )
            JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
          )
          JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
        )
        JOIN `permissions` `p` ON (
          (
            `p`.`id` = `rp`.`permission_id`
          )
        )
      )
      JOIN `global_units` `cp`
    )
  WHERE
    (
      (`p`.`type` = 0)
      AND (`r`.`id` in (select id from roles r where acronym in ('SuperAdmin'))) #-- r.id in 17 before data=17
      AND (
        `cp`.`global_unit_type_id` = 1
        OR `cp`.`global_unit_type_id` = 3
        OR `cp`.`global_unit_type_id` = 4
        OR `cp`.`global_unit_type_id` = 5
      )
    )
  AND u.id = v_user_id
  UNION
    SELECT
      `u`.`id` AS `id`,
      `r`.`acronym` AS `acronym`,
      `r`.`id` AS `rolid`,
      REPLACE (
        REPLACE (
          `p`.`permission`,
          '{0}',
          CONCAT(
            `cp`.`acronym`,
            ':',
            ph.description,
            ':',
            ph.`year`
          )
        ),
        '{1}',
        `pro`.`id`
      ) AS permission,
      NULL AS `project_id`,
      `cp`.`acronym` AS `crp_acronym`,
      `p`.`id` AS `permission_id`
    FROM
      (
        (
          (
            (
              (
                (
                  (
                    (
                      `users` `u`
                      JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                    )
                    JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                  )
                  JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                )
                JOIN `permissions` `p` ON (
                  (
                    `p`.`id` = `rp`.`permission_id`
                  )
                )
              )
              JOIN `crp_users` `crp` ON (
                (
                  (`u`.`id` = `crp`.`user_id`)
                  AND (
                    `crp`.`global_unit_id` = `r`.`global_unit_id`
                  )
                )
              )
            )
            JOIN `crp_program_leaders` `cprog` ON (
              (
                (`cprog`.`user_id` = `u`.`id`)
                AND (`cprog`.`is_active` = 1)
              )
            )
          )
          JOIN `crp_programs` `pro` ON (
            (
              (
                `pro`.`id` = `cprog`.`crp_program_id`
              )
              AND (`pro`.`program_type` = 1)
              AND (`pro`.`is_active` = 1)
            )
          )
        )
        JOIN `global_units` `cp` ON (
          (
            (
              `cp`.`id` = `crp`.`global_unit_id`
            )
            AND (
              `pro`.`global_unit_id` = `cp`.`id`
            )
          )
        )
        JOIN phases ph ON cp.id = ph.global_unit_id
      )
    WHERE
      (
        (`p`.`type` = 3)
        AND (
          `r`.`acronym` IN (
            'FPL',
            'FPM',
            'RPL',
            'RPM',
            'PMU'
          )
        )
        AND (
          `cp`.`global_unit_type_id` = 1
          OR `cp`.`global_unit_type_id` = 3
          OR `cp`.`global_unit_type_id` = 4
          OR `cp`.`global_unit_type_id` = 5
        )
        AND p.permission NOT IN (
          'crp:{0}:studies:{1}:canEdit'
        )
      )
    AND ph.editable = 1
    AND u.id = v_user_id
    UNION
      SELECT
        `u`.`id` AS `id`,
        `r`.`acronym` AS `acronym`,
        `r`.`id` AS `rolid`,
        REPLACE (
          REPLACE (
            `p`.`permission`,
            '{0}',
            CONCAT(
              `cp`.`acronym`,
              ':',
              ph.description,
              ':',
              ph.`year`
            )
          ),
          '{1}',
          `pro`.`id`
        ) AS `name_exp_32`,
        `pro`.`id` AS `project_id`,
        `cp`.`acronym` AS `crp_acronym`,
        `p`.`id` AS `permission_id`
      FROM
        (
          (
            (
              (
                (
                  (
                    (
                      (
                        (
                          `users` `u`
                          JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                        )
                        JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                      )
                      JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                    )
                    JOIN `permissions` `p` ON (
                      (
                        `p`.`id` = `rp`.`permission_id`
                      )
                    )
                  )
                  JOIN `crp_users` `crp` ON (
                    (
                      (`u`.`id` = `crp`.`user_id`)
                      AND (
                        `crp`.`global_unit_id` = `r`.`global_unit_id`
                      )
                    )
                  )
                )
                JOIN `global_units` `cp` ON (
                  (
                    `cp`.`id` = `crp`.`global_unit_id`
                  )
                )
                JOIN `global_unit_projects` `gup` ON (
                  (
                    `gup`.`global_unit_id` = `cp`.`id`
                  )
                )
              )
              JOIN `projects` `pro` ON (
                (
                  (
                    `pro`.`id` = `gup`.`project_id`
                  )
                  AND (`pro`.`is_active` = 1)
                )
              )
            )
            JOIN phases ph ON cp.id = ph.global_unit_id
            JOIN project_phases pph ON pph.project_id = pro.id
            AND pph.id_phase = ph.id
            INNER JOIN projects_info pinf ON pinf.id_phase = pph.id_phase
            AND pinf.project_id = pph.project_id
            INNER JOIN `liaison_institutions` `lin` ON (
              (
                `lin`.`id` = `pinf`.`liaison_institution_id`
              )
            )
          )
          JOIN `liaison_users` `lus` ON (
            (
              (
                `lus`.`institution_id` = `lin`.`id`
              )
              AND (`u`.`id` = `lus`.`user_id`)
              AND (`lus`.`is_active` = 1)
              AND lus.global_unit_id = cp.id
            )
          )
        )
      WHERE
        (
          (`p`.`type` = 1)
          AND (
            `r`.`acronym` IN (
              'PMU',
              'ML',
              'CP',
              'FPL',
              'FPM',
              'RPM',
              'RPL'
            )
          )
          AND (
            `cp`.`global_unit_type_id` = 1
            OR `cp`.`global_unit_type_id` = 3
            OR `cp`.`global_unit_type_id` = 4
            OR `cp`.`global_unit_type_id` = 5
          )
        )
      AND ph.editable = 1
      AND p.id NOT IN(560)
      AND u.id = v_user_id
      UNION
        SELECT
          `u`.`id` AS `id`,
          `r`.`acronym` AS `acronym`,
          `r`.`id` AS `rolid`,
          REPLACE (
            REPLACE (
              `p`.`permission`,
              '{0}',
              CONCAT(
                `cp`.`acronym`,
                ':',
                ph.description,
                ':',
                ph.`year`
              )
            ),
            '{1}',
            `pro`.`id`
          ) AS `name_exp_32`,
          `pro`.`id` AS `project_id`,
          `cp`.`acronym` AS `crp_acronym`,
          `p`.`id` AS `permission_id`
        FROM
          (
            (
              (
                (
                  (
                    (
                      (
                        (
                          (
                            `users` `u`
                            JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                          )
                          JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                        )
                        JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                      )
                      JOIN `permissions` `p` ON (
                        (
                          `p`.`id` = `rp`.`permission_id`
                        )
                      )
                    )
                    JOIN `crp_users` `crp` ON (
                      (
                        (`u`.`id` = `crp`.`user_id`)
                        AND (
                          `crp`.`global_unit_id` = `r`.`global_unit_id`
                        )
                      )
                    )
                  )
                  JOIN `global_units` `cp` ON (
                    (
                      `cp`.`id` = `crp`.`global_unit_id`
                    )
                  )
                  JOIN `global_unit_projects` `gup` ON (
                    (
                      `gup`.`global_unit_id` = `cp`.`id`
                    )
                  )
                )
                JOIN `projects` `pro` ON (
                  (
                    (
                      `pro`.`id` = `gup`.`project_id`
                    )
                    AND (`pro`.`is_active` = 1)
                  )
                )
              )
              JOIN phases ph ON cp.id = ph.global_unit_id
              JOIN project_phases pph ON pph.project_id = pro.id
              AND pph.id_phase = ph.id
              INNER JOIN projects_info pinf ON pinf.id_phase = pph.id_phase
              AND pinf.project_id = pph.project_id
              INNER JOIN `liaison_institutions` `lin` ON (
                (
                  `lin`.`id` = `pinf`.`liaison_institution_id`
                )
              )
            )
            JOIN `crp_program_leaders` `lus` ON (
              (
                (
                  `lus`.`crp_program_id` = `lin`.`crp_program`
                )
                AND lus.manager = 1
                AND (`u`.`id` = `lus`.`user_id`)
                AND (`lus`.`is_active` = 1)
              )
            )
          )
        WHERE
          (
            (`p`.`type` = 1)
            AND (
              `r`.`acronym` IN (
                'PMU',
                'ML',
                'CP',
                'FPL',
                'FPM',
                'RPM',
                'RPL'
              )
            )
            AND (
              `cp`.`global_unit_type_id` = 1
              OR `cp`.`global_unit_type_id` = 3
              OR `cp`.`global_unit_type_id` = 4
              OR `cp`.`global_unit_type_id` = 5
            )
          )
        AND ph.editable = 1
        AND u.id = v_user_id
        UNION
          SELECT
            `u`.`id` AS `id`,
            `r`.`acronym` AS `acronym`,
            `r`.`id` AS `rolid`,
            REPLACE (
              REPLACE (
                `p`.`permission`,
                '{0}',
                CONCAT(
                  `cp`.`acronym`,
                  ':',
                  ph.description,
                  ':',
                  ph.`year`
                )
              ),
              '{1}',
              `pro`.`id`
            ) AS `name_exp_32`,
            `pro`.`id` AS `project_id`,
            `cp`.`acronym` AS `crp_acronym`,
            `p`.`id` AS `permission_id`
          FROM
            (
              (
                (
                  (
                    (
                      (
                        (
                          (
                            (
                              `users` `u`
                              JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                            )
                            JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                          )
                          JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                        )
                        JOIN `permissions` `p` ON (
                          (
                            `p`.`id` = `rp`.`permission_id`
                          )
                        )
                      )
                      JOIN `crp_users` `crp` ON (
                        (
                          (`u`.`id` = `crp`.`user_id`)
                          AND (
                            `crp`.`global_unit_id` = `r`.`global_unit_id`
                          )
                        )
                      )
                    )
                    JOIN `global_units` `cp` ON (
                      (
                        `cp`.`id` = `crp`.`global_unit_id`
                      )
                    )
                    JOIN `global_unit_projects` `gup` ON (
                      (
                        `gup`.`global_unit_id` = `cp`.`id`
                      )
                    )
                  )
                  JOIN `projects` `pro` ON (
                    (
                      (
                        `pro`.`id` = `gup`.`project_id`
                      )
                      AND (`pro`.`is_active` = 1)
                    )
                  )
                )
                JOIN phases ph ON cp.id = ph.global_unit_id
                JOIN project_phases pph ON pph.project_id = pro.id
                AND pph.id_phase = ph.id
                INNER JOIN projects_info pinf ON pinf.id_phase = pph.id_phase
                AND pinf.project_id = pph.project_id
                INNER JOIN `project_cluster_activities` `lin` ON (
                  (
                    `lin`.`project_id` = `pro`.`id`
                    AND lin.is_active = 1
                  )
                )
              )
              JOIN `crp_cluster_activity_leaders` `lus` ON (
                (
                  (
                    `lus`.`cluster_activity_id` = `lin`.`cluster_activity_id`
                  )
                  AND (`u`.`id` = `lus`.`user_id`)
                  AND (`lus`.`is_active` = 1)
                )
              )
            )
          WHERE
            (
              (`p`.`type` = 1)
              AND (`r`.`acronym` IN('CL'))
              AND (
                `cp`.`global_unit_type_id` = 1
                OR `cp`.`global_unit_type_id` = 3
                OR `cp`.`global_unit_type_id` = 4
                OR `cp`.`global_unit_type_id` = 5
              )
            )
          AND ph.editable = 1
          AND u.id = v_user_id
          UNION
            SELECT
              `u`.`id` AS `id`,
              `r`.`acronym` AS `acronym`,
              `r`.`id` AS `rolid`,
              REPLACE (
                REPLACE (
                  `p`.`permission`,
                  '{0}',
                  CONCAT(
                    `cp`.`acronym`,
                    ':',
                    ph.description,
                    ':',
                    ph.`year`
                  )
                ),
                '{1}',
                `pro`.`id`
              ) AS `name_exp_39`,
              `pro`.`id` AS `project_id`,
              `cp`.`acronym` AS `crp_acronym`,
              `p`.`id` AS `permission_id`
            FROM
              (
                (
                  (
                    (
                      (
                        (
                          (
                            (
                              `users` `u`
                              JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                            )
                            JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                          )
                          JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                        )
                        JOIN `permissions` `p` ON (
                          (
                            `p`.`id` = `rp`.`permission_id`
                          )
                        )
                      )
                      JOIN `crp_users` `crp` ON (
                        (
                          (`u`.`id` = `crp`.`user_id`)
                          AND (
                            `crp`.`global_unit_id` = `r`.`global_unit_id`
                          )
                        )
                      )
                    )
                    JOIN `global_units` `cp` ON (
                      (
                        `cp`.`id` = `crp`.`global_unit_id`
                      )
                    )
                    JOIN `global_unit_projects` `gup` ON (
                      (
                        `gup`.`global_unit_id` = `cp`.`id`
                      )
                    )
                  )
                  JOIN `projects` `pro` ON (
                    (
                      (
                        `pro`.`id` = `gup`.`project_id`
                      )
                      AND (`pro`.`is_active` = 1)
                    )
                  )
                )
                JOIN phases ph ON cp.id = ph.global_unit_id
                JOIN project_phases pph ON pph.project_id = pro.id
                AND pph.id_phase = ph.id
                INNER JOIN projects_info pinf ON pinf.id_phase = pph.id_phase
                AND pinf.project_id = pph.project_id
                INNER JOIN `liaison_users` `lus` ON (
                  (
                    (
                      `lus`.`id` = `pinf`.`liaison_user_id`
                    )
                    AND (`u`.`id` = `lus`.`user_id`)
                    AND (`lus`.`is_active` = 1)
                    AND lus.global_unit_id = cp.id
                  )
                )
              )
            WHERE
              (
                (`p`.`type` = 1)
                AND (
                  `r`.`acronym` IN (
                    'PMU',
                    'ML',
                    'CP',
                    'FPL',
                    'FPM',
                    'RPM',
                    'RPL'
                  )
                )
                AND (
                  `cp`.`global_unit_type_id` = 1
                  OR `cp`.`global_unit_type_id` = 3
                  OR `cp`.`global_unit_type_id` = 4
                  OR `cp`.`global_unit_type_id` = 5
                )
              )
            AND p.id NOT IN(560)
            AND ph.editable = 1
            AND u.id = v_user_id
            UNION
              SELECT
                `u`.`id` AS `id`,
                `r`.`acronym` AS `acronym`,
                `r`.`id` AS `rolid`,
                REPLACE (
                  REPLACE (
                    `p`.`permission`,
                    '{0}',
                    CONCAT(
                      `cp`.`acronym`,
                      ':',
                      ph.description,
                      ':',
                      ph.`year`
                    )
                  ),
                  '{1}',
                  `pro`.`id`
                ) AS `name_exp_39`,
                `pro`.`id` AS `project_id`,
                `cp`.`acronym` AS `crp_acronym`,
                `p`.`id` AS `permission_id`
              FROM
                (
                  (
                    (
                      (
                        (
                          (
                            (
                              (
                                `users` `u`
                                JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                              )
                              JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                            )
                            JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                          )
                          JOIN `permissions` `p` ON (
                            (
                              `p`.`id` = `rp`.`permission_id`
                            )
                          )
                        )
                        JOIN `crp_users` `crp` ON (
                          (
                            (`u`.`id` = `crp`.`user_id`)
                            AND (
                              `crp`.`global_unit_id` = `r`.`global_unit_id`
                            )
                          )
                        )
                      )
                      JOIN `global_units` `cp` ON (
                        (
                          `cp`.`id` = `crp`.`global_unit_id`
                        )
                      )
                      JOIN `global_unit_projects` `gup` ON (
                        (
                          `gup`.`global_unit_id` = `cp`.`id`
                        )
                      )
                    )
                    JOIN `projects` `pro` ON (
                      (
                        (
                          `pro`.`id` = `gup`.`project_id`
                        )
                        AND (`pro`.`is_active` = 1)
                      )
                    )
                    JOIN phases ph ON cp.id = ph.global_unit_id
                    JOIN project_phases pph ON pph.project_id = pro.id
                    AND pph.id_phase = ph.id
                    INNER JOIN projects_info pinf ON pinf.id_phase = pph.id_phase
                    AND pinf.project_id = pph.project_id
                  )
                )
              WHERE
                (
                  (`p`.`type` = 1 AND p.id = 2)
                  AND (`r`.`acronym` IN('PMU'))
                  AND (
                    `cp`.`global_unit_type_id` = 1
                    OR `cp`.`global_unit_type_id` = 3
                    OR `cp`.`global_unit_type_id` = 4
                    OR `cp`.`global_unit_type_id` = 5
                  )
                )
              AND ph.editable = 1
              AND u.id = v_user_id
              UNION
                SELECT
                  `u`.`id` AS `id`,
                  `r`.`acronym` AS `acronym`,
                  `r`.`id` AS `rolid`,
                  REPLACE (
                    REPLACE (
                      `p`.`permission`,
                      '{0}',
                      CONCAT(
                        `cp`.`acronym`,
                        ':',
                        ph.description,
                        ':',
                        ph.`year`
                      )
                    ),
                    '{1}',
                    `pro`.`id`
                  ) AS `name_exp_46`,
                  `pro`.`id` AS `project_id`,
                  `cp`.`acronym` AS `crp_acronym`,
                  `p`.`id` AS `permission_id`
                FROM
                  (
                    (
                      (
                        (
                          (
                            (
                              (
                                (
                                  (
                                    `users` `u`
                                    JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                                  )
                                  JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                                )
                                JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                              )
                              JOIN `permissions` `p` ON (
                                (
                                  `p`.`id` = `rp`.`permission_id`
                                )
                              )
                            )
                            JOIN `crp_users` `crp` ON (
                              (
                                (`u`.`id` = `crp`.`user_id`)
                                AND (
                                  `crp`.`global_unit_id` = `r`.`global_unit_id`
                                )
                              )
                            )
                          )
                          JOIN `global_units` `cp` ON (
                            (
                              `cp`.`id` = `crp`.`global_unit_id`
                            )
                          )
                          JOIN `global_unit_projects` `gup` ON (
                            (
                              `gup`.`global_unit_id` = `cp`.`id`
                            )
                          )
                        )
                        JOIN `projects` `pro` ON (
                          (
                            (
                              `pro`.`id` = `gup`.`project_id`
                            )
                            AND (`pro`.`is_active` = 1)
                          )
                        )
                        JOIN phases ph ON cp.id = ph.global_unit_id
                        JOIN project_phases pph ON pph.project_id = pro.id
                        AND pph.id_phase = ph.id
                        INNER JOIN projects_info pinf ON pinf.id_phase = pph.id_phase
                        AND pinf.project_id = pph.project_id
                      )
                      JOIN `project_partners` `pp` ON (
                        (
                          (
                            `pp`.`project_id` = `pro`.`id`
                          )
                          AND (`pp`.`is_active` = 1)
                        )
                      )
                    )
                    JOIN `project_partner_persons` `pers` ON (
                      (
                        (
                          `pp`.`id` = `pers`.`project_partner_id`
                        )
                        AND (`pers`.`is_active` = 1)
                        AND (`pers`.`contact_type` = 'PL')
                        AND (`pers`.`user_id` = `u`.`id`)
                      )
                    )
                  )
                WHERE
                  (
                    (`p`.`type` = 1)
                    AND (`r`.`acronym` = 'PL')
                    AND (
                      `cp`.`global_unit_type_id` = 1
                      OR `cp`.`global_unit_type_id` = 3
                      OR `cp`.`global_unit_type_id` = 4
                      OR `cp`.`global_unit_type_id` = 5
                    )
                  )
                AND ph.editable = 1
                AND u.id = v_user_id
                UNION
                  SELECT
                    `u`.`id` AS `id`,
                    `r`.`acronym` AS `acronym`,
                    `r`.`id` AS `rolid`,
                    REPLACE (
                      REPLACE (
                        `p`.`permission`,
                        '{0}',
                        CONCAT(
                          `cp`.`acronym`,
                          ':',
                          ph.description,
                          ':',
                          ph.`year`
                        )
                      ),
                      '{1}',
                      `pro`.`id`
                    ) AS `name_exp_46`,
                    `pro`.`id` AS `project_id`,
                    `cp`.`acronym` AS `crp_acronym`,
                    `p`.`id` AS `permission_id`
                  FROM
                    (
                      (
                        (
                          (
                            (
                              (
                                (
                                  (
                                    (
                                      `users` `u`
                                      JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                                    )
                                    JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                                  )
                                  JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                                )
                                JOIN `permissions` `p` ON (
                                  (
                                    `p`.`id` = `rp`.`permission_id`
                                  )
                                )
                              )
                              JOIN `crp_users` `crp` ON (
                                (
                                  (`u`.`id` = `crp`.`user_id`)
                                  AND (
                                    `crp`.`global_unit_id` = `r`.`global_unit_id`
                                  )
                                )
                              )
                            )
                            JOIN `global_units` `cp` ON (
                              (
                                `cp`.`id` = `crp`.`global_unit_id`
                              )
                            )
                            JOIN `global_unit_projects` `gup` ON (
                              (
                                `gup`.`global_unit_id` = `cp`.`id`
                              )
                            )
                          )
                          JOIN `projects` `pro` ON (
                            (
                              (
                                `pro`.`id` = `gup`.`project_id`
                              )
                              AND (`pro`.`is_active` = 1)
                            )
                          )
                          JOIN phases ph ON cp.id = ph.global_unit_id
                          JOIN project_phases pph ON pph.project_id = pro.id
                          AND pph.id_phase = ph.id
                          INNER JOIN projects_info pinf ON pinf.id_phase = pph.id_phase
                          AND pinf.project_id = pph.project_id
                        )
                        JOIN `project_partners` `pp` ON (
                          (
                            (
                              `pp`.`project_id` = `pro`.`id`
                            )
                            AND (`pp`.`is_active` = 1)
                          )
                        )
                      )
                      JOIN `project_partner_persons` `pers` ON (
                        (
                          (
                            `pp`.`id` = `pers`.`project_partner_id`
                          )
                          AND (`pers`.`is_active` = 1)
                          AND (`pers`.`contact_type` = 'PC')
                          AND (`pers`.`user_id` = `u`.`id`)
                        )
                      )
                    )
                  WHERE
                    (
                      (`p`.`type` = 1)
                      AND (`r`.`acronym` = 'PC')
                      AND (
                        `cp`.`global_unit_type_id` = 1
                        OR `cp`.`global_unit_type_id` = 3
                        OR `cp`.`global_unit_type_id` = 4
                        OR `cp`.`global_unit_type_id` = 5
                      )
                    )
                  AND ph.editable = 1
                  AND u.id = v_user_id
                  UNION
                    SELECT
                      `u`.`id` AS `id`,
                      `r`.`acronym` AS `acronym`,
                      `r`.`id` AS `rolid`,
                      REPLACE (
                        REPLACE (
                          `p`.`permission`,
                          '{0}',
                          CONCAT(
                            `cp`.`acronym`,
                            ':',
                            ph.description,
                            ':',
                            ph.`year`
                          )
                        ),
                        '{1}',
                        `pro`.`id`
                      ) AS `name_exp_46`,
                      `pro`.`id` AS `project_id`,
                      `cp`.`acronym` AS `crp_acronym`,
                      `p`.`id` AS `permission_id`
                    FROM
                      (
                        (
                          (
                            (
                              (
                                (
                                  (
                                    `users` `u`
                                    JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                                  )
                                  JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                                )
                                JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                              )
                              JOIN `permissions` `p` ON (
                                (
                                  `p`.`id` = `rp`.`permission_id`
                                )
                              )
                            )
                            JOIN `crp_users` `crp` ON (
                              (
                                (`u`.`id` = `crp`.`user_id`)
                                AND (
                                  `crp`.`global_unit_id` = `r`.`global_unit_id`
                                )
                              )
                            )
                          )
                          JOIN `global_units` `cp` ON (
                            (
                              `cp`.`id` = `crp`.`global_unit_id`
                            )
                          )
                          JOIN phases ph ON cp.id = ph.global_unit_id
                        )
                        JOIN `funding_sources` `pro` ON (
                          (
                            (
                              `pro`.`global_unit_id` = `cp`.`id`
                            )
                            AND (`pro`.`is_active` = 1)
                          )
                        )
                        INNER JOIN `funding_sources_info` `pro_info` ON (
                          (
                            (
                              `pro_info`.`funding_source_id` = `pro`.`id`
                            )
                          )
                        )
                      )
                    WHERE
                      `p`.`id` IN (438, 462)
                    AND (
                      pro_info.type <> 1
                      OR pro_info.type IS NULL
                    )
                    AND (
                      `cp`.`global_unit_type_id` = 1
                      OR `cp`.`global_unit_type_id` = 3
                      OR `cp`.`global_unit_type_id` = 4
                      OR `cp`.`global_unit_type_id` = 5
                    )
                    AND r.`acronym` IN (
                      'PMU',
                      'ML',
                      'FPL',
                      'FPM',
                      'RPM',
                      'RPL'
                    )
                    AND ph.editable = 1
                    AND u.id = v_user_id
                    UNION
                      SELECT
                        u.id,
                        `ro`.`acronym` AS `acronym`,
                        `ro`.`id` AS `rolid`,
                        REPLACE (
                          REPLACE (
                            `per`.`permission`,
                            '{0}',
                            CONCAT(`cp`.`acronym`)
                          ),
                          '{1}',
                          pro.id
                        ) AS `name_exp_46`,
                        pro.id AS `project_id`,
                        `cp`.`acronym` AS `crp_acronym`,
                        `per`.`id` AS `permission_id`
                      FROM
                        users u
                      INNER JOIN user_roles urol ON urol.user_id = u.id
                      INNER JOIN roles ro ON ro.id = urol.role_id
                      AND ro.acronym = 'FM'
                      INNER JOIN crp_users cpu ON cpu.user_id = u.id
                      AND cpu.is_active = 1
                      INNER JOIN global_units cp ON cp.id = cpu.global_unit_id
                      INNER JOIN funding_sources pro ON pro.global_unit_id = cp.id
                      AND pro.is_active = 1
                      INNER JOIN role_permissions rp ON rp.role_id = ro.id
                      INNER JOIN permissions per ON per.id = rp.permission_id
                      WHERE
                        u.is_active = 1
                      AND per.id IN (438, 462)
                      AND u.id = v_user_id
                      AND cp.global_unit_type_id = 1
                      UNION
                        SELECT
                          u.id,
                          `ro`.`acronym` AS `acronym`,
                          `ro`.`id` AS `rolid`,
                          REPLACE (
                            REPLACE (
                              `per`.`permission`,
                              '{0}',
                              CONCAT(
                                `cp`.`acronym`,
                                ':',
                                ph.description,
                                ':',
                                ph.`year`
                              )
                            ),
                            '{1}',
                            pro.id
                          ) AS `name_exp_46`,
                          pro.id AS `project_id`,
                          `cp`.`acronym` AS `crp_acronym`,
                          `per`.`id` AS `permission_id`
                        FROM
                          users u
                        INNER JOIN user_roles urol ON urol.user_id = u.id
                        INNER JOIN roles ro ON ro.id = urol.role_id
                        AND ro.acronym = 'FM'
                        INNER JOIN crp_users cpu ON cpu.user_id = u.id
                        AND cpu.is_active = 1
                        INNER JOIN global_units cp ON cp.id = cpu.global_unit_id
                        INNER JOIN global_unit_projects gup ON gup.global_unit_id = cp.id
                        INNER JOIN projects pro ON pro.id = gup.project_id
                        JOIN phases ph ON cp.id = ph.global_unit_id
                        JOIN project_phases pph ON pph.project_id = pro.id
                        AND pph.id_phase = ph.id
                        INNER JOIN projects_info pinf ON pinf.id_phase = pph.id_phase
                        AND pinf.project_id = pph.project_id
                        INNER JOIN role_permissions rp ON rp.role_id = ro.id
                        INNER JOIN permissions per ON per.id = rp.permission_id
                        WHERE
                          u.is_active = 1
                        AND per.id NOT IN (438, 462)
                        AND pro.is_active = 1
                        AND ph.editable = 1
                        AND u.id = v_user_id
                        AND cp.global_unit_type_id = 1
                        UNION
                          SELECT
                            u.id,
                            `ro`.`acronym` AS `acronym`,
                            `ro`.`id` AS `rolid`,
                            REPLACE (
                              REPLACE (
                                `per`.`permission`,
                                '{0}',
                                CONCAT(`cp`.`acronym`)
                              ),
                              '{1}',
                              pro.id
                            ) AS `name_exp_46`,
                            pro.id AS `project_id`,
                            `cp`.`acronym` AS `crp_acronym`,
                            `per`.`id` AS `permission_id`
                          FROM
                            users u
                          INNER JOIN user_roles urol ON urol.user_id = u.id
                          INNER JOIN roles ro ON ro.id = urol.role_id
                          AND ro.acronym = 'FM'
                          INNER JOIN crp_users cpu ON cpu.user_id = u.id
                          AND cpu.is_active = 1
                          INNER JOIN global_units cp ON cp.id = cpu.global_unit_id
                          INNER JOIN funding_sources pro ON pro.global_unit_id = cp.id
                          INNER JOIN role_permissions rp ON rp.role_id = ro.id
                          INNER JOIN permissions per ON per.id = rp.permission_id
                          WHERE
                            u.is_active = 1
                          AND per.id IN (438, 462)
                          AND pro.is_active = 1
                          AND u.id = v_user_id
                          AND cp.global_unit_type_id = 1
                          UNION
                            SELECT
                              `u`.`id` AS `id`,
                              `r`.`acronym` AS `acronym`,
                              `r`.`id` AS `rolid`,
                              REPLACE (
                                REPLACE (
                                  REPLACE (
                                    `p`.`permission`,
                                    '{0}',
                                    CONCAT(
                                      `cp`.`acronym`,
                                      ':',
                                      ph.description,
                                      ':',
                                      ph.`year`
                                    )
                                  ),
                                  '{1}',
                                  `pro`.`id`
                                ),
                                '{2}',
                                lin.institution_id
                              ) AS `name_exp_32`,
                              `pro`.`id` AS `project_id`,
                              `cp`.`acronym` AS `crp_acronym`,
                              `p`.`id` AS `permission_id`
                            FROM
                              (
                                (
                                  (
                                    (
                                      (
                                        (
                                          (
                                            (
                                              (
                                                `users` `u`
                                                JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                                              )
                                              JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                                            )
                                            JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                                          )
                                          JOIN `permissions` `p` ON (
                                            (
                                              `p`.`id` = `rp`.`permission_id`
                                            )
                                          )
                                        )
                                        JOIN `crp_users` `crp` ON (
                                          (
                                            (`u`.`id` = `crp`.`user_id`)
                                            AND (
                                              `crp`.`global_unit_id` = `r`.`global_unit_id`
                                            )
                                          )
                                        )
                                      )
                                      JOIN `global_units` `cp` ON (
                                        (
                                          `cp`.`id` = `crp`.`global_unit_id`
                                        )
                                      )
                                      JOIN `global_unit_projects` `gup` ON (
                                        (
                                          `gup`.`global_unit_id` = `cp`.`id`
                                        )
                                      )
                                    )
                                    JOIN `projects` `pro` ON (
                                      (
                                        (
                                          `pro`.`id` = `gup`.`project_id`
                                        )
                                        AND (`pro`.`is_active` = 1)
                                      )
                                    )
                                    JOIN phases ph ON cp.id = ph.global_unit_id
                                    JOIN project_phases pph ON pph.project_id = pro.id
                                    AND pph.id_phase = ph.id
                                    INNER JOIN projects_info pinf ON pinf.id_phase = pph.id_phase
                                    AND pinf.project_id = pph.project_id
                                  )
                                  JOIN `liaison_users` `lus` ON (
                                    (
                                      (`u`.`id` = `lus`.`user_id`)
                                      AND (`lus`.`is_active` = 1)
                                      AND lus.global_unit_id = cp.id
                                    )
                                  )
                                )
                                JOIN `liaison_institutions` `lin` ON (
                                  (
                                    `lus`.`institution_id` = `lin`.`id`
                                  )
                                )
                              )
                            INNER JOIN project_partners pbi ON pbi.institution_id = lin.institution_id
                            AND pro.id = pbi.project_id
                            AND pbi.is_active = 1
                            WHERE
                              (
                                (`p`.`type` = 1)
                                AND (`r`.`acronym` IN('CP'))
                                AND (
                                  `cp`.`global_unit_type_id` = 1
                                  OR `cp`.`global_unit_type_id` = 3
                                  OR `cp`.`global_unit_type_id` = 4
                                  OR `cp`.`global_unit_type_id` = 5
                                )
                              )
                            AND p.id IN (447, 206, 429, 488,560)
                            AND ph.editable = 1
                            AND u.id = v_user_id
                            UNION
                              SELECT
                                `u`.`id` AS `id`,
                                `r`.`acronym` AS `acronym`,
                                `r`.`id` AS `rolid`,
                                REPLACE (
                                  REPLACE (
                                    `p`.`permission`,
                                    '{0}',
                                    CONCAT(
                                      `cp`.`acronym`,
                                      ':',
                                      ph.description,
                                      ':',
                                      ph.`year`
                                    )
                                  ),
                                  '{1}',
                                  `pro`.`id`
                                ) AS `name_exp_46`,
                                `pro`.`id` AS `project_id`,
                                `cp`.`acronym` AS `crp_acronym`,
                                `p`.`id` AS `permission_id`
                              FROM
                                (
                                  (
                                    (
                                      (
                                        (
                                          (
                                            (
                                              `users` `u`
                                              JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                                            )
                                            JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                                          )
                                          JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                                        )
                                        JOIN `permissions` `p` ON (
                                          (
                                            `p`.`id` = `rp`.`permission_id`
                                          )
                                        )
                                      )
                                      JOIN `crp_users` `crp` ON (
                                        (
                                          (`u`.`id` = `crp`.`user_id`)
                                          AND (
                                            `crp`.`global_unit_id` = `r`.`global_unit_id`
                                          )
                                        )
                                      )
                                    )
                                    JOIN `global_units` `cp` ON (
                                      (
                                        `cp`.`id` = `crp`.`global_unit_id`
                                      )
                                    )
                                    JOIN phases ph ON cp.id = ph.global_unit_id
                                  )
                                  JOIN `funding_sources` `pro` ON (
                                    (
                                      (
                                        `pro`.`global_unit_id` = `cp`.`id`
                                      )
                                      AND (`pro`.`is_active` = 1)
                                    )
                                  )
                                  INNER JOIN `funding_sources_info` `pro_info` ON (
                                    (
                                      (
                                        `pro_info`.`funding_source_id` = `pro`.`id`
                                      )
                                    )
                                  )
                                )
                              WHERE
                                `p`.`id` IN (438, 462)
                              AND (
                                pro_info.type <> 1
                                OR pro_info.type IS NULL
                              )
                              AND (
                                `cp`.`global_unit_type_id` = 1
                                OR `cp`.`global_unit_type_id` = 3
                                OR `cp`.`global_unit_type_id` = 4
                                OR `cp`.`global_unit_type_id` = 5
                              )
                              AND r.`acronym` IN ('CP')
                              AND ph.editable = 1
                              AND u.id = v_user_id
                              UNION
                                SELECT
                                  `u`.`id` AS `id`,
                                  `r`.`acronym` AS `acronym`,
                                  `r`.`id` AS `rolid`,
                                  REPLACE (
                                    REPLACE (
                                      `p`.`permission`,
                                      '{0}',
                                      CONCAT(`cp`.`acronym`)
                                    ),
                                    '{1}',
                                    `pro`.`id`
                                  ) AS `name_exp_46`,
                                  NULL AS `project_id`,
                                  `cp`.`acronym` AS `crp_acronym`,
                                  `p`.`id` AS `permission_id`
                                FROM
                                  (
                                    (
                                      (
                                        (
                                          (
                                            (
                                              (
                                                `users` `u`
                                                JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                                              )
                                              JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                                            )
                                            JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                                          )
                                          JOIN `permissions` `p` ON (
                                            (
                                              `p`.`id` = `rp`.`permission_id`
                                            )
                                          )
                                        )
                                        JOIN `crp_users` `crp` ON (
                                          (
                                            (`u`.`id` = `crp`.`user_id`)
                                            AND (
                                              `crp`.`global_unit_id` = `r`.`global_unit_id`
                                            )
                                          )
                                        )
                                      )
                                      JOIN `global_units` `cp` ON (
                                        (
                                          `cp`.`id` = `crp`.`global_unit_id`
                                        )
                                      )
                                    )
                                    JOIN `deliverables` `pro` ON (
                                      (
                                        (
                                          `pro`.`global_unit_id` = `cp`.`id`
                                        )
                                        AND (`pro`.`is_active` = 1)
                                      )
                                    )
                                  )
                                INNER JOIN deliverable_leaders fin ON fin.deliverable_id = pro.id
                                INNER JOIN liaison_users lu ON lu.user_id = u.id
                                AND lu.global_unit_id = cp.id
                                INNER JOIN liaison_institutions li ON li.id = lu.institution_id
                                AND li.institution_id = fin.instituion_id
                                WHERE
                                  `p`.`id` = 467
                                AND r.`acronym` IN ('CP')
                                AND u.id = v_user_id
                                AND (
                                  `cp`.`global_unit_type_id` = 1
                                  OR `cp`.`global_unit_type_id` = 3
                                  OR `cp`.`global_unit_type_id` = 4
                                  OR `cp`.`global_unit_type_id` = 5
                                )
                                UNION
                                  SELECT
                                    `u`.`id` AS `id`,
                                    `r`.`acronym` AS `acronym`,
                                    `r`.`id` AS `rolid`,
                                    REPLACE (
                                      REPLACE (
                                        `p`.`permission`,
                                        '{0}',
                                        CONCAT(`cp`.`acronym`)
                                      ),
                                      '{1}',
                                      `li`.`ip_program`
                                    ) AS `name_exp_46`,
                                    NULL AS `project_id`,
                                    `cp`.`acronym` AS `crp_acronym`,
                                    `p`.`id` AS `permission_id`
                                  FROM
                                    (
                                      (
                                        (
                                          (
                                            (
                                              (
                                                (
                                                  `users` `u`
                                                  JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                                                )
                                                JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                                              )
                                              JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                                            )
                                            JOIN `permissions` `p` ON (
                                              (
                                                `p`.`id` = `rp`.`permission_id`
                                              )
                                            )
                                          )
                                          JOIN `crp_users` `crp` ON (
                                            (
                                              (`u`.`id` = `crp`.`user_id`)
                                              AND (
                                                `crp`.`global_unit_id` = `r`.`global_unit_id`
                                              )
                                            )
                                          )
                                        )
                                        JOIN `global_units` `cp` ON (
                                          (
                                            `cp`.`id` = `crp`.`global_unit_id`
                                          )
                                        )
                                      )
                                    )
                                  INNER JOIN ip_liaison_users lu ON lu.user_id = u.id
                                  INNER JOIN ip_liaison_institutions li ON li.id = lu.institution_id
                                  WHERE
                                    `p`.`id` = 464
                                  AND r.`acronym` IN ('FPL', 'FPM', 'RPM', 'RPL')
                                  AND u.id = v_user_id
                                  AND (
                                    `cp`.`global_unit_type_id` = 1
                                    OR `cp`.`global_unit_type_id` = 3
                                    OR `cp`.`global_unit_type_id` = 4
                                    OR `cp`.`global_unit_type_id` = 5
                                  )
                                  UNION
                                    SELECT
                                      `u`.`id` AS `id`,
                                      `r`.`acronym` AS `acronym`,
                                      `r`.`id` AS `rolid`,
                                      REPLACE (
                                        REPLACE (
                                          `p`.`permission`,
                                          '{0}',
                                          CONCAT(`cp`.`acronym`)
                                        ),
                                        '{1}',
                                        `li`.`id`
                                      ) AS `name_exp_46`,
                                      NULL AS `project_id`,
                                      `cp`.`acronym` AS `crp_acronym`,
                                      `p`.`id` AS `permission_id`
                                    FROM
                                      (
                                        (
                                          (
                                            (
                                              (
                                                (
                                                  (
                                                    `users` `u`
                                                    JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                                                  )
                                                  JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                                                )
                                                JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                                              )
                                              JOIN `permissions` `p` ON (
                                                (
                                                  `p`.`id` = `rp`.`permission_id`
                                                )
                                              )
                                            )
                                            JOIN `crp_users` `crp` ON (
                                              (
                                                (`u`.`id` = `crp`.`user_id`)
                                                AND (
                                                  `crp`.`global_unit_id` = `r`.`global_unit_id`
                                                )
                                              )
                                            )
                                          )
                                          JOIN `global_units` `cp` ON (
                                            (
                                              `cp`.`id` = `crp`.`global_unit_id`
                                            )
                                          )
                                        )
                                      )
                                    INNER JOIN liaison_users lu ON lu.user_id = u.id
                                    AND lu.global_unit_id = cp.id
                                    INNER JOIN liaison_institutions li ON li.id = lu.institution_id
                                    WHERE
                                      `p`.`id` = 468
                                    AND r.`acronym` IN (
                                      'FPL',
                                      'FPM',
                                      'RPM',
                                      'RPL',
                                      'CP'
                                    )
                                    AND (
                                      `cp`.`global_unit_type_id` = 1
                                      OR `cp`.`global_unit_type_id` = 3
                                      OR `cp`.`global_unit_type_id` = 4
                                      OR `cp`.`global_unit_type_id` = 5
                                    )
                                    AND u.id = v_user_id
                                    UNION
                                      SELECT
                                        `u`.`id` AS `id`,
                                        `r`.`acronym` AS `acronym`,
                                        `r`.`id` AS `rolid`,
                                        REPLACE (
                                          REPLACE (
                                            `p`.`permission`,
                                            '{0}',
                                            CONCAT(`cp`.`acronym`)
                                          ),
                                          '{1}',
                                          `pro`.`id`
                                        ) AS `name_exp_46`,
                                        `pro`.`id` AS `project_id`,
                                        `cp`.`acronym` AS `crp_acronym`,
                                        `p`.`id` AS `permission_id`
                                      FROM
                                        (
                                          (
                                            (
                                              (
                                                (
                                                  (
                                                    (
                                                      `users` `u`
                                                      JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                                                    )
                                                    JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                                                  )
                                                  JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                                                )
                                                JOIN `permissions` `p` ON (
                                                  (
                                                    `p`.`id` = `rp`.`permission_id`
                                                  )
                                                )
                                              )
                                              JOIN `crp_users` `crp` ON (
                                                (
                                                  (`u`.`id` = `crp`.`user_id`)
                                                  AND (
                                                    `crp`.`global_unit_id` = `r`.`global_unit_id`
                                                  )
                                                )
                                              )
                                            )
                                            JOIN `global_units` `cp` ON (
                                              (
                                                `cp`.`id` = `crp`.`global_unit_id`
                                              )
                                            )
                                          )
                                          JOIN `funding_sources` `pro` ON (
                                            (
                                              (
                                                `pro`.`global_unit_id` = `cp`.`id`
                                              )
                                              AND (`pro`.`is_active` = 1)
                                            )
                                          )
                                        )
                                      WHERE
                                        `p`.`id` IN (438, 462)
                                      AND pro.global_unit_id IN (5, 3, 21, 22)
                                      AND r.`acronym` IN (
                                        'PMU',
                                        'ML',
                                        'FPL',
                                        'FPM',
                                        'RPM',
                                        'RPL',
                                        'CP'
                                      )
                                      AND (
                                        `cp`.`global_unit_type_id` = 1
                                        OR `cp`.`global_unit_type_id` = 3
                                        OR `cp`.`global_unit_type_id` = 4
                                        OR `cp`.`global_unit_type_id` = 5
                                      )
                                      AND u.id = v_user_id
                                      UNION
                                        SELECT
                                          `u`.`id` AS `id`,
                                          `r`.`acronym` AS `ro_acronym`,
                                          `r`.`id` AS `role_id`,
                                          REPLACE (
                                            `p`.`permission`,
                                            '{0}',
                                            `cp`.`acronym`
                                          ) AS `permission`,
                                          NULL AS `project_id`,
                                          `cp`.`acronym` AS `center_acronym`,
                                          `p`.`id` AS `permission_id`
                                        FROM
                                          (
                                            (
                                              (
                                                (
                                                  (
                                                    `users` `u`
                                                    LEFT JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                                                  )
                                                  JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                                                )
                                                JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                                              )
                                              JOIN `permissions` `p` ON (
                                                (
                                                  `p`.`id` = `rp`.`permission_id`
                                                )
                                              )
                                            )
                                            JOIN `global_units` `cp`
                                          )
                                        WHERE
                                          (
                                            (`p`.`type` = 0)
                                            AND (`r`.`id` = 136)
                                            AND (
                                              `cp`.`global_unit_type_id` = 2
                                            )
                                          )
                                        AND u.id = v_user_id
                                        UNION
                                          SELECT
                                            `u`.`id` AS `id`,
                                            `r`.`acronym` AS `ro_acronym`,
                                            `r`.`id` AS `role_id`,
                                            REPLACE (
                                              `p`.`permission`,
                                              '{0}',
                                              `cp`.`acronym`
                                            ) AS `permission`,
                                            NULL AS `project_id`,
                                            `cp`.`acronym` AS `center_acronym`,
                                            `p`.`id` AS `permission_id`
                                          FROM
                                            (
                                              (
                                                (
                                                  (
                                                    (
                                                      `users` `u`
                                                      LEFT JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                                                    )
                                                    JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                                                  )
                                                  JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                                                )
                                                JOIN `permissions` `p` ON (
                                                  (
                                                    `p`.`id` = `rp`.`permission_id`
                                                  )
                                                )
                                              )
                                              JOIN `global_units` `cp`
                                            )
                                          WHERE
                                            (
                                              (`p`.`type` = 0)
                                              AND (`r`.`acronym` = 'Admin')
                                              AND (
                                                `cp`.`global_unit_type_id` = 2
                                              )
                                            )
                                          AND u.id = v_user_id
                                          UNION
                                            SELECT
                                              `u`.`id` AS `id`,
                                              `r`.`acronym` AS `acronym`,
                                              `r`.`id` AS `rolid`,
                                              REPLACE (
                                                REPLACE (
                                                  `p`.`permission`,
                                                  '{0}',
                                                  `cp`.`acronym`
                                                ),
                                                '{1}',
                                                `ar`.`id`
                                              ) AS `permission`,
                                              NULL AS `project_id`,
                                              `cp`.`acronym` AS `crp_acronym`,
                                              `p`.`id` AS `permission_id`
                                            FROM
                                              (
                                                (
                                                  (
                                                    (
                                                      (
                                                        (
                                                          (
                                                            `users` `u`
                                                            JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                                                          )
                                                          JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                                                        )
                                                        JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                                                      )
                                                      JOIN `permissions` `p` ON (
                                                        (
                                                          `p`.`id` = `rp`.`permission_id`
                                                        )
                                                      )
                                                    )
                                                    JOIN `crp_users` `crp` ON (
                                                      (
                                                        (`u`.`id` = `crp`.`user_id`)
                                                        AND (
                                                          `crp`.`global_unit_id` = `r`.`global_unit_id`
                                                        )
                                                      )
                                                    )
                                                  )
                                                  JOIN `global_units` `cp` ON (
                                                    (
                                                      `cp`.`id` = `crp`.`global_unit_id`
                                                    )
                                                  )
                                                )
                                                JOIN `center_areas` `ar` ON (
                                                  (
                                                    (
                                                      `ar`.`global_unit_id` = `cp`.`id`
                                                    )
                                                    AND (`ar`.`is_active` = 1)
                                                  )
                                                )
                                              )
                                            WHERE
                                              (
                                                (`p`.`type` = 0)
                                                AND (`r`.`acronym` = 'Coord')
                                                AND (
                                                  `cp`.`global_unit_type_id` = 2
                                                )
                                              )
                                            AND u.id = v_user_id
                                            UNION
                                              SELECT
                                                `u`.`id` AS `id`,
                                                `r`.`acronym` AS `acronym`,
                                                `r`.`id` AS `rolid`,
                                                REPLACE (
                                                  REPLACE (
                                                    `p`.`permission`,
                                                    '{0}',
                                                    `cp`.`acronym`
                                                  ),
                                                  '{1}',
                                                  `ar`.`id`
                                                ) AS `permission`,
                                                NULL AS `project_id`,
                                                `cp`.`acronym` AS `crp_acronym`,
                                                `p`.`id` AS `permission_id`
                                              FROM
                                                (
                                                  (
                                                    (
                                                      (
                                                        (
                                                          (
                                                            (
                                                              (
                                                                `users` `u`
                                                                JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                                                              )
                                                              JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                                                            )
                                                            JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                                                          )
                                                          JOIN `permissions` `p` ON (
                                                            (
                                                              `p`.`id` = `rp`.`permission_id`
                                                            )
                                                          )
                                                        )
                                                        JOIN `crp_users` `crp` ON (
                                                          (
                                                            (`u`.`id` = `crp`.`user_id`)
                                                            AND (
                                                              `crp`.`global_unit_id` = `r`.`global_unit_id`
                                                            )
                                                          )
                                                        )
                                                      )
                                                      JOIN `global_units` `cp` ON (
                                                        (
                                                          `cp`.`id` = `crp`.`global_unit_id`
                                                        )
                                                      )
                                                    )
                                                    JOIN `center_areas` `ar` ON (
                                                      (
                                                        (
                                                          `ar`.`global_unit_id` = `cp`.`id`
                                                        )
                                                        AND (`ar`.`is_active` = 1)
                                                      )
                                                    )
                                                  )
                                                  JOIN `center_leaders` `cprog` ON (
                                                    (
                                                      (`cprog`.`user_id` = `u`.`id`)
                                                      AND (`cprog`.`is_active` = 1)
                                                      AND (
                                                        `cprog`.`research_area_id` = `ar`.`id`
                                                      )
                                                    )
                                                  )
                                                )
                                              WHERE
                                                (
                                                  (`p`.`type` = 0)
                                                  AND (`r`.`acronym` = 'RAD')
                                                  AND (
                                                    `cp`.`global_unit_type_id` = 2
                                                  )
                                                )
                                              AND u.id = v_user_id
                                              UNION
                                                SELECT
                                                  `u`.`id` AS `id`,
                                                  `r`.`acronym` AS `acronym`,
                                                  `r`.`id` AS `rolid`,
                                                  REPLACE (
                                                    REPLACE (
                                                      REPLACE (
                                                        `p`.`permission`,
                                                        '{0}',
                                                        `cp`.`acronym`
                                                      ),
                                                      '{1}',
                                                      `ar`.`id`
                                                    ),
                                                    '{2}',
                                                    `rprog`.`id`
                                                  ) AS `permission`,
                                                  NULL AS `project_id`,
                                                  `cp`.`acronym` AS `crp_acronym`,
                                                  `p`.`id` AS `permission_id`
                                                FROM
                                                  (
                                                    (
                                                      (
                                                        (
                                                          (
                                                            (
                                                              (
                                                                (
                                                                  (
                                                                    `users` `u`
                                                                    JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                                                                  )
                                                                  JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                                                                )
                                                                JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                                                              )
                                                              JOIN `permissions` `p` ON (
                                                                (
                                                                  `p`.`id` = `rp`.`permission_id`
                                                                )
                                                              )
                                                            )
                                                            JOIN `crp_users` `crp` ON (
                                                              (
                                                                (`u`.`id` = `crp`.`user_id`)
                                                                AND (
                                                                  `crp`.`global_unit_id` = `r`.`global_unit_id`
                                                                )
                                                              )
                                                            )
                                                          )
                                                          JOIN `global_units` `cp` ON (
                                                            (
                                                              `cp`.`id` = `crp`.`global_unit_id`
                                                            )
                                                          )
                                                        )
                                                        JOIN `center_areas` `ar` ON (
                                                          (
                                                            (
                                                              `ar`.`global_unit_id` = `cp`.`id`
                                                            )
                                                            AND (`ar`.`is_active` = 1)
                                                          )
                                                        )
                                                      )
                                                      JOIN `center_programs` `rprog` ON (
                                                        (
                                                          (
                                                            `rprog`.`research_area_id` = `ar`.`id`
                                                          )
                                                          AND (`ar`.`is_active` = 1)
                                                        )
                                                      )
                                                    )
                                                    JOIN `center_leaders` `cprog` ON (
                                                      (
                                                        (`cprog`.`user_id` = `u`.`id`)
                                                        AND (`cprog`.`is_active` = 1)
                                                        AND (
                                                          `cprog`.`research_program_id` = `rprog`.`id`
                                                        )
                                                      )
                                                    )
                                                  )
                                                WHERE
                                                  (
                                                    (`p`.`type` = 0)
                                                    AND (`r`.`acronym` = 'RPL')
                                                    AND (
                                                      `cp`.`global_unit_type_id` = 2
                                                    )
                                                  )
                                                AND u.id = v_user_id
                                                UNION
                                                  SELECT
                                                    `u`.`id` AS `id`,
                                                    `r`.`acronym` AS `acronym`,
                                                    `r`.`id` AS `rolid`,
                                                    REPLACE (
                                                      REPLACE (
                                                        REPLACE (
                                                          `p`.`permission`,
                                                          '{0}',
                                                          `cp`.`acronym`
                                                        ),
                                                        '{1}',
                                                        `ar`.`id`
                                                      ),
                                                      '{2}',
                                                      `rprog`.`id`
                                                    ) AS `permission`,
                                                    NULL AS `project_id`,
                                                    `cp`.`acronym` AS `crp_acronym`,
                                                    `p`.`id` AS `permission_id`
                                                  FROM
                                                    (
                                                      (
                                                        (
                                                          (
                                                            (
                                                              (
                                                                (
                                                                  (
                                                                    (
                                                                      `users` `u`
                                                                      JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                                                                    )
                                                                    JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                                                                  )
                                                                  JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                                                                )
                                                                JOIN `permissions` `p` ON (
                                                                  (
                                                                    `p`.`id` = `rp`.`permission_id`
                                                                  )
                                                                )
                                                              )
                                                              JOIN `crp_users` `crp` ON (
                                                                (
                                                                  (`u`.`id` = `crp`.`user_id`)
                                                                  AND (
                                                                    `crp`.`global_unit_id` = `r`.`global_unit_id`
                                                                  )
                                                                )
                                                              )
                                                            )
                                                            JOIN `global_units` `cp` ON (
                                                              (
                                                                `cp`.`id` = `crp`.`global_unit_id`
                                                              )
                                                            )
                                                          )
                                                          JOIN `center_areas` `ar` ON (
                                                            (
                                                              (
                                                                `ar`.`global_unit_id` = `cp`.`id`
                                                              )
                                                              AND (`ar`.`is_active` = 1)
                                                            )
                                                          )
                                                        )
                                                        JOIN `center_programs` `rprog` ON (
                                                          (
                                                            (
                                                              `rprog`.`research_area_id` = `ar`.`id`
                                                            )
                                                            AND (`ar`.`is_active` = 1)
                                                          )
                                                        )
                                                      )
                                                      JOIN `center_leaders` `cprog` ON (
                                                        (
                                                          (`cprog`.`user_id` = `u`.`id`)
                                                          AND (`cprog`.`is_active` = 1)
                                                          AND (
                                                            `cprog`.`research_program_id` = `rprog`.`id`
                                                          )
                                                        )
                                                      )
                                                    )
                                                  WHERE
                                                    (
                                                      (`p`.`type` = 0)
                                                      AND (`r`.`acronym` = 'SL')
                                                      AND (
                                                        `cp`.`global_unit_type_id` = 2
                                                      )
                                                      AND u.id = v_user_id
                                                    )
                                                  UNION
                                                    SELECT
                                                      `u`.`id` AS `id`,
                                                      `r`.`acronym` AS `ro_acronym`,
                                                      `r`.`id` AS `role_id`,
                                                      REPLACE (
                                                        `p`.`permission`,
                                                        '{0}',
                                                        `cp`.`acronym`
                                                      ) AS `permission`,
                                                      NULL AS `project_id`,
                                                      `cp`.`acronym` AS `center_acronym`,
                                                      `p`.`id` AS `permission_id`
                                                    FROM
                                                      (
                                                        (
                                                          (
                                                            (
                                                              (
                                                                `users` `u`
                                                                LEFT JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                                                              )
                                                              JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                                                            )
                                                            JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                                                          )
                                                          JOIN `permissions` `p` ON (
                                                            (
                                                              `p`.`id` = `rp`.`permission_id`
                                                            )
                                                          )
                                                        )
                                                        JOIN `global_units` `cp` ON r.global_unit_id = cp.id
                                                        JOIN phases ph ON cp.id = ph.global_unit_id
                                                      )
                                                    WHERE
                                                      (
                                                        (`p`.`type` = 0)
                                                        AND (`r`.`id` = 151)
                                                        AND (
                                                          `cp`.`global_unit_type_id` = 3
                                                        )
                                                      )
                                                    AND ph.editable = 1
                                                    AND u.id = v_user_id
                                                    UNION
                                                      SELECT
                                                        `u`.`id` AS `id`,
                                                        `r`.`acronym` AS `ro_acronym`,
                                                        `r`.`id` AS `role_id`,
                                                        REPLACE (
                                                          `p`.`permission`,
                                                          '{0}',
                                                          `cp`.`acronym`
                                                        ) AS `permission`,
                                                        NULL AS `project_id`,
                                                        `cp`.`acronym` AS `center_acronym`,
                                                        `p`.`id` AS `permission_id`
                                                      FROM
                                                        (
                                                          (
                                                            (
                                                              (
                                                                (
                                                                  `users` `u`
                                                                  LEFT JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                                                                )
                                                                JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                                                              )
                                                              JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                                                            )
                                                            JOIN `permissions` `p` ON (
                                                              (
                                                                `p`.`id` = `rp`.`permission_id`
                                                              )
                                                            )
                                                          )
                                                          JOIN `global_units` `cp` ON r.global_unit_id = cp.id
                                                          JOIN phases ph ON cp.id = ph.global_unit_id
                                                        )
                                                      WHERE
                                                        (
                                                          (`p`.`type` = 0)
                                                          AND (`r`.`id` = 138)
                                                          AND (
                                                            `cp`.`global_unit_type_id` = 3
                                                          )
                                                        )
                                                      AND ph.editable = 1
                                                      AND u.id = v_user_id
                                                      UNION
                                                        SELECT
                                                          `u`.`id` AS `id`,
                                                          `r`.`acronym` AS `acronym`,
                                                          `r`.`id` AS `rolid`,
                                                          REPLACE (
                                                            REPLACE (
                                                              `p`.`permission`,
                                                              '{0}',
                                                              CONCAT(
                                                                `cp`.`acronym`,
                                                                ':',
                                                                ph.description,
                                                                ':',
                                                                ph.`year`
                                                              )
                                                            ),
                                                            '{1}',
                                                            `powb`.`id`
                                                          ) AS `replace(replace(``p``.``permission``,'{0}',cp.acronym),'{1}',powb.id)`,
                                                          NULL AS `project_id`,
                                                          `cp`.`acronym` AS `crp_acronym`,
                                                          `p`.`id` AS `permission_id`
                                                        FROM
                                                          (
                                                            (
                                                              (
                                                                (
                                                                  (
                                                                    (
                                                                      (
                                                                        (
                                                                          `users` `u`
                                                                          JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                                                                        )
                                                                        JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                                                                      )
                                                                      JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                                                                    )
                                                                    JOIN `permissions` `p` ON (
                                                                      (
                                                                        `p`.`id` = `rp`.`permission_id`
                                                                      )
                                                                    )
                                                                  )
                                                                  JOIN `crp_users` `crp` ON (
                                                                    (
                                                                      (`u`.`id` = `crp`.`user_id`)
                                                                      AND (
                                                                        `crp`.`global_unit_id` = `r`.`global_unit_id`
                                                                      )
                                                                    )
                                                                  )
                                                                )
                                                                JOIN `crp_program_leaders` `cprog` ON (
                                                                  (
                                                                    (`cprog`.`user_id` = `u`.`id`)
                                                                    AND (`cprog`.`is_active` = 1)
                                                                  )
                                                                )
                                                              )
                                                              JOIN `crp_programs` `pro` ON (
                                                                (
                                                                  (
                                                                    `pro`.`id` = `cprog`.`crp_program_id`
                                                                  )
                                                                  AND (`pro`.`program_type` = 1)
                                                                  AND (`pro`.`is_active` = 1)
                                                                )
                                                              )
                                                            )
                                                            JOIN `global_units` `cp` ON (
                                                              (
                                                                (
                                                                  `cp`.`id` = `crp`.`global_unit_id`
                                                                )
                                                                AND (
                                                                  `pro`.`global_unit_id` = `cp`.`id`
                                                                )
                                                              )
                                                            )
                                                            JOIN phases ph ON cp.id = ph.global_unit_id
                                                            JOIN `liaison_institutions` `li` ON (
                                                              (
                                                                `li`.`global_unit_id` = cp.id
                                                              )
                                                              AND `pro`.`id` = li.crp_program
                                                              AND (
                                                                `li`.`crp_program` IS NOT NULL
                                                                OR `li`.`acronym` = 'PMU'
                                                              )
                                                            )
                                                            JOIN powb_synthesis powb ON (
                                                              powb.id_phase = ph.id
                                                              AND powb.liaison_institution_id = li.id
                                                            )
                                                          )
                                                        WHERE
                                                          (
                                                            (`p`.`type` = 1)
                                                            AND (
                                                              `r`.`acronym` IN (
                                                                'FPL',
                                                                'PMU',
                                                                'FPM',
                                                                'RPL',
                                                                'RPM'
                                                              )
                                                            )
                                                            AND (
                                                              `cp`.`global_unit_type_id` = 1
                                                              OR `cp`.`global_unit_type_id` = 3
                                                              OR `cp`.`global_unit_type_id` = 4
                                                              OR `cp`.`global_unit_type_id` = 5
                                                            )
                                                          )
                                                        AND ph.editable = 1
                                                        AND u.id = v_user_id
                                                        UNION
                                                          SELECT
                                                            `u`.`id` AS `id`,
                                                            `r`.`acronym` AS `acronym`,
                                                            `r`.`id` AS `rolid`,
                                                            REPLACE (
                                                              REPLACE (
                                                                `p`.`permission`,
                                                                '{0}',
                                                                CONCAT(
                                                                  `cp`.`acronym`,
                                                                  ':',
                                                                  ph.description,
                                                                  ':',
                                                                  ph.`year`
                                                                )
                                                              ),
                                                              '{1}',
                                                              `powb`.`id`
                                                            ) AS `replace(replace(``p``.``permission``,'{0}',cp.acronym),'{1}',powb.id)`,
                                                            NULL AS `project_id`,
                                                            `cp`.`acronym` AS `crp_acronym`,
                                                            `p`.`id` AS `permission_id`
                                                          FROM
                                                            (
                                                              (
                                                                (
                                                                  (
                                                                    (
                                                                      (
                                                                        (
                                                                          (
                                                                            `users` `u`
                                                                            JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                                                                          )
                                                                          JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                                                                        )
                                                                        JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                                                                      )
                                                                      JOIN `permissions` `p` ON (
                                                                        (
                                                                          `p`.`id` = `rp`.`permission_id`
                                                                        )
                                                                      )
                                                                    )
                                                                    JOIN `crp_users` `crp` ON (
                                                                      (
                                                                        (`u`.`id` = `crp`.`user_id`)
                                                                        AND (
                                                                          `crp`.`global_unit_id` = `r`.`global_unit_id`
                                                                        )
                                                                      )
                                                                    )
                                                                  )
                                                                )
                                                              )
                                                              JOIN `global_units` `cp` ON (
                                                                (
                                                                  (
                                                                    `cp`.`id` = `crp`.`global_unit_id`
                                                                  )
                                                                )
                                                              )
                                                              JOIN phases ph ON cp.id = ph.global_unit_id
                                                              JOIN `liaison_institutions` `li` ON (
                                                                (
                                                                  `li`.`global_unit_id` = cp.id
                                                                )
                                                                AND (
                                                                  `li`.`crp_program` IS NULL
                                                                  AND `li`.`acronym` = 'PMU'
                                                                )
                                                              )
                                                              JOIN powb_synthesis powb ON (
                                                                powb.id_phase = ph.id
                                                                AND powb.liaison_institution_id = li.id
                                                              )
                                                            )
                                                          WHERE
                                                            (
                                                              (`p`.`type` = 1)
                                                              AND (`r`.`acronym` IN('PMU'))
                                                              AND (
                                                                `cp`.`global_unit_type_id` = 1
                                                                OR `cp`.`global_unit_type_id` = 3
                                                                OR `cp`.`global_unit_type_id` = 4
                                                                OR `cp`.`global_unit_type_id` = 5
                                                              )
                                                            )
                                                          AND ph.editable = 1
                                                          AND u.id = v_user_id
                                                          UNION
                                                            SELECT
                                                              `u`.`id` AS `id`,
                                                              `r`.`acronym` AS `acronym`,
                                                              `r`.`id` AS `rolid`,
                                                              REPLACE (
                                                                `p`.`permission`,
                                                                '{0}',
                                                                CONCAT(
                                                                  `cp`.`acronym`,
                                                                  ':',
                                                                  ph.description,
                                                                  ':',
                                                                  ph.`year`
                                                                )
                                                              ) AS `replace(replace(``p``.``permission``,'{0}',cp.acronym),'{1}',pro.id)`,
                                                              NULL AS `project_id`,
                                                              `cp`.`acronym` AS `crp_acronym`,
                                                              `p`.`id` AS `permission_id`
                                                            FROM
                                                              (
                                                                (
                                                                  (
                                                                    (
                                                                      (
                                                                        (
                                                                          (
                                                                            (
                                                                              `users` `u`
                                                                              JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                                                                            )
                                                                            JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                                                                          )
                                                                          JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                                                                        )
                                                                        JOIN `permissions` `p` ON (
                                                                          (
                                                                            `p`.`id` = `rp`.`permission_id`
                                                                          )
                                                                        )
                                                                      )
                                                                      JOIN `crp_users` `crp` ON (
                                                                        (
                                                                          (`u`.`id` = `crp`.`user_id`)
                                                                          AND (
                                                                            `crp`.`global_unit_id` = `r`.`global_unit_id`
                                                                          )
                                                                        )
                                                                      )
                                                                    )
                                                                  )
                                                                )
                                                                JOIN `global_units` `cp` ON (
                                                                  (
                                                                    (
                                                                      `cp`.`id` = `crp`.`global_unit_id`
                                                                    )
                                                                  )
                                                                )
                                                                JOIN phases ph ON cp.id = ph.global_unit_id
                                                              )
                                                            WHERE
                                                              (
                                                                (`p`.`type` = 3)
                                                                AND (`r`.`acronym` IN('PMU'))
                                                                AND p.id = 501
                                                                AND (
                                                                  `cp`.`global_unit_type_id` = 1
                                                                  OR `cp`.`global_unit_type_id` = 3
                                                                  OR `cp`.`global_unit_type_id` = 4
                                                                  OR `cp`.`global_unit_type_id` = 5
                                                                )
                                                              )
                                                            AND ph.editable = 1
                                                            AND u.id = v_user_id
                                                            UNION
                                                              SELECT
                                                                `u`.`id` AS `id`,
                                                                `r`.`acronym` AS `acronym`,
                                                                `r`.`id` AS `rolid`,
                                                                REPLACE (
                                                                  REPLACE (
                                                                    `p`.`permission`,
                                                                    '{0}',
                                                                    CONCAT(
                                                                      `cp`.`acronym`,
                                                                      ':',
                                                                      ph.description,
                                                                      ':',
                                                                      ph.`year`
                                                                    )
                                                                  ),
                                                                  '{1}',
                                                                  `powb`.`id`
                                                                ) AS `replace(replace(``p``.``permission``,'{0}',cp.acronym),'{1}',powb.id)`,
                                                                NULL AS `project_id`,
                                                                `cp`.`acronym` AS `crp_acronym`,
                                                                `p`.`id` AS `permission_id`
                                                              FROM
                                                                (
                                                                  (
                                                                    (
                                                                      (
                                                                        (
                                                                          (
                                                                            (
                                                                              (
                                                                                `users` `u`
                                                                                JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                                                                              )
                                                                              JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                                                                            )
                                                                            JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                                                                          )
                                                                          JOIN `permissions` `p` ON (
                                                                            (
                                                                              `p`.`id` = `rp`.`permission_id`
                                                                            )
                                                                          )
                                                                        )
                                                                        JOIN `crp_users` `crp` ON (
                                                                          (
                                                                            (`u`.`id` = `crp`.`user_id`)
                                                                            AND (
                                                                              `crp`.`global_unit_id` = `r`.`global_unit_id`
                                                                            )
                                                                          )
                                                                        )
                                                                      )
                                                                    )
                                                                  )
                                                                  JOIN `global_units` `cp` ON (
                                                                    (
                                                                      (
                                                                        `cp`.`id` = `crp`.`global_unit_id`
                                                                      )
                                                                    )
                                                                  )
                                                                  JOIN phases ph ON cp.id = ph.global_unit_id
                                                                  JOIN `liaison_institutions` `li` ON (
                                                                    (
                                                                      `li`.`global_unit_id` = cp.id
                                                                    )
                                                                    AND (
                                                                      `li`.`crp_program` IS NULL
                                                                      AND `li`.`acronym` = 'PMU'
                                                                    )
                                                                  )
                                                                  JOIN powb_synthesis powb ON (
                                                                    powb.id_phase = ph.id
                                                                    AND powb.liaison_institution_id = li.id
                                                                  )
                                                                )
                                                              WHERE
                                                                (
                                                                  (`p`.`type` = 1)
                                                                  AND (
                                                                    `r`.`acronym` IN ('RPL', 'RPM')
                                                                  )
                                                                  AND (
                                                                    `cp`.`global_unit_type_id` = 1
                                                                    OR `cp`.`global_unit_type_id` = 3
                                                                    OR `cp`.`global_unit_type_id` = 4
                                                                    OR `cp`.`global_unit_type_id` = 5
                                                                  )
                                                                )
                                                              AND p.permission IN (
                                                                'crp:{0}:powbSynthesis:{1}:collaboration:canEdit',
                                                                'crp:{0}:powbSynthesis:manage:canAcess'
                                                              )
                                                              AND ph.editable = 1
                                                              AND u.id = v_user_id
                                                              UNION
                                                                SELECT
                                                                  `u`.`id` AS `id`,
                                                                  `r`.`acronym` AS `acronym`,
                                                                  `r`.`id` AS `rolid`,
                                                                  REPLACE (
                                                                    REPLACE (
                                                                      REPLACE (
                                                                        `p`.`permission`,
                                                                        '{0}',
                                                                        CONCAT(
                                                                          `cp`.`acronym`,
                                                                          ':',
                                                                          ph.description,
                                                                          ':',
                                                                          ph.`year`
                                                                        )
                                                                      ),
                                                                      '{1}',
                                                                      `powb`.`id`
                                                                    ),
                                                                    '{2}',
                                                                    liRegion.id
                                                                  ) AS 'permission',
                                                                  NULL AS `project_id`,
                                                                  `cp`.`acronym` AS `crp_acronym`,
                                                                  `p`.`id` AS `permission_id`
                                                                FROM
                                                                  (
                                                                    (
                                                                      (
                                                                        (
                                                                          (
                                                                            (
                                                                              (
                                                                                (
                                                                                  `users` `u`
                                                                                  JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                                                                                )
                                                                                JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                                                                              )
                                                                              JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                                                                            )
                                                                            JOIN `permissions` `p` ON (
                                                                              (
                                                                                `p`.`id` = `rp`.`permission_id`
                                                                              )
                                                                            )
                                                                          )
                                                                          JOIN `crp_users` `crp` ON (
                                                                            (
                                                                              (`u`.`id` = `crp`.`user_id`)
                                                                              AND (
                                                                                `crp`.`global_unit_id` = `r`.`global_unit_id`
                                                                              )
                                                                            )
                                                                          )
                                                                        )
                                                                      )
                                                                    )
                                                                    JOIN `global_units` `cp` ON (
                                                                      (
                                                                        (
                                                                          `cp`.`id` = `crp`.`global_unit_id`
                                                                        )
                                                                      )
                                                                    )
                                                                    JOIN phases ph ON cp.id = ph.global_unit_id
                                                                    JOIN `liaison_institutions` `li` ON (
                                                                      (
                                                                        `li`.`global_unit_id` = cp.id
                                                                      )
                                                                      AND (
                                                                        `li`.`crp_program` IS NULL
                                                                        AND `li`.`acronym` = 'PMU'
                                                                      )
                                                                    )
                                                                    JOIN `liaison_users` `liasUsers` ON (
                                                                      (`liasUsers`.`user_id` = u.id)
                                                                    )
                                                                    JOIN `liaison_institutions` `liRegion` ON (
                                                                      (
                                                                        `liRegion`.`global_unit_id` = cp.id
                                                                        AND liasUsers.institution_id = liRegion.id
                                                                      )
                                                                    )
                                                                    JOIN powb_synthesis powb ON (
                                                                      powb.id_phase = ph.id
                                                                      AND powb.liaison_institution_id = li.id
                                                                    )
                                                                  )
                                                                WHERE
                                                                  (
                                                                    (`p`.`type` = 1)
                                                                    AND (
                                                                      `r`.`acronym` IN ('RPL', 'RPM')
                                                                    )
                                                                    AND (
                                                                      `cp`.`global_unit_type_id` = 1
                                                                      OR `cp`.`global_unit_type_id` = 3
                                                                      OR `cp`.`global_unit_type_id` = 4
                                                                      OR `cp`.`global_unit_type_id` = 5
                                                                    )
                                                                  )
                                                                AND p.permission = 'crp:{0}:powbSynthesis:{1}:collaboration{2}:effort'
                                                                AND ph.editable = 1
                                                                AND u.id = v_user_id
                                                                UNION
                                                                  SELECT
                                                                    `u`.`id` AS `id`,
                                                                    `r`.`acronym` AS `acronym`,
                                                                    `r`.`id` AS `rolid`,
                                                                    REPLACE (
                                                                      REPLACE (
                                                                        REPLACE (
                                                                          `p`.`permission`,
                                                                          '{0}',
                                                                          CONCAT(
                                                                            `cp`.`acronym`,
                                                                            ':',
                                                                            ph.description,
                                                                            ':',
                                                                            ph.`year`
                                                                          )
                                                                        ),
                                                                        '{1}',
                                                                        `powb`.`id`
                                                                      ),
                                                                      '{2}',
                                                                      li.id
                                                                    ) AS 'permission',
                                                                    NULL AS `project_id`,
                                                                    `cp`.`acronym` AS `crp_acronym`,
                                                                    `p`.`id` AS `permission_id`
                                                                  FROM
                                                                    (
                                                                      (
                                                                        (
                                                                          (
                                                                            (
                                                                              (
                                                                                (
                                                                                  (
                                                                                    `users` `u`
                                                                                    JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                                                                                  )
                                                                                  JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                                                                                )
                                                                                JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                                                                              )
                                                                              JOIN `permissions` `p` ON (
                                                                                (
                                                                                  `p`.`id` = `rp`.`permission_id`
                                                                                )
                                                                              )
                                                                            )
                                                                            JOIN `crp_users` `crp` ON (
                                                                              (
                                                                                (`u`.`id` = `crp`.`user_id`)
                                                                                AND (
                                                                                  `crp`.`global_unit_id` = `r`.`global_unit_id`
                                                                                )
                                                                              )
                                                                            )
                                                                          )
                                                                        )
                                                                      )
                                                                      JOIN `global_units` `cp` ON (
                                                                        (
                                                                          (
                                                                            `cp`.`id` = `crp`.`global_unit_id`
                                                                          )
                                                                        )
                                                                      )
                                                                      JOIN phases ph ON cp.id = ph.global_unit_id
                                                                      JOIN `liaison_institutions` `li` ON (
                                                                        (
                                                                          `li`.`global_unit_id` = cp.id
                                                                        )
                                                                        AND (
                                                                          `li`.`crp_program` IS NOT NULL
                                                                        )
                                                                      )
                                                                      JOIN powb_synthesis powb ON (
                                                                        powb.id_phase = ph.id
                                                                        AND powb.liaison_institution_id = li.id
                                                                      )
                                                                    )
                                                                  WHERE
                                                                    (
                                                                      (`p`.`type` = 1)
                                                                      AND (`r`.`acronym` IN('PMU'))
                                                                      AND (
                                                                        `cp`.`global_unit_type_id` = 1
                                                                        OR `cp`.`global_unit_type_id` = 3
                                                                        OR `cp`.`global_unit_type_id` = 4
                                                                        OR `cp`.`global_unit_type_id` = 5
                                                                      )
                                                                    )
                                                                  AND ph.editable = 1
                                                                  AND u.id = v_user_id
                                                                  UNION
                                                                    SELECT
                                                                      `u`.`id` AS `id`,
                                                                      `r`.`acronym` AS `ro_acronym`,
                                                                      `r`.`id` AS `role_id`,
                                                                      REPLACE (
                                                                        `p`.`permission`,
                                                                        '{0}',
                                                                        `cp`.`acronym`
                                                                      ) AS `permission`,
                                                                      NULL AS `project_id`,
                                                                      `cp`.`acronym` AS `center_acronym`,
                                                                      `p`.`id` AS `permission_id`
                                                                    FROM
                                                                      (
                                                                        (
                                                                          (
                                                                            (
                                                                              (
                                                                                `users` `u`
                                                                                LEFT JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                                                                              )
                                                                              JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                                                                            )
                                                                            JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                                                                          )
                                                                          JOIN `permissions` `p` ON (
                                                                            (
                                                                              `p`.`id` = `rp`.`permission_id`
                                                                            )
                                                                          )
                                                                        )
                                                                        JOIN `global_units` `cp` ON r.global_unit_id = cp.id
                                                                        JOIN phases ph ON cp.id = ph.global_unit_id
                                                                      )
                                                                    WHERE
                                                                      (
                                                                        (`p`.`type` = 0)
                                                                        AND (`r`.`id` = 173)
                                                                        AND (
                                                                          `cp`.`global_unit_type_id` = 4
                                                                        )
                                                                      )
                                                                    AND ph.editable = 1
                                                                    AND u.id = v_user_id
                                                                    UNION
                                                                      SELECT
                                                                        `u`.`id` AS `id`,
                                                                        `r`.`acronym` AS `ro_acronym`,
                                                                        `r`.`id` AS `role_id`,
                                                                        REPLACE (
                                                                          `p`.`permission`,
                                                                          '{0}',
                                                                          `cp`.`acronym`
                                                                        ) AS `permission`,
                                                                        NULL AS `project_id`,
                                                                        `cp`.`acronym` AS `center_acronym`,
                                                                        `p`.`id` AS `permission_id`
                                                                      FROM
                                                                        (
                                                                          (
                                                                            (
                                                                              (
                                                                                (
                                                                                  `users` `u`
                                                                                  LEFT JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                                                                                )
                                                                                JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                                                                              )
                                                                              JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                                                                            )
                                                                            JOIN `permissions` `p` ON (
                                                                              (
                                                                                `p`.`id` = `rp`.`permission_id`
                                                                              )
                                                                            )
                                                                          )
                                                                          JOIN `global_units` `cp` ON r.global_unit_id = cp.id
                                                                          JOIN phases ph ON cp.id = ph.global_unit_id
                                                                        )
                                                                      WHERE
                                                                        (
                                                                          (`p`.`type` = 0)
                                                                          AND (`r`.`id` = 160)
                                                                          AND (
                                                                            `cp`.`global_unit_type_id` = 4
                                                                          )
                                                                        )
                                                                      AND ph.editable = 1
                                                                      AND u.id = v_user_id
                                                                      UNION
                                                                        SELECT
                                                                          `u`.`id` AS `id`,
                                                                          `r`.`acronym` AS `acronym`,
                                                                          `r`.`id` AS `rolid`,
                                                                          REPLACE (
                                                                            REPLACE (
                                                                              `p`.`permission`,
                                                                              '{0}',
                                                                              CONCAT(
                                                                                `cp`.`acronym`,
                                                                                ':',
                                                                                ph.description,
                                                                                ':',
                                                                                ph.`year`
                                                                              )
                                                                            ),
                                                                            '{1}',
                                                                            `ps`.id
                                                                          ) AS `replace(replace(``p``.``permission``,'{0}',cp.acronym),'{1}',ps.id)`,
                                                                          `ps`.id AS `project_id`,
                                                                          `cp`.`acronym` AS `crp_acronym`,
                                                                          `p`.`id` AS `permission_id`
                                                                        FROM
                                                                          (
                                                                            (
                                                                              (
                                                                                (
                                                                                  (
                                                                                    (
                                                                                      (
                                                                                        (
                                                                                          `users` `u`
                                                                                          JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                                                                                        )
                                                                                        JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                                                                                      )
                                                                                      JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                                                                                    )
                                                                                    JOIN `permissions` `p` ON (
                                                                                      (
                                                                                        `p`.`id` = `rp`.`permission_id`
                                                                                      )
                                                                                    )
                                                                                  )
                                                                                  JOIN `crp_users` `crp` ON (
                                                                                    (
                                                                                      (`u`.`id` = `crp`.`user_id`)
                                                                                      AND (
                                                                                        `crp`.`global_unit_id` = `r`.`global_unit_id`
                                                                                      )
                                                                                    )
                                                                                  )
                                                                                )
                                                                                JOIN `crp_program_leaders` `cprog` ON (
                                                                                  (
                                                                                    (`cprog`.`user_id` = `u`.`id`)
                                                                                    AND (`cprog`.`is_active` = 1)
                                                                                  )
                                                                                )
                                                                              )
                                                                              JOIN `crp_programs` `pro` ON (
                                                                                (
                                                                                  (
                                                                                    `pro`.`id` = `cprog`.`crp_program_id`
                                                                                  )
                                                                                  AND (`pro`.`program_type` = 1)
                                                                                  AND (`pro`.`is_active` = 1)
                                                                                )
                                                                              )
                                                                            )
                                                                            JOIN `global_units` `cp` ON (
                                                                              (
                                                                                (
                                                                                  `cp`.`id` = `crp`.`global_unit_id`
                                                                                )
                                                                                AND (
                                                                                  `pro`.`global_unit_id` = `cp`.`id`
                                                                                )
                                                                              )
                                                                            )
                                                                            JOIN phases ph ON cp.id = ph.global_unit_id
                                                                            JOIN project_expected_study_flagships pex ON (
                                                                              pex.id_phase = ph.id
                                                                              AND pro.id = pex.crp_program_id
                                                                            )
                                                                            JOIN project_expected_studies ps ON (
                                                                              ps.id = pex.expected_id
                                                                              AND ps.project_id IS NULL
                                                                            )
                                                                          )
                                                                        WHERE
                                                                          (
                                                                            (
                                                                              `r`.`acronym` IN (
                                                                                'FPL',
                                                                                'PMU',
                                                                                'FPM',
                                                                                'RPL',
                                                                                'RPM'
                                                                              )
                                                                            )
                                                                            AND (
                                                                              `cp`.`global_unit_type_id` = 1
                                                                              OR `cp`.`global_unit_type_id` = 3
                                                                              OR `cp`.`global_unit_type_id` = 4
                                                                              OR `cp`.`global_unit_type_id` = 5
                                                                            )
                                                                            AND p.permission IN (
                                                                              'crp:{0}:studies:{1}:canEdit'
                                                                            )
                                                                          )
                                                                        AND ph.editable = 1
                                                                        AND u.id = v_user_id
                                                                        UNION
                                                                          SELECT
                                                                            `u`.`id` AS `id`,
                                                                            `r`.`acronym` AS `acronym`,
                                                                            `r`.`id` AS `rolid`,
                                                                            REPLACE (
                                                                              REPLACE (
                                                                                `p`.`permission`,
                                                                                '{0}',
                                                                                CONCAT(
                                                                                  `cp`.`acronym`,
                                                                                  ':',
                                                                                  ph.description,
                                                                                  ':',
                                                                                  ph.`year`
                                                                                )
                                                                              ),
                                                                              '{1}',
                                                                              `del`.id
                                                                            ) AS `replace(replace(``p``.``permission``,'{0}',cp.acronym),'{1}',del.id)`,
                                                                            `del`.id AS `project_id`,
                                                                            `cp`.`acronym` AS `crp_acronym`,
                                                                            `p`.`id` AS `permission_id`
                                                                          FROM
                                                                            (
                                                                              (
                                                                                (
                                                                                  (
                                                                                    (
                                                                                      (
                                                                                        (
                                                                                          (
                                                                                            `users` `u`
                                                                                            JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                                                                                          )
                                                                                          JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                                                                                        )
                                                                                        JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                                                                                      )
                                                                                      JOIN `permissions` `p` ON (
                                                                                        (
                                                                                          `p`.`id` = `rp`.`permission_id`
                                                                                        )
                                                                                      )
                                                                                    )
                                                                                    JOIN `crp_users` `crp` ON (
                                                                                      (
                                                                                        (`u`.`id` = `crp`.`user_id`)
                                                                                        AND (
                                                                                          `crp`.`global_unit_id` = `r`.`global_unit_id`
                                                                                        )
                                                                                      )
                                                                                    )
                                                                                  )
                                                                                  JOIN `crp_program_leaders` `cprog` ON (
                                                                                    (
                                                                                      (`cprog`.`user_id` = `u`.`id`)
                                                                                      AND (`cprog`.`is_active` = 1)
                                                                                    )
                                                                                  )
                                                                                )
                                                                                JOIN `crp_programs` `pro` ON (
                                                                                  (
                                                                                    (
                                                                                      `pro`.`id` = `cprog`.`crp_program_id`
                                                                                    )
                                                                                    AND (`pro`.`program_type` = 1)
                                                                                    AND (`pro`.`is_active` = 1)
                                                                                  )
                                                                                )
                                                                              )
                                                                              JOIN `global_units` `cp` ON (
                                                                                (
                                                                                  (
                                                                                    `cp`.`id` = `crp`.`global_unit_id`
                                                                                  )
                                                                                  AND (
                                                                                    `pro`.`global_unit_id` = `cp`.`id`
                                                                                  )
                                                                                )
                                                                              )
                                                                              JOIN phases ph ON cp.id = ph.global_unit_id
                                                                              JOIN deliverable_programs devpro ON (
                                                                                devpro.id_phase = ph.id
                                                                                AND pro.id = devpro.crp_program
                                                                              )
                                                                              JOIN deliverables del ON (
                                                                                del.id = devpro.deliverable_id
                                                                                AND del.project_id IS NULL
                                                                              )
                                                                            )
                                                                          WHERE
                                                                            (
                                                                              (
                                                                                `r`.`acronym` IN (
                                                                                  'FPL',
                                                                                  'PMU',
                                                                                  'FPM',
                                                                                  'RPL',
                                                                                  'RPM'
                                                                                )
                                                                              )
                                                                              AND (
                                                                                `cp`.`global_unit_type_id` = 1
                                                                                OR `cp`.`global_unit_type_id` = 3
                                                                                OR `cp`.`global_unit_type_id` = 4
                                                                                OR `cp`.`global_unit_type_id` = 5
                                                                              )
                                                                              AND p.permission IN (
                                                                                'crp:{0}:publication:{1}:*'
                                                                              )
                                                                            )
                                                                          AND ph.editable = 1
                                                                          AND u.id = v_user_id
                                                                          UNION
                                                                            SELECT
                                                                              `u`.`id` AS `id`,
                                                                              `r`.`acronym` AS `acronym`,
                                                                              `r`.`id` AS `rolid`,
                                                                              REPLACE (
                                                                                REPLACE (
                                                                                  `p`.`permission`,
                                                                                  '{0}',
                                                                                  CONCAT(
                                                                                    `cp`.`acronym`,
                                                                                    ':',
                                                                                    ph.description,
                                                                                    ':',
                                                                                    ph.`year`
                                                                                  )
                                                                                ),
                                                                                '{1}',
                                                                                `repo`.`id`
                                                                              ) AS `replace(replace(``p``.``permission``,'{0}',cp.acronym),'{1}',repo.id)`,
                                                                              NULL AS `project_id`,
                                                                              `cp`.`acronym` AS `crp_acronym`,
                                                                              `p`.`id` AS `permission_id`
                                                                            FROM
                                                                              (
                                                                                (
                                                                                  (
                                                                                    (
                                                                                      (
                                                                                        (
                                                                                          (
                                                                                            (
                                                                                              `users` `u`
                                                                                              JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                                                                                            )
                                                                                            JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                                                                                          )
                                                                                          JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                                                                                        )
                                                                                        JOIN `permissions` `p` ON (
                                                                                          (
                                                                                            `p`.`id` = `rp`.`permission_id`
                                                                                          )
                                                                                        )
                                                                                      )
                                                                                      JOIN `crp_users` `crp` ON (
                                                                                        ((`u`.`id` = `crp`.`user_id`))
                                                                                      )
                                                                                    )
                                                                                    JOIN `crp_program_leaders` `cprog` ON (
                                                                                      (
                                                                                        (`cprog`.`user_id` = `u`.`id`)
                                                                                        AND (`cprog`.`is_active` = 1)
                                                                                      )
                                                                                    )
                                                                                  )
                                                                                  JOIN `crp_programs` `pro` ON (
                                                                                    (
                                                                                      (
                                                                                        `pro`.`id` = `cprog`.`crp_program_id`
                                                                                      )
                                                                                      AND (`pro`.`program_type` = 1)
                                                                                      AND (`pro`.`is_active` = 1)
                                                                                    )
                                                                                  )
                                                                                )
                                                                                JOIN `global_units` `cp` ON (
                                                                                  (
                                                                                    (
                                                                                      `cp`.`id` = `crp`.`global_unit_id`
                                                                                    )
                                                                                    AND (
                                                                                      `pro`.`global_unit_id` = `cp`.`id`
                                                                                    )
                                                                                  )
                                                                                )
                                                                                JOIN phases ph ON cp.id = ph.global_unit_id
                                                                                JOIN `liaison_institutions` `li` ON (
                                                                                  (
                                                                                    `li`.`global_unit_id` = cp.id
                                                                                  )
                                                                                  AND `pro`.`id` = li.crp_program
                                                                                  AND (
                                                                                    `li`.`crp_program` IS NOT NULL
                                                                                    OR `li`.`acronym` = 'PMU'
                                                                                  )
                                                                                )
                                                                                JOIN report_synthesis repo ON (
                                                                                  repo.id_phase = ph.id
                                                                                  AND repo.liaison_institution_id = li.id
                                                                                )
                                                                              )
                                                                            WHERE
                                                                              (
                                                                                (`p`.`type` = 3)
                                                                                AND (
                                                                                  `r`.`acronym` IN ('FPL', 'FPM', 'PMU')
                                                                                )
                                                                                AND (
                                                                                  `cp`.`global_unit_type_id` = 1
                                                                                  OR `cp`.`global_unit_type_id` = 3
                                                                                  OR `cp`.`global_unit_type_id` = 4
                                                                                  OR `cp`.`global_unit_type_id` = 5
                                                                                )
                                                                              )
                                                                            AND p.permission LIKE '%crp:{0}:reportSynthesis:{1}:%'
                                                                            AND ph.editable = 1
                                                                            AND u.id = v_user_id
                                                                            UNION
                                                                              SELECT
                                                                                `u`.`id` AS `id`,
                                                                                `r`.`acronym` AS `acronym`,
                                                                                `r`.`id` AS `rolid`,
                                                                                REPLACE (
                                                                                  REPLACE (
                                                                                    `p`.`permission`,
                                                                                    '{0}',
                                                                                    CONCAT(
                                                                                      `cp`.`acronym`,
                                                                                      ':',
                                                                                      ph.description,
                                                                                      ':',
                                                                                      ph.`year`
                                                                                    )
                                                                                  ),
                                                                                  '{1}',
                                                                                  `repo`.`id`
                                                                                ) AS `replace(replace(``p``.``permission``,'{0}',cp.acronym),'{1}',repo.id)`,
                                                                                NULL AS `project_id`,
                                                                                `cp`.`acronym` AS `crp_acronym`,
                                                                                `p`.`id` AS `permission_id`
                                                                              FROM
                                                                                (
                                                                                  (
                                                                                    (
                                                                                      (
                                                                                        (
                                                                                          (
                                                                                            (
                                                                                              (
                                                                                                `users` `u`
                                                                                                JOIN `user_roles` `ro` ON ((`ro`.`user_id` = `u`.`id`))
                                                                                              )
                                                                                              JOIN `roles` `r` ON ((`r`.`id` = `ro`.`role_id`))
                                                                                            )
                                                                                            JOIN `role_permissions` `rp` ON ((`rp`.`role_id` = `r`.`id`))
                                                                                          )
                                                                                          JOIN `permissions` `p` ON (
                                                                                            (
                                                                                              `p`.`id` = `rp`.`permission_id`
                                                                                            )
                                                                                          )
                                                                                        )
                                                                                        JOIN `crp_users` `crp` ON (
                                                                                          (
                                                                                            (`u`.`id` = `crp`.`user_id`)
                                                                                            AND (
                                                                                              `crp`.`global_unit_id` = `r`.`global_unit_id`
                                                                                            )
                                                                                          )
                                                                                        )
                                                                                      )
                                                                                    )
                                                                                  )
                                                                                  JOIN `global_units` `cp` ON (
                                                                                    (
                                                                                      (
                                                                                        `cp`.`id` = `crp`.`global_unit_id`
                                                                                      )
                                                                                    )
                                                                                  )
                                                                                  JOIN phases ph ON cp.id = ph.global_unit_id
                                                                                  JOIN `liaison_institutions` `li` ON (
                                                                                    (
                                                                                      `li`.`global_unit_id` = cp.id
                                                                                    )
                                                                                    AND(
                                                                                    (
                                                                                      `li`.`crp_program` IS NULL
                                                                                      AND `li`.`acronym` = 'PMU'
                                                                                    ) OR (`li`.`crp_program` IS NOT NULL)
                                                                                    )                                                                                     
                                                                                  )
                                                                                  JOIN report_synthesis repo ON (
                                                                                    repo.id_phase = ph.id
                                                                                    AND repo.liaison_institution_id = li.id
                                                                                  )
                                                                                )
                                                                              WHERE
                                                                                (
                                                                                  (`p`.`type` = 3)
                                                                                  AND (`r`.`acronym` IN('PMU'))
                                                                                  AND (
                                                                                    `cp`.`global_unit_type_id` = 1
                                                                                    OR `cp`.`global_unit_type_id` = 3
                                                                                    OR `cp`.`global_unit_type_id` = 4
                                                                                    OR `cp`.`global_unit_type_id` = 5
                                                                                  )
                                                                                )
                                                                              AND p.permission LIKE '%crp:{0}:reportSynthesis:{1}:%'
                                                                              AND ph.editable = 1
                                                                              AND u.id = v_user_id ; CREATE INDEX user_permission_ind ON user_permission (permission(10)) ;
                                                                              END;;
DELIMITER ;
