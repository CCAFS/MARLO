...
(SELECT u.id FROM `users` u WHERE u.email = 'mel-system@no-reply') AND cu2.global_unit_id = 24) AS t);
...
WHERE r.acronym = 'ES' AND global_unit_id = 24)) as t);
...
(SELECT r.id FROM `roles` r WHERE r.acronym = 'ES' AND global_unit_id = 24)
AND rp2.permission_id = (SELECT p.id FROM `permissions` p WHERE p.permission = 'api')) as t);

DELETE r FROM `roles` AS r WHERE r.id = (SELECT id FROM (SELECT r2.id FROM `roles` AS r2 WHERE r2.acronym = 'ES' AND r2.global_unit_id = 24) as t);
...
INSERT INTO `roles` (`description`, `acronym`, `global_unit_id`, `order`) VALUES ('External System', 'ES', 1, 16);
INSERT INTO `roles` (`description`, `acronym`, `global_unit_id`, `order`) VALUES ('External System', 'ES', 11, 16);

#Start off by providing read only access to all services and the ability to create new institutions for role ES
#CCAFS
INSERT INTO `role_permissions` (`role_id`, `permission_id`) VALUES ((SELECT r.id FROM `roles` r WHERE r.acronym = 'ES' AND global_unit_id = 1), 
...
#FTA
INSERT INTO `role_permissions` (`role_id`, `permission_id`) VALUES ((SELECT r.id FROM `roles` r WHERE r.acronym = 'ES' AND global_unit_id = 11), 
(SELECT p.id FROM `permissions` p WHERE p.permission = 'api:*:read'));
INSERT INTO `role_permissions` (`role_id`, `permission_id`) VALUES ((SELECT r.id FROM `roles` r WHERE r.acronym = 'ES' AND global_unit_id = 11), 
(SELECT p.id FROM `permissions` p WHERE p.permission = 'api:institutions:create'));
...
(SELECT r.id FROM roles r WHERE r.acronym = 'ES' AND global_unit_id = 1));

#Add myCifor user to user_roles
INSERT INTO `user_roles` (`user_id`, `role_id`) 
VALUES ((SELECT u.id FROM `users` u WHERE u.email = 'my-cifor@no-reply'), 
(SELECT r.id FROM roles r WHERE r.acronym = 'ES' AND global_unit_id = 11));

#Add MEL user to CCAFS crp_users table
INSERT INTO `crp_users` (`user_id`,`global_unit_id`,`is_active`,`created_by`,`active_since`,`modified_by`,`modification_justification`)
