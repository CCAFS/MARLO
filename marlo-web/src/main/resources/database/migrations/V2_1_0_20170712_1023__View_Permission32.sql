...
SELECT
  ...
  REPLACE (
    `p`.`permission`,
    '{0}',
    CONCAT(`cp`.`acronym`,':',ph.description,':', ph.`year`)
  ) AS `permission`,
  NULL AS `project_id`,
...
join phases ph on cp.id=ph.crp_id
join project_phases pph on pph.project_id=pro.id and pph.id_phase=ph.id
INNER JOIN projects_info pinf on pinf.id_phase=pph.id_phase and pinf.project_id=pph.project_id
...
INNER JOIN projects_info pinf on pinf.id_phase=pph.id_phase and pinf.project_id=pph.project_id
                        ...
                        JOIN `project_partners` `pp` ON (
                          (
                            (
                              `pp`.`project_id` = `pro`.`id` and  pp.id_phase=ph.id
                            )
                            AND (`pp`.`is_active` = 1)
                          )
                        )
...
and lus.crp_id=cp.id
                            ...
                            INNER JOIN project_partners pbi ON pbi.institution_id = lin.institution_id and pbi.id_phase=ph.id
                            ...
                            UNION
                              ...
                              INNER JOIN liaison_users lu ON lu.user_id = u.id and lu.crp_id=cp.id
