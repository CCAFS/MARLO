...
/*
 * bi_reports already had global_unit_id, but it was never used to filter the reports, so some rows
 * were left without a global unit. They also belong to AICCRA. Rows that already have a global unit
 * assigned are not modified.
 */
UPDATE `bi_reports`
SET `global_unit_id` = 45
WHERE `global_unit_id` IS NULL
