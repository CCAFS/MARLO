                    ...
                    UNION
                      ...
                      UNION
                        ...
                        UNION
                          ...
                          inner join custom_parameters par  on par.parameter_id=(select id from parameters where `key`='crp_contact_point_edit_project') and par.global_unit_id=cp.id and par.`value`='true'
                            AND pro.id = pbi.project_id
                            AND pbi.is_active = 1
                            WHERE
                              (
                            ...
                            AND p.id IN (
select rolePermi.permission_id
 from roles rolep inner JOIN role_permissions rolePermi on rolePermi.role_id=rolep.id
where rolep.acronym='PL' and rolep.global_unit_id=cp.id
