        ...
        <div class="grid-item">
          ...
          <div style="display: flex; flex-wrap: wrap; gap: 8px; align-items: center; margin-top: 4px;">
            ...
            <span style="display: inline-flex; align-items: center; gap: 4px;">
              {{#if isoAlpha2}}<img src="https://marlo-pdf-resources-dev.s3.us-east-1.amazonaws.com/flags/{{isoAlpha2}}.svg" alt="{{#if name}}{{name}}{{else}}Country{{/if}}" style="width: 20px; height: 15px; object-fit: contain; display: inline-block; vertical-align: middle;" />{{/if}}
              {{#if name}}<span style="font-size: 12px; color: #1F1F1F; text-transform: none;">{{#safeEmpty}}{{name}}{{/safeEmpty}}</span>{{/if}}
            </span>
            {{/each}}
          </div>
        </div>
