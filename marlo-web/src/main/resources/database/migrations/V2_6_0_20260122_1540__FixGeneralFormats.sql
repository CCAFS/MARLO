      ...
      <div class="grid grid--2 contribution-card__grid">
        ...
        <div class="grid-item">
          ...
          <div style="display: flex; flex-wrap: wrap; gap: 8px; align-items: center; margin-top: 4px;">
            {{#each countries}}
            <span style="display: inline-flex !important; align-items: center !important; gap: 4px; flex-wrap: nowrap; vertical-align: middle;">
              {{#if isoAlpha2}}<img src="https://marlo-pdf-resources-dev.s3.us-east-1.amazonaws.com/flags/{{isoAlpha2}}.svg" alt="{{#if name}}{{name}}{{else}}Country{{/if}}" style="width: 20px; height: 15px; object-fit: contain; flex-shrink: 0; vertical-align: middle; display: block;" />{{/if}}
              {{#if name}}<span style="font-size: 12px; color: #1F1F1F; text-transform: none; white-space: nowrap; display: inline-block; vertical-align: middle;">{{#safeEmpty}}{{name}}{{/safeEmpty}}</span>{{/if}}
            </span>
            {{/each}}
          </div>
