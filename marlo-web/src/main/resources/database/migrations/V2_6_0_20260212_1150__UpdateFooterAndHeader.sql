...
<style>
  ...
  .grid-item > span:first-child { display: block; font-size: 10px; letter-spacing: 0.1em; color: #6a7a89; margin-bottom: 8px; text-transform: uppercase; }
  ...
  #pageHeader { top: 0; border-bottom: 1px solid rgba(10, 76, 112, 0.12); display: flex; align-items: center; }
  #pageFooter { bottom: 0; border-top: 1px solid rgba(10, 76, 112, 0.12); font-size: 9px; color: #6a7a89; display: flex; align-items: center; justify-content: space-between; }
  .logo { height: 42px; position: absolute; right: 50px; top: 19px; }
  .disclaimer { font-size: 10px; color: #6a7a89; margin-top: 30px; line-height: 1.4; }
</style>
</head>
<body>
<div id="pageHeader">
  <span style="font-size: 12px; letter-spacing: 0.3em; color: #6a7a89; padding-right: 160px; line-height: 80px; display: inline-block; vertical-align: middle;">{{projectAcronym}} CLUSTER SUMMARY</span>
      ...
      <div class="partner-card__section">
        ...
        <ul>
          ...
          <li style="display: flex; align-items: center; flex-wrap: wrap; gap: 8px;">
            {{#if isoAlpha2}}<img src="https://marlo-pdf-resources-dev.s3.us-east-1.amazonaws.com/flags/{{isoAlpha2}}.svg" alt="{{#if country}}{{country}}{{else}}Country{{/if}}" style="width: 20px; height: 15px; object-fit: contain; flex-shrink: 0;" />{{/if}}
      ...
      <div class="grid grid--2 contribution-card__grid">
        ...
        <div class="grid-item">
          ...
          <div style="display: flex; flex-wrap: wrap; gap: 8px; align-items: center; margin-top: 4px;">
            {{#each countries}}
            <span style="display: inline-flex !important; align-items: center !important; gap: 4px; flex-wrap: nowrap; vertical-align: middle;">
              {{#if isoAlpha2}}<img src="https://marlo-pdf-resources-dev.s3.us-east-1.amazonaws.com/flags/{{isoAlpha2}}.svg" alt="{{#if name}}{{name}}{{else}}Country{{/if}}" style="width: 20px; height: 15px; object-fit: contain; flex-shrink: 0;" />{{/if}}
              {{#if name}}<span style="font-size: 12px; color: #1F1F1F; text-transform: none; white-space: nowrap; display: inline-block; vertical-align: middle;">{{#safeEmpty}}{{name}}{{/safeEmpty}}</span>{{/if}}
            </span>
            {{/each}}
          </div>
        </div>
      ...
      <div class="contribution-card__block">
        ...
        <div style="display: flex; flex-wrap: wrap; gap: 12px; align-items: center; margin-top: 8px;">
          ...
          <div style="display: flex; align-items: center; gap: 6px;">
            {{#if iconUrl}}<img src="{{iconUrl}}" alt="{{#if name}}{{name}}{{else}}SDG{{/if}}" style="width: 40px; height: 40px; object-fit: contain; vertical-align: middle;" />{{/if}}
            {{#if name}}<span style="font-size: 10px; color: #1F1F1F; vertical-align: middle;">{{#safeEmpty}}{{name}}{{/safeEmpty}}</span>{{/if}}
          </div>
          {{/each}}
        </div>
      </div>
    ...
    <div class="contribution-card">
      ...
      <div class="contribution-card__block">
        ...
        <table style="width: 100%; border-collapse: collapse; margin-top: 12px; font-size: 10px;">
          <thead>
            <tr style="background: #f4f7fb; border-bottom: 1px solid #dfe6ee;">
              <th style="text-align: left; padding: 8px; font-weight: 600; color: #0a4c70; width: 15%;">ID</th>
              <th style="text-align: left; padding: 8px; font-weight: 600; color: #0a4c70;">Title</th>
            </tr>
          </thead>
          <tbody>
            {{#each deliverables}}
            <tr style="border-bottom: 1px solid #eef2f7;">
              <td style="padding: 8px; color: #0a4c70; font-weight: 600;">D{{#safeEmpty}}{{id}}{{/safeEmpty}}</td>
              <td style="padding: 8px; color: #1F1F1F;">{{#if title}}{{#safeEmpty}}{{title}}{{/safeEmpty}}{{else}}&lt;No title&gt;{{/if}}</td>
            </tr>
            {{/each}}
          </tbody>
        </table>
