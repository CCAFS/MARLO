...
package org.cgiar.ccafs.marlo.interceptor.bilateral;
 ...
 */
public class EditCofundedInterceptor extends AbstractInterceptor implements Serializable {
  ...
  @Inject
  public EditCofundedInterceptor(CrpManager crpManager,
  ...
  void setPermissionParameters(ActionInvocation invocation) {
    ...
    if (project != null) {
      ...
      if (baseAction.canAccessSuperAdmin() || baseAction.canAcessCrpAdmin()) {
        ...
        List<ProjectBilateralCofinancing> projects =
          projectBilateralCofinancingManager.getUserCofundedProjects(user.getId(), crp.getAcronym());
        if (projects.contains(project) && baseAction
          .hasPermission(baseAction.generatePermission(Permission.PROJECT_W3_COFUNDED_BASE_PERMISSION, params))) {
          canEdit = true;
        }
      }
