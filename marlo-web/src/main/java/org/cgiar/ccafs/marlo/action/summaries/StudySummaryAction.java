 ...
 */
public class StudySummaryAction extends BaseSummariesAction implements Summary {
  ...
  public String execute() throws Exception {
    ...
    try {
      Resource reportResource =
        resourceManager.createDirectly(this.getClass().getResource("/pentaho/crp/StudyPDF.prpt"), MasterReport.class);
  ...
  private TypedTableModel getMasterTableModel(String center, String date, String year) {
    ...
    TypedTableModel model = new TypedTableModel(new String[] {"center", "date", "year", "isReporting", "id", "type"},
      new Class[] {String.class, String.class, String.class, Boolean.class, Long.class, String.class});
    String type = null;
    // Type
    ...
    model.addRow(new Object[] {center, date, year, this.getSelectedPhase().isReporting(),
      projectExpectedStudyInfo.getProjectExpectedStudy().getId(), type});
    return model;
  }
  ...
  private TypedTableModel getStudyTableModel() {
    ...
    TypedTableModel model = new TypedTableModel(
      new String[] {"year", "policyAmount", "title", "status", "type", "outcomeImpactStatement", "isContributionText",
    ...
    String title = null, status = null, type = null, outcomeImpactStatement = null, isContributionText = null,
      policyInvestimentType = null, organizationType = null, stageProcess = null, stageStudy = null, srfTargets = null,
      subIdos = null, topLevelComments = null, geographicScope = null, region = null, countries = null,
      ...
      if (!projectExpectedStudyInfo.getRepIndGeographicScope().getId()
        ...
        if (studyCountries != null && studyCountries.size() > 0) {
          ...
          for (ProjectExpectedStudyCountry projectExpectedStudyCountry : studyCountries) {
            countriesSet.add("<br>&nbsp;&nbsp;&nbsp;&nbsp; ● " + projectExpectedStudyCountry.getLocElement().getName());
          }
    ...
    // Contact person
    if (projectExpectedStudyInfo.getContacts() != null && !projectExpectedStudyInfo.getContacts().trim().isEmpty()) {
      contacts = HTMLParser.plainTextToHtml(projectExpectedStudyInfo.getContacts());
    }

    // Projects
    List<ExpectedStudyProject> studyProjectList = projectExpectedStudyInfo.getProjectExpectedStudy()
      .getExpectedStudyProjects().stream()
      .filter(e -> e.isActive() && e.getPhase() != null && e.getPhase().equals(this.getSelectedPhase()))
      .sorted((sp1, sp2) -> sp2.getProject().getId().compareTo(sp1.getProject().getId())).collect(Collectors.toList());
    Set<String> studyProjectSet = new HashSet<>();
    if (projectExpectedStudyInfo.getProjectExpectedStudy().getProject() != null) {
      studyProjectSet.add(
        "<br>&nbsp;&nbsp;&nbsp;&nbsp; ● P" + projectExpectedStudyInfo.getProjectExpectedStudy().getProject().getId());
    }
    ...
    model.addRow(new Object[] {year, policyAmount, title, status, type, outcomeImpactStatement, isContributionText,
      policyInvestimentType, organizationType, stageProcess, stageStudy, srfTargets, subIdos, topLevelComments,
      geographicScope, region, countries, scopeComments, crps, flagships, regions, institutions,
