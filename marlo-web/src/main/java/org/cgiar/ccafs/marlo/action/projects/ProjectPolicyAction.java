...
public class ProjectPolicyAction extends BaseAction {
	...
	public ProjectPolicyAction(APConfig config, GlobalUnitManager globalUnitManager,
			...
			RepIndGenderYouthFocusLevelManager focusLevelManager,
			...
			ProjectPolicyCountryManager projectPolicyCountryManager,
			ProjectPolicyOwnerManager projectPolicyOwnerManager, ProjectPolicyCrpManager projectPolicyCrpManager,
			...
			ProjectPolicyValidator validator, ProjectExpectedStudyPolicyManager projectExpectedStudyPolicyManager,
	...
	public ProjectPolicyCrossCuttingMarker getPolicyCrossCuttingMarker(long markerID) {
		...
		if (this.isDraft()) {
			...
			if (policy.getCrossCuttingMarkers() != null) {
				for (ProjectPolicyCrossCuttingMarker projectPolicyCrossCuttingMarker : policy
						.getCrossCuttingMarkers()) {
			...
			crossCuttingMarker = projectPolicyCrossCuttingMarkerManager.getPolicyCrossCountryMarkerId(policyID,
	...
	public void prepare() throws Exception {
		...
		if (this.getRequest().getParameter(APConstants.TRANSACTION_ID) != null) {
			...
			if (policy.getProjectPolicyInfo() != null) {
				...
				if (policy.getProjectPolicyInfo().getRepIndPolicyInvestimentType() != null
					...
					policy.getProjectPolicyInfo().setRepIndPolicyInvestimentType(
							repIndPolicyInvestimentTypeManager.getRepIndPolicyInvestimentTypeById(
		...
		if (policy != null) {
			...
			if (path.toFile().exists() && this.getCurrentUser().isAutoSave()) {
				...
				if (policy.getGeographicScopes() != null) {
					for (ProjectPolicyGeographicScope projectPolicyGeographicScope : policy.getGeographicScopes()) {
						projectPolicyGeographicScope
								.setRepIndGeographicScope(repIndGeographicScopeManager.getRepIndGeographicScopeById(
										projectPolicyGeographicScope.getRepIndGeographicScope().getId()));
				...
				if (haveCountries) {
					...
					if (policy.getCountriesIdsText() != null) {
						String[] countriesText = policy.getCountriesIdsText().replace("[", "").replace("]", "")
								.split(",");
				...
				if (policy.getOwners() != null) {
					for (ProjectPolicyOwner projectPolicyOwner : policy.getOwners()) {
						projectPolicyOwner.setRepIndPolicyType(repIndPolicyTypeManager
				...
				if (policy.getCrps() != null) {
					for (ProjectPolicyCrp projectPolicyCrp : policy.getCrps()) {
						projectPolicyCrp.setGlobalUnit(
				...
				if (policy.getCrossCuttingMarkers() != null) {
					for (ProjectPolicyCrossCuttingMarker projectPolicyCrossCuttingMarker : policy
							.getCrossCuttingMarkers()) {
						projectPolicyCrossCuttingMarker.setCgiarCrossCuttingMarker(
								cgiarCrossCuttingMarkerManager.getCgiarCrossCuttingMarkerById(
										projectPolicyCrossCuttingMarker.getCgiarCrossCuttingMarker().getId()));
						if (projectPolicyCrossCuttingMarker.getRepIndGenderYouthFocusLevel() != null) {
							if (projectPolicyCrossCuttingMarker.getRepIndGenderYouthFocusLevel().getId() != -1) {
								projectPolicyCrossCuttingMarker.setRepIndGenderYouthFocusLevel(focusLevelManager
										.getRepIndGenderYouthFocusLevelById(projectPolicyCrossCuttingMarker
				...
				if (policy.getProjectPolicyCountries() == null) {
					...
					List<ProjectPolicyCountry> geographics = projectPolicyCountryManager
							.getPolicyCountrybyPhase(policy.getId(), phase.getId());

					// Load Countries
					policy.setCountries(
							geographics.stream().filter(sc -> sc.getLocElement().getLocElementType().getId() == 2)
									.collect(Collectors.toList()));
			...
			List<ProjectInnovation> innovations = projectL.getProjectInnovations().stream().filter(c -> c.isActive())
					.collect(Collectors.toList());
			...
			List<PolicyMilestone> milestones = policy.getPolicyMilestones().stream()
					.filter(c -> c.isActive() && c.getPhase() != null
							&& c.getPhase().getId().intValue() == this.getActualPhase().getId().intValue())
					.collect(Collectors.toList());
			 ...
			 */
			List<ProjectInnovationShared> innovationShareds = new ArrayList<>(project.getProjectInnovationShareds()
					.stream()
  ...
  public String save() {
		...
		if (policy.getCrossCuttingMarkers() != null) {
			for (ProjectPolicyCrossCuttingMarker crossCuttingOwner : policy.getCrossCuttingMarkers()) {
				if (crossCuttingOwner.getId() == null) {
					...
					if (crossCuttingOwner.getRepIndGenderYouthFocusLevel() != null) {
						if (crossCuttingOwner.getRepIndGenderYouthFocusLevel().getId() != null
							...
							RepIndGenderYouthFocusLevel focusLevel = focusLevelManager
									.getRepIndGenderYouthFocusLevelById(
											crossCuttingOwner.getRepIndGenderYouthFocusLevel().getId());
					...
					ProjectPolicyCrossCuttingMarker crossCuttingOwnerSave = projectPolicyCrossCuttingMarkerManager
							.getProjectPolicyCrossCuttingMarkerById(crossCuttingOwner.getId());
					...
					if (hasChanges) {
						projectPolicyCrossCuttingMarkerManager
								.saveProjectPolicyCrossCuttingMarker(crossCuttingOwnerSave);
					}
					// This is to add innovationCrpSave to generate correct auditlog.
					policy.getProjectPolicyCrossCuttingMarkers().add(crossCuttingOwnerSave);
	...
	public void saveEvidence(ProjectPolicy projectPolicy, Phase phase) {
		...
		if (projectPolicy.getProjectExpectedStudyPolicies() != null
			...
			List<ProjectExpectedStudyPolicy> policyPrev = new ArrayList<>(
					projectPolicy.getProjectExpectedStudyPolicies().stream()
							.filter(nu -> nu.isActive() && nu.getPhase().getId().equals(phase.getId()))
							.collect(Collectors.toList()));
		...
		if (policy.getEvidences() != null) {
			for (ProjectExpectedStudyPolicy studyPolicy : policy.getEvidences()) {
				if (studyPolicy.getId() == null) {
					...
					ProjectExpectedStudy expectedStudy = projectExpectedStudyManager
							.getProjectExpectedStudyById(studyPolicy.getProjectExpectedStudy().getId());
	...
	public void saveGeographicScopes(ProjectPolicy projectPolicy, Phase phase) {
		...
		if (projectPolicy.getProjectPolicyGeographicScopes() != null
			...
			List<ProjectPolicyGeographicScope> scopePrev = new ArrayList<>(
		...
		if (policy.getGeographicScopes() != null) {
			for (ProjectPolicyGeographicScope scope : policy.getGeographicScopes()) {
				if (scope.getId() == null) {
					...
					RepIndGeographicScope geoScope = repIndGeographicScopeManager
							.getRepIndGeographicScopeById(scope.getRepIndGeographicScope().getId());
	...
	public void saveInnovations(ProjectPolicy projectPolicy, Phase phase) {
		...
		// Search and deleted form Information
		if (projectPolicy.getProjectPolicyInnovations() != null
				&& projectPolicy.getProjectPolicyInnovations().size() > 0) {
		...
		if (policy.getInnovations() != null) {
			for (ProjectPolicyInnovation policyInnovation : policy.getInnovations()) {
				if (policyInnovation.getId() == null) {
					...
					ProjectInnovation innovation = projectInnovationManager
							.getProjectInnovationById(policyInnovation.getProjectInnovation().getId());
	...
	public void saveMilestones(ProjectPolicy projectPolicy, Phase phase) {
		...
		// Search and deleted form Information
		if (projectPolicy.getPolicyMilestones() != null
				&& projectPolicy.getPolicyMilestones().size() > 0) {

			List<PolicyMilestone> milestonePrev = new ArrayList<>(projectPolicy.getPolicyMilestones()
					.stream().filter(nu -> nu.isActive() && nu.getPhase().getId().equals(phase.getId()))
					.collect(Collectors.toList()));
		...
		if (policy.getOwners() != null) {
			for (ProjectPolicyOwner policyOwner : policy.getOwners()) {
				if (policyOwner.getId() == null) {
					...
					RepIndPolicyType repIndPolicyType = repIndPolicyTypeManager
							.getRepIndPolicyTypeById(policyOwner.getRepIndPolicyType().getId());
