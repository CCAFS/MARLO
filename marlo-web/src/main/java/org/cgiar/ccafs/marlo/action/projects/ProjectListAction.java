...
public class ProjectListAction extends BaseAction {
		...
		if (this.canAccessSuperAdmin()) {
			...
			if (liaisonUser != null && this.canAddBilateralProject()) {
				LiaisonInstitution liaisonInstitution = liaisonInstitutionManager
						.getLiaisonInstitutionById(liaisonUser.getLiaisonInstitution().getId());
	...
	public String delete() {
		...
		if (this.canDelete(projectID)) {
			String permissionStr = this.generatePermission(Permission.PROJECT_DELETE_BASE_PERMISSION,
			...
			if (permission) {
				...
				try {
					...
					project.setModificationJustification(
							"Deleted by : " + this.getCurrentUser().getComposedCompleteName());
					projectManager.deleteProject(project);
					for (ProjectPhase projectPhase : project.getProjectPhases()) {
						projectPhaseManager.deleteProjectPhase(projectPhase.getId());
					}
					this.addActionMessage("message:" + this.getText("deleting.successProject",
							new String[] { this.getText("project").toLowerCase() }));
				} catch (Exception e) {
					logger.error("Unable to delete project", e);
					this.addActionError(
							this.getText("deleting.problem", new String[] { this.getText("project").toLowerCase() }));
					/**
	...
	public void loadFlagshipgsAndRegions(List<Project> list) {
		for (Project project : list) {
			...
			project.setBilateralBudget(projectBudgetManager.getTotalBudget(project.getId(),
					this.getActualPhase().getId(), 3, this.getActualPhase().getYear()));
	...
	public void prepare() throws Exception {
			...
			if (liaisonUser != null) {
				for (Project project : centerProjects) {
					...
					if (info.getLiaisonInstitutionCenter() != null) {
						if (liaisonUser.getLiaisonInstitution().getId()
								.equals(info.getLiaisonInstitutionCenter().getId())) {
		...
		if (this.isReportingActive()) {
			...
			allProjects = allProjects.stream().filter(mp -> mp.isActive()
					&& mp.getProjecInfoPhase(this.getActualPhase()) != null
					&& (mp.getProjecInfoPhase(this.getActualPhase()).getEndDate() == null || Integer.parseInt(
							dateFormat.format(mp.getProjecInfoPhase(this.getActualPhase()).getEndDate())) >= this
									.getCurrentCycleYear()))
			...
			closedProjects = closedProjects.stream().filter(mp -> mp.isActive()
					&& mp.getProjecInfoPhase(this.getActualPhase()) != null
					&& (mp.getProjecInfoPhase(this.getActualPhase()).getEndDate() == null || Integer.parseInt(
							dateFormat.format(mp.getProjecInfoPhase(this.getActualPhase()).getEndDate())) >= this
									.getCurrentCycleYear()))
