...
public class DeliverableMetadataByWOS extends BaseAction {
  ...
  private com.google.gson.JsonArray asArray(JsonElement el) {
    ...
    // Accept string/object as singletons
    if (el.isJsonPrimitive() || el.isJsonObject()) {
      arr.add(el);
      return arr;
    }
    return null;
  ...
  private void copyIfPresent(JsonObject obj, String from, String to) {
    if (obj.has(from) && !obj.get(from).isJsonNull()) {
      obj.addProperty(to, safeAsString(obj.get(from)));
    }
  }

  /**
   * Ensures the given field is a JSON array.
   * If it's a primitive string/null, wrap it into a singleton array (or empty if blank).
   * If it's already an array, leave it as-is. If it's an object, wrap it as single-element array.
   */
  private void ensureArray(JsonObject obj, String field) {
    if (!obj.has(field)) {
      return;
    }
    ...
    if (el == null || el.isJsonNull()) {
      obj.add(field, new com.google.gson.JsonArray());
      return;
    }
    if (el.isJsonArray()) {
      return;
  ...
  public String execute() throws Exception {
    ...
    if (this.jsonStringResponse != null && !StringUtils.equalsIgnoreCase(this.jsonStringResponse, "null")) {
      ...
      if (!hasHandle) {
        ...
        try {
          JsonElement root = JsonParser.parseString(jsonStringResponse);
          ...
          if (root != null && root.isJsonObject()) {
            ...
            ensureArray(obj, "Authors");
            ensureArray(obj, "Affiliation");
            ensureArray(obj, "Keywords");
            ensureArray(obj, "Language");
            ensureArray(obj, "Countries");
            ensureArray(obj, "Funding source");
            ensureArray(obj, "Region of the research");

            this.responseToHandle = new Gson().fromJson(obj, MetadataWOSModelToHandle.class);
          } else {
            this.responseToHandle = new Gson().fromJson(jsonStringResponse, MetadataWOSModelToHandle.class);
          }
        } catch (Exception ex) {
          LOG.error("Unable to parse MetadataWOSModelToHandle: {}", ex.getMessage(), ex);
          // Manejo alterno: dejar responseToHandle en null o propagar
        }
        this.saveInfoToHandle();
        // 2024/07/15 cgamboa flow is created for data generated by handle query
      }
  ...
  private String safeAsString(JsonElement el) {
    if (el == null || el.isJsonNull()) {
      return "";
    }
    if (el.isJsonPrimitive()) {
  ...
  /**
   * Save affiliation (not mapped) data generated by the WOS service
   *
   * @param phase phase related to the process
   * @param deliverable deliverable related to the process
   * @return
   */
  private void saveAffiliationsNotMappedToHandle(Phase phase, Deliverable deliverable) {
    try {
      ...
      // 2024/07/15 cgamboa deliverableAffiliationsNotMappedManager.findAll() was changed by
      // findBydeliverableMetadataExternalSourcesId(long externalSourceId)
      List<DeliverableAffiliationsNotMapped> dbAffiliationsNotMappedTmp = new ArrayList<>();
      dbAffiliationsNotMappedTmp =
        this.deliverableAffiliationsNotMappedManager.findBydeliverableMetadataExternalSourcesId(externalSource.getId());
      if (incomingInstitutions != null) {
        List<DeliverableAffiliationsNotMapped> dbAffiliationsNotMapped =
          dbAffiliationsNotMappedTmp != null ? dbAffiliationsNotMappedTmp.stream()
            .filter(danm -> danm != null && danm.getId() != null && danm.getDeliverableMetadataExternalSources() != null
              && danm.getDeliverableMetadataExternalSources().getId() != null
              && danm.getDeliverableMetadataExternalSources().getId().equals(externalSource.getId()))
            .collect(Collectors.toList()) : Collections.emptyList();
      ...
      LOG.error(" unable to save affiliation not mapped data in saveAffiliationsNotMappedToHandle function");
    }
  }


  /**
   * Save affiliation data generated by the WOS service
   *
   * @param phase phase related to the process
   * @param deliverable deliverable related to the process
   * @return
  ...
  private void saveAltmetricInfoToandle(Phase phase, Deliverable deliverable) {
    try {
      DeliverableAltmetricInfo altmetricInfo =
        this.deliverableAltmetricInfoManager.findByPhaseAndDeliverable(phase, deliverable);
      MetadataAltmetricModel2 incomingAltmetricInfo = null;
      if (this.responseToHandle != null && this.responseToHandle.getAltmetricInfo2() != null) {
        incomingAltmetricInfo = this.responseToHandle.getAltmetricInfo2();
      }
      if (incomingAltmetricInfo != null) {
        if (altmetricInfo == null) {
          altmetricInfo = new DeliverableAltmetricInfo();
        ...
        altmetricInfo.setAltmetricId(incomingAltmetricInfo.getAltmetricId());
        altmetricInfo.setAltmetricJid(incomingAltmetricInfo.getAltmetricJid());
        altmetricInfo.setAuthors(incomingAltmetricInfo.getAuthors() != null ? incomingAltmetricInfo.getAuthors()
          .stream().filter(StringUtils::isNotBlank).collect(Collectors.joining("; ")) : null);
        altmetricInfo.setCitedByBlogs(incomingAltmetricInfo.getCitedByBlogs());
        altmetricInfo.setCitedByDelicious(incomingAltmetricInfo.getCitedByDelicious());
        altmetricInfo.setCitedByFacebookPages(incomingAltmetricInfo.getCitedByFacebookPages());
        altmetricInfo.setCitedByForumUsers(incomingAltmetricInfo.getCitedByForumUsers());
        ...
        altmetricInfo.setCitedByRedditUsers(incomingAltmetricInfo.getCitedByRedditUsers());
        altmetricInfo
          .setCitedByResearchHighlightPlatforms(incomingAltmetricInfo.getCitedByResearchHighlightPlatforms());
        altmetricInfo.setCitedByStackExchangeResources(incomingAltmetricInfo.getCitedByStackExchangeResources());
        altmetricInfo.setCitedByTwitterUsers(incomingAltmetricInfo.getCitedByTwitterUsers());
        altmetricInfo.setCitedByWeiboUsers(incomingAltmetricInfo.getCitedByWeiboUsers());
        altmetricInfo.setCitedByWikipediaPages(incomingAltmetricInfo.getCitedByWikipediaPages());
      ...
      LOG.error(" uanble to save almetric data in saveAltmetricInfoToandle function");
    }
  }
  ...
  /**
   * Save external source authors data generated by the WOS service
   *
   * @param phase phase related to the process
   * @param deliverable deliverable related to the process
   * @return
  ...
  private void saveExternalSourceAuthorsToHandle(Phase phase, Deliverable deliverable) {
    try {
      ...
      LOG.error("unable to save external source author data in saveExternalSourceAuthorsToHandle function ");
    }
  }
  ...
  /**
   * Save external source data generated by the WOS service
   *
   * @param phase phase related to the process
   * @param deliverable deliverable related to the process
   * @return
  ...
  private void saveExternalSourcesToHandle(Phase phase, Deliverable deliverable) {
    try {
      ...
      // externalSource.setOpenAccessLink(this.response.getOpenAcessLink());
      externalSource.setIsiStatus(this.getBooleanStringOrNotAvailable(this.responseToHandle.getIsISI()));
      externalSource.setJournalName(this.responseToHandle.getJournalName());
      // externalSource.setVolume(this.response.getVolume());
      externalSource.setDoi(this.responseToHandle.getDoi());
  ...
  /**
   * Save data generated by the WOS service
   *
   * @return
   */
  private void saveInfoToHandle() {
    ...
    this.saveAltmetricInfoToandle(phase, deliverable);
  ...
  public JsonElement transformObjectToHandle(JsonObject jsonObject) {
    try {
      ...
      // Simple field remaps
      copyIfPresent(jsonObject, "Handle", "url");
      copyIfPresent(jsonObject, "Title", "title");
      copyIfPresent(jsonObject, "Type", "publicationType");
      copyIfPresent(jsonObject, "Pages", "pages");
      copyIfPresent(jsonObject, "Journal", "journalName");
      copyIfPresent(jsonObject, "ISI", "isISI");

      // Authors can be string or array -> normalize to ["..."]
      com.google.gson.JsonArray authorsSrc = asArray(jsonObject.get("Authors"));
      if (authorsSrc != null) {
        com.google.gson.JsonArray authorsOut = new com.google.gson.JsonArray();
        for (JsonElement authorEl : authorsSrc) {
          String authorName = safeAsString(authorEl);
          if (org.apache.commons.lang3.StringUtils.isNotBlank(authorName)) {
            JsonObject authorObj = new JsonObject();
            authorObj.addProperty("fullName", authorName);
            authorsOut.add(authorObj);
          }
        }
        jsonObject.add("authors", authorsOut);
      }

      // Affiliation can be object/array/string -> iterate safely
      com.google.gson.JsonArray affSrc = asArray(jsonObject.get("Affiliation"));
      if (affSrc != null) {
        com.google.gson.JsonArray instOut = new com.google.gson.JsonArray();
        for (JsonElement instEl : affSrc) {
          if (instEl == null || instEl.isJsonNull()) {
            continue;
          }

          if (instEl.isJsonObject()) {
            JsonObject src = instEl.getAsJsonObject();
            String fullName = safeAsString(src.get("name"));
            ...
            if (src.has("prediction") && src.get("prediction").isJsonObject()) {
              JsonObject pred = src.getAsJsonObject("prediction");
              confidant = safeAsString(pred.get("confidant"));
              if (pred.has("value") && pred.get("value").isJsonObject()) {
                JsonObject val = pred.getAsJsonObject("value");
                code = safeAsString(val.get("code"));
                country = safeAsString(val.get("hqLocationISOalpha2"));
              }
            }

            JsonObject oneInstitution = new JsonObject();
            oneInstitution.addProperty("fullName", fullName);
            oneInstitution.addProperty("clarisaMatchConfidence", confidant);
            oneInstitution.addProperty("clarisaId", code);
            oneInstitution.addProperty("country", country);
            instOut.add(oneInstitution);

          } else if (instEl.isJsonPrimitive()) {
            // Rare case: plain string
            String name = safeAsString(instEl);
            if (!name.isEmpty()) {
              JsonObject oneInstitution = new JsonObject();
              oneInstitution.addProperty("fullName", name);
              oneInstitution.addProperty("clarisaMatchConfidence", "0");
              jsonObject.addProperty("clarisaId", "0");
              instOut.add(oneInstitution);
            }
          }
        }
        jsonObject.add("institutions", instOut);
      }

      // Open Access mapping
      if (jsonObject.has("Open Access")) {
        String val = safeAsString(jsonObject.get("Open Access"));
        String oa = "false";
        if ("Open Access".equalsIgnoreCase(val) || "OpenAccess".equalsIgnoreCase(val) || "true".equalsIgnoreCase(val)) {
          oa = "true";
        }
        jsonObject.addProperty("isOpenAccess", oa);
      }

      // Issued date -> year
      if (jsonObject.has("Issued date")) {
        String date = safeAsString(jsonObject.get("Issued date")); // e.g. "2024-12-30"
        String year = date.matches("\\d{4}(-.*)?") ? date.substring(0, 4) : "";
        if (!year.isEmpty()) {
          jsonObject.addProperty("publicationYear", Integer.parseInt(year));
        }
      }

      // DOI passthrough
      if (jsonObject.has("DOI")) {
        String doi = safeAsString(jsonObject.get("DOI"));
        if (!doi.isEmpty()) {
          jsonObject.addProperty("doi", doi);
        }
      }

      return jsonObject;
    } catch (Exception e) {
      LOG.error("unable to transform object in transformObjectToHandle", e);
      return null;
    }
  }
}