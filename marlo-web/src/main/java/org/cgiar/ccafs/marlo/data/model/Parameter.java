package org.cgiar.ccafs.marlo.data.model;
// Generated May 17, 2017 3:36:29 PM by Hibernate Tools 4.3.1.Final


import java.util.HashSet;
import java.util.Set;

/**
 * Parameter generated by hbm2java
 */
public class Parameter extends MarloBaseEntity implements java.io.Serializable {


  /**
   * 
   */
  private static final long serialVersionUID = 3839084224660636191L;

  private String key;
  private String description;
  private Integer format;
  private String defaultValue;
  private Integer category;

  private GlobalUnitType globalUnitType;
  private Set<CustomParameter> customParameters = new HashSet<CustomParameter>(0);

  public Parameter() {
  }

  public Integer getCategory() {
    return this.category;
  }


  public Set<CustomParameter> getCustomParameters() {
    return this.customParameters;
  }

  public String getDefaultValue() {
    return this.defaultValue;
  }

  public String getDescription() {
    return this.description;
  }

  public Integer getFormat() {
    return this.format;
  }

  public GlobalUnitType getGlobalUnitType() {
    return globalUnitType;
  }

  public String getKey() {
    return this.key;
  }

  public void setCategory(Integer category) {
    this.category = category;
  }

  public void setCustomParameters(Set<CustomParameter> customParameterses) {
    this.customParameters = customParameterses;
  }

  public void setDefaultValue(String defaultValue) {
    this.defaultValue = defaultValue;
  }

  public void setDescription(String description) {
    this.description = description;
  }

  public void setFormat(Integer format) {
    this.format = format;
  }

  public void setGlobalUnitType(GlobalUnitType globalUnitType) {
    this.globalUnitType = globalUnitType;
  }

  public void setKey(String key) {
    this.key = key;
  }

  @Override
  public String toString() {
    return "Parameter [id=" + this.getId() + ", key=" + key + ", description=" + description + ", format=" + format
      + ", defaultValue=" + defaultValue + ", category=" + category + "]";
  }


}

