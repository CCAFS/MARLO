...
public class ProjectsSummaryAction extends BaseSummariesAction implements Summary {
	...
	@Inject
	public ProjectsSummaryAction(APConfig config, GlobalUnitManager crpManager,
	...
	private MasterReport addi8nParameters(MasterReport masterReport) {
		...
		masterReport.getParameterValues().put("i8nInstitutionLeader",
				this.getText("summaries.oaprojects.leadInstitution"));
		masterReport.getParameterValues().put("i8nProjectLeader", this.getText("summaries.oaprojects.projectLeader"));
		masterReport.getParameterValues().put("i8nActivitiesAmount",
				this.getText("summaries.oaprojects.activitiesAmount"));
	...
	public String execute() throws Exception {
		...
		try {
			Resource reportResource = resourceManager
					.createDirectly(this.getClass().getResource("/pentaho/crp/Projects.prpt"), MasterReport.class);
	...
	private TypedTableModel getProjecsDetailsTableModel() {
		TypedTableModel model = new TypedTableModel(new String[] { "projectId", "projectTitle", "projectSummary",
				"status", "managementLiaison", "flagships", "regions", "institutionLeader", "projectLeader",
				"activitiesOnGoing", "expectedDeliverables", "outcomes", "expectedStudies", "phaseID", "crossCutting",
				"type", "locations", "start_date", "end_date", "budgetw1w2", "totalw3bilateralcenter", "ppa", "global",
				"regional", "regionLoc", "countryLoc", "deliverables2017", "deliverables2018", "deliverables2019",
				"deliverables2020", "deliverables2021", "deliverables2022", "projectCollaborators" },
				new Class[] { Long.class, String.class, String.class, String.class, String.class, String.class,
						...
						Integer.class, Long.class, String.class, String.class, String.class, String.class, String.class,
		...
		for (Project project : activeProjects) {
			...
			for (ProjectBudget projectBudget : project.getProjectBudgets().stream()
					.filter(pb -> pb.isActive() && pb.getYear() == this.getSelectedYear()
							&& pb.getFundingSource() != null && pb.getPhase() != null
				...
				if (projectBudget.getFundingSource().getFundingSourceInfo(this.getSelectedPhase()) != null
						&& projectBudget.getFundingSource().getFundingSourceInfo(this.getSelectedPhase())
								.getBudgetType() != null
						&& projectBudget.getFundingSource().getFundingSourceInfo(this.getSelectedPhase())
								.getBudgetType().getName() != null) {
					typeList.add(projectBudget.getFundingSource().getFundingSourceInfo(this.getSelectedPhase())
							.getBudgetType().getName());
				}
			}
			// Remove duplicates
			...
			for (Activity activity : project.getActivities().stream()
					...
					.filter(a -> a.isActive() && (a.getActivityStatus() == 2) && a.getPhase() != null
			...
			for (yearSearched = 2017; yearSearched <= 2022; yearSearched++) {
				for (Deliverable deliverable : project.getDeliverables().stream()
						...
						.filter(d -> d.isActive() && d.getDeliverableInfo(this.getActualPhase()) != null
								&& d.getDeliverableInfo().isActive()
								&& ((d.getDeliverableInfo().getStatus() == null
										&& d.getDeliverableInfo().getYear() == yearSearched)
										|| (d.getDeliverableInfo().getStatus() != null
												&& d.getDeliverableInfo().getStatus().intValue() == Integer
														.parseInt(ProjectStatusEnum.Extended.getStatusId())
												&& d.getDeliverableInfo().getNewExpectedYear() != null
			...
			for (ExpectedStudyProject expectedStudyProject : project.getExpectedStudyProjects().stream().filter(
					c -> c.isActive() && c.getProjectExpectedStudy().getPhase() == this.getActualPhase().getId())
					.collect(Collectors.toList())) {
			...
			if (!project.getProjectLocations().isEmpty()) {
				...
				locationTypes = project.getProjectLocationElementTypes().stream()
						.filter(plt -> plt.getIsGlobal() && plt.getLocElementType().isActive())
						.collect(Collectors.toList());
				...
				for (ProjectLocation pl : project.getProjectLocations().stream().filter(
			...
			try {
				...
				LOG.warn("Failed to get " + APConstants.CYCLE
		...
		if (budgets != null) {
			for (ProjectBudget projectBudget : budgets) {
				if (projectBudget.getPhase().equals(this.getSelectedPhase())) {
					...
					double gender = projectBudget.getGenderPercentage() != null ? projectBudget.getGenderPercentage()
							: 0.0;
	 ...
	 * @param coFinancing coFinancing 1: cofinancing+no cofinancing, 2: cofinancing
	 *                    3: no cofinancing
