...
public class FinancialSummaryAction extends BaseAction {
	...
	public void prepare() throws Exception {
		...
		if (this.getRequest().getParameter(APConstants.TRANSACTION_ID) != null) {
			...
			try {
				synthesisID = Long
						.parseLong(StringUtils.trim(this.getRequest().getParameter(APConstants.REPORT_SYNTHESIS_ID)));
				...
				if (this.isSelectedPhaseAR2021()) {
					if (this.isPMU()) {
						...
						if (this.isEmpty(reportSynthesis.getReportSynthesisCrpFinancialReports())) {
							ReportSynthesisCrpFinancialReport reportSynthesisCrpFinancialReport = new ReportSynthesisCrpFinancialReport();
							// create one to one relation
							reportSynthesis.setReportSynthesisCrpFinancialReport(reportSynthesisCrpFinancialReport);
							reportSynthesis.getReportSynthesisCrpFinancialReports()
									.add(reportSynthesisCrpFinancialReport);
							reportSynthesisCrpFinancialReport.setReportSynthesis(reportSynthesis);
							// save the changes
							reportSynthesis = reportSynthesisManager.saveReportSynthesis(reportSynthesis);
						} else {
							reportSynthesis.setReportSynthesisCrpFinancialReport(
									reportSynthesis.getReportSynthesisCrpFinancialReports().stream()
											.sorted((f1, f2) -> Comparator
													.comparing(ReportSynthesisCrpFinancialReport::getActiveSince)
													.reversed().compare(f1, f2))
					...
					if (this.isPMU()) {
						...
						if (reportSynthesis.getReportSynthesisFinancialSummary()
							...
							reportSynthesis.getReportSynthesisFinancialSummary()
									.setBudgets(new ArrayList<>(reportSynthesis.getReportSynthesisFinancialSummary()
							...
							List<LiaisonInstitution> flagshipList = loggedCrp.getLiaisonInstitutions().stream()
									.filter(c -> c.getCrpProgram() != null && c.isActive() && c.getCrpProgram()
											.getProgramType() == ProgramType.FLAGSHIP_PROGRAM_TYPE.getValue())
									.collect(Collectors.toList());
							...
							for (LiaisonInstitution liInstitution : flagshipList) {
								...
								reportSynthesis.getReportSynthesisFinancialSummary().getBudgets()
										.add(financialSummaryBudget);
							}
		...
		liaisonInstitutions
				.addAll(loggedCrp
						.getLiaisonInstitutions().stream().filter(c -> c.getCrpProgram() == null && c.isActive()
								&& c.getAcronym() != null && c.getAcronym().equals("PMU"))
						.collect(Collectors.toList()));
		...
		if (!this.isSelectedPhaseAR2021() && this.isFlagship()) {
			...
			if (reportSynthesisDB != null) {
				if (reportSynthesisDB.getReportSynthesisFinancialSummary() != null) {
					...
					if (reportSynthesisDB.getReportSynthesisFinancialSummary()
							.getReportSynthesisFinancialSummaryBudgets() != null
							&& !reportSynthesisDB.getReportSynthesisFinancialSummary()
									.getReportSynthesisFinancialSummaryBudgets().isEmpty()) {
						reportSynthesis.getReportSynthesisFinancialSummary()
								.setBudgets(new ArrayList<>(reportSynthesisDB.getReportSynthesisFinancialSummary()
	...
	public String save() {
		if (this.hasPermission("canEdit")) {
			...
			if (this.isSelectedPhaseAR2021()) {
				ReportSynthesisCrpFinancialReport reportSynthesisCrpFinancialReportDB = this.reportSynthesisCrpFinancialReportManager
						.findByReportSynthesis(synthesisID);

				if (this.isPMU()) {
					this.saveCrpFinancialReport(reportSynthesisCrpFinancialReportDB);
				}
			} else {
				ReportSynthesisFinancialSummary financialSummaryDB = reportSynthesisManager
						.getReportSynthesisById(synthesisID).getReportSynthesisFinancialSummary();

				if (this.isPMU()) {
					this.saveFinancialSummaryBudgets(financialSummaryDB);
					financialSummaryDB
							.setNarrative(reportSynthesis.getReportSynthesisFinancialSummary().getNarrative());

					financialSummaryDB = reportSynthesisFinancialSummaryManager
							.saveReportSynthesisFinancialSummary(financialSummaryDB);
				}
			}
	...
	private void saveCrpFinancialReport(ReportSynthesisCrpFinancialReport reportSynthesisCrpFinancialReportDB) {
		...
		if (crpFinancialReport != null) {
			reportSynthesisCrpFinancialReportDB
					.setFinancialStatusNarrative(crpFinancialReport.getFinancialStatusNarrative());
			...
			reportSynthesisCrpFinancialReportDB
					.setCapitalEquipmentComments(crpFinancialReport.getCapitalEquipmentComments());
			...
			reportSynthesisCrpFinancialReportDB.setCollaboratorPartnerships2020Forecast(
					crpFinancialReport.getCollaboratorPartnerships2020Forecast());
			reportSynthesisCrpFinancialReportDB
			...
			reportSynthesisCrpFinancialReportDB
					.setConsultancy2020Forecast(crpFinancialReport.getConsultancy2020Forecast());
				...
				if (budget.getId() == null) {
					...
					if (budget.getLiaisonInstitution() != null && budget.getLiaisonInstitution().getId() != null) {
						LiaisonInstitution liaisonInstitution = liaisonInstitutionManager
								.getLiaisonInstitutionById(budget.getLiaisonInstitution().getId());

						budgetSave.setLiaisonInstitution(liaisonInstitution);
					} else {
						PowbExpenditureAreas expenditureAreas = powbExpenditureAreasManager
								.getPowbExpenditureAreasById(budget.getExpenditureArea().getId());
					...
					ReportSynthesisFinancialSummaryBudget budgetPrev = reportSynthesisFinancialSummaryBudgetManager
							.getReportSynthesisFinancialSummaryBudgetById(budget.getId());
					...
					if (hasChanges) {
						reportSynthesisFinancialSummaryBudgetManager
								.saveReportSynthesisFinancialSummaryBudget(budgetPrev);
					}
