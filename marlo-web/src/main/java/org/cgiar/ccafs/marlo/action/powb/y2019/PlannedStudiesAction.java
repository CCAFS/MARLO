 ...
 */
public class PlannedStudiesAction extends BaseAction {
  ...
  @Inject
  public PlannedStudiesAction(APConfig config, GlobalUnitManager crpManager, PowbSynthesisManager powbSynthesisManager,
  ...
  private String getGeographicScopeString(ProjectExpectedStudy projectExpectedStudy) {
    StringBuilder sbRegions = new StringBuilder();
    StringBuilder sbCountries = new StringBuilder();

    List<ProjectExpectedStudyGeographicScope> geographicScopes =
      projectExpectedStudyGeographicScopeManager.findAll().stream()
        .filter(g -> g != null && g.getPhase() != null && g.getPhase().equals(this.getActualPhase())
          && g.getProjectExpectedStudy() != null && g.getProjectExpectedStudy().getId() != null
          && g.getProjectExpectedStudy().getId().equals(projectExpectedStudy.getId()))
        .collect(Collectors.toList());

    if (geographicScopes != null && !geographicScopes.isEmpty()) {
      for (ProjectExpectedStudyGeographicScope geographicScope : geographicScopes) {
        if (geographicScope.getRepIndGeographicScope() != null
          && geographicScope.getRepIndGeographicScope().getId() != null) {
          if (geographicScope.getRepIndGeographicScope().getId().intValue() == GlobalScopeEnum.GLOBAL.getId()) {
            sbRegions = sbRegions.append(GlobalScopeEnum.GLOBAL.getType());
            break;
          } else if (geographicScope.getRepIndGeographicScope().getId().intValue() == GlobalScopeEnum.REGIONAL
            .getId()) {
            List<ProjectExpectedStudyRegion> regions = projectExpectedStudyRegionManager
              .getProjectExpectedStudyRegionbyPhase(projectExpectedStudy.getId(), this.getActualPhase().getId());

            if (regions != null && !regions.isEmpty()) {
              if (sbRegions.indexOf(": ") == -1) {
                sbRegions = sbRegions.append(GlobalScopeEnum.REGIONAL.getType()).append(": ");
              }

              for (ProjectExpectedStudyRegion region : regions) {
                if (region.getLocElement() != null && region.getLocElement().getName() != null
                  && !region.getLocElement().getName().isEmpty()) {
                  String name = region.getLocElement().getName();
                  sbRegions = sbRegions.append(name.indexOf(',') != -1 ? '(' : "").append(name)
                    .append(name.indexOf(',') != -1 ? ')' : "").append(", ");
                }
              }
            }
          } else {
            List<ProjectExpectedStudyCountry> countries = projectExpectedStudyCountryManager
              .getProjectExpectedStudyCountrybyPhase(projectExpectedStudy.getId(), this.getActualPhase().getId());

            if (countries != null && !countries.isEmpty()) {
              if (sbCountries.indexOf(": ") == -1) {
                sbCountries = sbCountries.append(GlobalScopeEnum.NATIONAL.getType()).append(": ");
              }

              for (ProjectExpectedStudyCountry country : countries) {
                if (country.getLocElement() != null && country.getLocElement().getName() != null
                  && !country.getLocElement().getName().isEmpty()) {
                  String name = country.getLocElement().getName();
                  sbCountries = sbCountries.append(name.indexOf(',') != -1 ? '(' : "").append(name)
                    .append(name.indexOf(',') != -1 ? ')' : "").append(", ");
                }
              }
            }
    ...
    int regionComma = sbRegions.lastIndexOf(", ");
    if (regionComma != -1) {
      sbRegions = sbRegions.delete(regionComma, sbRegions.length());
    }

    int countryComma = sbCountries.lastIndexOf(", ");
    if (countryComma != -1) {
      sbCountries = sbCountries.delete(countryComma, sbCountries.length());
    }

    return sbRegions.append(sbRegions.length() > 0 ? '\n' : "").append(sbCountries).toString();
  }
  ...
  public void popUpProject(long phaseID, LiaisonInstitution liaisonInstitution) {
    ...
    if (this.isFlagship()) {
      if (projectFocusManager.findAll() != null) {
        ...
        for (ProjectFocus focus : projectFocus) {
          ...
          for (ProjectExpectedStudy projectExpectedStudy : expectedStudies) {
            ...
            projectExpectedStudy.setGeographicScopeString(this.getGeographicScopeString(projectExpectedStudy));
      ...
      for (PowbEvidencePlannedStudyDTO powbEvidencePlannedStudyDTO : flagshipPlannedList) {
        ...
        expectedStudy.setSelectedFlahsgips(new ArrayList<>());
        expectedStudy.getSelectedFlahsgips().addAll(powbEvidencePlannedStudyDTO.getLiaisonInstitutions());
        expectedStudy.setGeographicScopeString(this.getGeographicScopeString(expectedStudy));
      ...
      if (path.toFile().exists() && this.getCurrentUser().isAutoSave()) {
        ...
        if (powbSynthesis.getPowbEvidence().getPlannedStudiesValue() != null) {
          ...
          for (int i = 0; i < studyValues.length; i++) {
             ...
             * .getProjectExpectedStudyCountrybyPhase(study.getId(), phase.getId())
             * .stream()
            ...
            if (study.getProjectExpectedStudyGeographicScopes() != null) {
              study.setGeographicScopes(new ArrayList<>(study.getProjectExpectedStudyGeographicScopes().stream()
  ...
  public String save() {
    if (this.hasPermission("canEdit")) {
       ...
       * the @PowbSynthesis if we want a row created in the auditlog table.
       */
