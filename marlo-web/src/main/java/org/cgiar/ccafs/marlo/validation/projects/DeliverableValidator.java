...
public class DeliverableValidator extends BaseValidator {
	...
	public DeliverableValidator(GlobalUnitManager crpManager, ProjectManager projectManager,
			...
			RepIndTypeActivityManager repIndTypeActivityManager, DeliverableUserManager deliverableUserManager) {
	...
	private Path getAutoSaveFilePath(Deliverable deliverable, long crpID, BaseAction action) {
		...
		String autoSaveFile = deliverable.getId() + "_" + composedClassName + "_" + action.getActualPhase().getName()
	...
	public void validate(BaseAction action, Deliverable deliverable, boolean saving) {
		...
		if (action.isPlanningActive()) {
			if (deliverable.getDeliverableInfo().getStatus() != null && deliverable.getDeliverableInfo()
				...
				validate = deliverable.getDeliverableInfo(action.getActualPhase()).getYear() >= action
						.getCurrentCycleYear();
			}
			if (deliverable.getDeliverableInfo().getStatus() != null && deliverable.getDeliverableInfo()
		...
		if (validate) {
			...
			if (!(deliverable.getDeliverableInfo(action.getActualPhase()).getStatus() != null
				...
				Boolean isManagingPartnerPersonRequerid = action
						.hasSpecificities(APConstants.CRP_MANAGING_PARTNERS_CONTACT_PERSONS);
							...
							if (isManagingPartnerPersonRequerid) {
								if (deliverable.getOtherPartnerships().get(i).getPartnershipPersons() == null) {
									...
									action.getInvalidFields().put(
											"deliverable.otherPartnerships[" + i + "].partnershipPersons",
											InvalidFieldsMessages.EMPTYFIELD);
								} else {
									boolean haveUser = false;
									for (int j = 0; j < deliverable.getOtherPartnerships().get(i)
											.getPartnershipPersons().size(); j++) {
										if (deliverable.getOtherPartnerships().get(i).getPartnershipPersons().get(j)
												.getUser() != null
												&& deliverable.getOtherPartnerships().get(i).getPartnershipPersons()
														.get(j).getUser().getId() != null) {
											haveUser = true;
											break;
										}
									}
				...
				if (deliverable.getResponsiblePartnership() == null) {
					...
					action.getInvalidFields().put("deliverable.responsiblePartnership", action
							.getText(InvalidFieldsMessages.EMPTYLIST, new String[] { "deliverable responsible" }));
				} else {
					...
					for (int i = 0; i < deliverable.getResponsiblePartnership().size(); i++) {
						if (deliverable.getResponsiblePartnership().get(i).getInstitution() == null
							...
							action.getInvalidFields().put(
									"deliverable.responsiblePartnership[" + i + "].institution.id",
									InvalidFieldsMessages.EMPTYFIELD);
							...
							if (isManagingPartnerPersonRequerid) {
								if (deliverable.getResponsiblePartnership().get(i).getPartnershipPersons() == null) {
									...
									action.getInvalidFields().put(
											"deliverable.responsiblePartnership[" + i + "].partnershipPersons",
											InvalidFieldsMessages.EMPTYFIELD);
								} else {
									boolean haveUser = false;
									for (int j = 0; j < deliverable.getResponsiblePartnership().get(i)
											.getPartnershipPersons().size(); j++) {
										if (deliverable.getResponsiblePartnership().get(i).getPartnershipPersons()
												.get(j).getUser() != null
												&& deliverable.getResponsiblePartnership().get(i)
														.getPartnershipPersons().get(j).getUser().getId() != null) {
											haveUser = true;
											break;
										}
									}
			...
			if (action.isReportingActive()) {
				...
				if (action.hasSpecificities(APConstants.CRP_HAS_DISEMINATION)
					...
					boolean isPRP = false;
					// type 63 = Peer-reviewed publication (PRP). doi should
					// only be mandatory for PRPs
					if (action.getActualPhase() != null
							&& deliverable.getDeliverableInfo(action.getActualPhase()) != null
					...
					if (deliverable.getDissemination() != null
							&& deliverable.getDissemination().getAlreadyDisseminated() != null) {
						isDisseminated = deliverable.getDissemination().getAlreadyDisseminated();
					...
					this.validatePublicationMetadata(deliverable.getPublication(), deliverable.getDeliverableInfo(),
							action);
					 ...
					 * getDeliverableType().getId().intValue() == 63) { if
					 * (deliverable.getDissemination() == null) {
					 * //action.addMessage(action.getText(
					 * "project.deliverable.v.qualityCheck.assurance"));
					 * //action.getInvalidFields().put(
					 * "input-deliverable.qualityCheck.qualityAssurance.id", //
					 * InvalidFieldsMessages.EMPTYFIELD); } }
					 */
		...
		// Validate Status if the extended year is greater than the current
		// phase year
	...
	private void validateDeliverableInfo(DeliverableInfo deliverableInfo, Deliverable deliverable, Project project,
		...
		if (deliverable.getDeliverableInfo(action.getActualPhase()).getDeliverableType() != null) {
			if (deliverable.getDeliverableInfo(action.getActualPhase()).getDeliverableType().getId() == null
				...
				if (deliverable.getDeliverableInfo(action.getActualPhase()).getDeliverableType()
					...
					if (deliverable.getDeliverableInfo(action.getActualPhase()).getDeliverableType()
							.getDeliverableCategory().getId() == -1) {
						action.addMessage(action.getText("project.deliverable.generalInformation.type"));
						action.getInvalidFields().put(
								"input-deliverable.deliverableInfo.deliverableType.deliverableType.id",
								InvalidFieldsMessages.EMPTYFIELD);
						deliverable.getDeliverableInfo(action.getActualPhase()).setDeliverableType(null);
		...
		if (deliverable.getDeliverableInfo(action.getActualPhase()).getStatus() != null
			...
			if (!(this.isValidString(deliverable.getDeliverableInfo(action.getActualPhase()).getStatusDescription())
					&& this.wordCount(
							deliverable.getDeliverableInfo(action.getActualPhase()).getStatusDescription()) <= 150)) {
		...
		if (!action.isCenterGlobalUnit()) {
			if (!(project.getProjecInfoPhase(action.getActualPhase()).getAdministrative() != null && project
					.getProjecInfoPhase(action.getActualPhase()).getAdministrative().booleanValue() == true)) {
				if (deliverable.getDeliverableInfo(action.getActualPhase()).getCrpClusterKeyOutput() != null
						&& deliverable.getDeliverableInfo(action.getActualPhase()).getCrpClusterKeyOutput()
								.getId() != null) {
					if (deliverable.getDeliverableInfo(action.getActualPhase()).getCrpClusterKeyOutput()
							.getId() == -1) {
		...
		// deliverableInfo.getGeographicScope().getId() == null
		...
		// } else {
		// if
		// (deliverableInfo.getGeographicScope().getId().equals(action.getReportingIndGeographicScopeRegional()))
		// {
		// if (deliverable.getDeliverableRegions() == null) {
		// action.addMessage(action.getText("deliverable.region"));
		// action.getInvalidFields().put("input-deliverable.deliverableRegions",
		// InvalidFieldsMessages.EMPTYFIELD);
		// }
		// }
		// if
		// (deliverableInfo.getGeographicScope().getId().equals(action.getReportingIndGeographicScopeMultiNational())
		// ||
		...
		// deliverable.getCountriesIds().isEmpty()) {
		...
		if (deliverable.getCrossCuttingMarkers() != null) {
			...
			for (CgiarCrossCuttingMarker cgiarCrossCuttingMarker : cgiarCrossCuttingMarkers) {
				List<DeliverableCrossCuttingMarker> deliverableCrossCuttingMarkers = deliverable
						.getCrossCuttingMarkers().stream()
		...
		if (deliverableParticipant.getHasParticipants()) {
			...
			if (deliverableParticipant.getRepIndTypeActivity() == null
				...
				RepIndTypeActivity repIndTypeActivity = repIndTypeActivityManager
						.getRepIndTypeActivityById(deliverableParticipant.getRepIndTypeActivity().getId());
	...
	public void validateDissemination(DeliverableDissemination dissemination, boolean saving, BaseAction action,
			boolean isPRP) {
		/*
		 * if (dissemination.getIsOpenAccess() != null) { if
		 * (!dissemination.getIsOpenAccess().booleanValue()) { Boolean
		 * hasIntellectualProperty = (dissemination.getIntellectualProperty() !=
		 * null && dissemination.getIntellectualProperty().booleanValue()) ||
		 * (dissemination.getType() != null &&
		 * dissemination.getType().equals("intellectualProperty")); Boolean
		 * hasLimitedExclusivity = (dissemination.getLimitedExclusivity() !=
		 * null && dissemination.getLimitedExclusivity().booleanValue()) ||
		 * (dissemination.getType() != null &&
		 * dissemination.getType().equals("limitedExclusivity")); Boolean
		 * hasRestrictedUse = (dissemination.getRestrictedUseAgreement() != null
		 * && dissemination.getRestrictedUseAgreement().booleanValue()) ||
		 * (dissemination.getType() != null &&
		 * dissemination.getType().equals("restrictedUseAgreement")); Boolean
		 * hasEffectiveDate = (dissemination.getEffectiveDateRestriction() !=
		 * null && dissemination.getEffectiveDateRestriction().booleanValue())
		 * || (dissemination.getType() != null &&
		 * dissemination.getType().equals("effectiveDateRestriction")); Boolean
		 * hasNotDisseminated = (dissemination.getNotDisseminated() != null &&
		 * dissemination.getNotDisseminated().booleanValue()) ||
		 * (dissemination.getType() != null &&
		 * dissemination.getType().equals("notDisseminated")); if
		 * (hasIntellectualProperty || hasLimitedExclusivity || hasRestrictedUse
		 * || hasEffectiveDate || hasNotDisseminated) { if (hasRestrictedUse &&
		 * dissemination.getRestrictedAccessUntil() == null) {
		 * action.addMessage(action.getText(
		 * "project.deliverable.dissemination.v.restrictedUseAgreement"));
		 * action.getInvalidFields().put(
		 * "input-deliverable.dissemination.restrictedAccessUntil",
		 * InvalidFieldsMessages.EMPTYFIELD); } if (hasEffectiveDate &&
		 * dissemination.getRestrictedEmbargoed() == null) {
		 * action.addMessage(action.getText(
		 * "project.deliverable.dissemination.v.restrictedEmbargoed"));
		 * action.getInvalidFields().put(
		 * "input-deliverable.dissemination.restrictedEmbargoed",
		 * InvalidFieldsMessages.EMPTYFIELD); } } else {
		 * action.addMessage(action.getText(
		 * "project.deliverable.dissemination.v.openAccessRestriction"));
		 * action.getInvalidFields().put("input-deliverable.dissemination.type",
		 * InvalidFieldsMessages.EMPTYFIELD); } } } else {
		 * action.addMessage(action.getText(
		 * "project.deliverable.dissemination.v.isOpenAccess"));
		 * action.getInvalidFields().put(
		 * "input-deliverable.dissemination.isOpenAccess",
		 * InvalidFieldsMessages.EMPTYFIELD); }
		 */
			...
			if (dissemination.getAlreadyDisseminated().booleanValue()) {
				...
				if (dissemination.getConfidential() != null) {
					if (dissemination.getConfidential()) {
						if (dissemination.getConfidentialUrl() == null
								|| dissemination.getConfidentialUrl().trim().isEmpty()) {
	...
	public void validateLicense(Deliverable deliverable, BaseAction action) {
		/*
		 * if (deliverable.getDeliverableInfo(action.getActualPhase()).
		 * getAdoptedLicense().booleanValue()) { if
		 * (deliverable.getDeliverableInfo(action.getActualPhase()).getLicense()
		 * != null) { if
		 * (deliverable.getDeliverableInfo(action.getActualPhase()).getLicense()
		 * .equals(LicensesTypeEnum.OTHER.getValue())) { if
		 * (deliverable.getDeliverableInfo(action.getActualPhase()).
		 * getOtherLicense() != null) { if
		 * (!(this.isValidString(deliverable.getDeliverableInfo(action.
		 * getActualPhase()).getOtherLicense()) &&
		 * this.wordCount(deliverable.getDeliverableInfo(action.getActualPhase()
		 * ).getOtherLicense()) <= 100)) { action.addMessage(action.getText(
		 * "project.deliverable.license.v.other"));
		 * action.getInvalidFields().put(
		 * "input-deliverable.deliverableInfo.otherLicense",
		 * InvalidFieldsMessages.EMPTYFIELD); } if
		 * (deliverable.getDeliverableInfo(action.getActualPhase()).
		 * getAllowModifications() == null) { action.addMessage(action.getText(
		 * "project.deliverable.license.v.allowModification"));
		 * action.getInvalidFields().put(
		 * "input-deliverable.deliverableInfo.dissemination.allowModification",
		 * InvalidFieldsMessages.EMPTYFIELD); } } } } else {
		 * action.addMessage(action.getText("project.deliverable.v.license"));
		 * action.getInvalidFields().put(
		 * "input-deliverable.deliverableInfo.license",
		 * InvalidFieldsMessages.EMPTYFIELD); } }
		 */
	}
	...
	public void validateMetadata(List<DeliverableMetadataElement> elements, BaseAction action, boolean isPRP,
		...
		for (DeliverableMetadataElement deliverableMetadataElement : elements) {
			if (deliverableMetadataElement != null) {
				if (deliverableMetadataElement.getMetadataElement().getId() != null) {
					/*
					 * if (8L ==
					 * deliverableMetadataElement.getMetadataElement().getId().
					 * longValue()) { if
					 * ((this.isValidString(deliverableMetadataElement.
					 * getElementValue()) &&
					 * this.wordCount(deliverableMetadataElement.getElementValue
					 * ()) <= 100)) { description = description || true; } //
					 * break; }
					 */
					...
					if (isPRP) {
						...
						if ((this.doesNotHaveDOI == null || this.doesNotHaveDOI.booleanValue() == false)
							...
							if (deliverableMetadataElement.getMetadataElement().getId() != null
								...
								if (deliverableMetadataElement.getElementValue() != null
									...
									String cleanDoi = DOIService
											.tryGetDoiName(deliverableMetadataElement.getElementValue());
									if (cleanDoi.isEmpty()) {
										action.addMessage(action.getText("metadata.doi"));
										action.getInvalidFields().put("input-doi-bridge",
						...
						// Adding title, creation date and authors validation,
						// as per Margarita and Manuel request
						if (deliverableMetadataElement.getId() != null
								&& deliverableMetadataElement.getId().longValue() == 38L) {
							// Authors
		...
		/*
		 * if (!description) { action.addMessage(action.getText(
		 * "project.deliverable.metadata.v.description"));
		 * action.getInvalidFields().put(
		 * "input-deliverable.deliverableInfo.metadataElements[7].elementValue",
		 * InvalidFieldsMessages.EMPTYFIELD); }
		 */
	}
			...
			if (deliverablePublicationMetadata != null && deliverablePublicationMetadata.getId() != null
				...
				if (action.hasDeliverableRule(deliverableInfo, APConstants.DELIVERABLE_RULE_JORNAL_ARTICLES)) {
					...
					if (!this.isValidString(deliverablePublicationMetadata.getVolume())) {
						...
						action.getInvalidFields().put("input-deliverable.publication.volume",
								InvalidFieldsMessages.EMPTYFIELD);
					}
					...
					if (!this.isValidString(deliverablePublicationMetadata.getIssue())) {
						...
						action.getInvalidFields().put("input-deliverable.publication.issue",
								InvalidFieldsMessages.EMPTYFIELD);
					}
					...
					if (!this.isValidString(deliverablePublicationMetadata.getPages())) {
						...
						action.getInvalidFields().put("input-deliverable.publication.pages",
								InvalidFieldsMessages.EMPTYFIELD);
					}
				}
				...
				if (deliverableInfo.getDeliverableType() != null && deliverableInfo.getDeliverableType().getId() != null
					...
					if (!(this.isValidString(deliverablePublicationMetadata.getJournal())
						...
						action.getInvalidFields().put("input-deliverable.publication.journal",
								InvalidFieldsMessages.EMPTYFIELD);
					}
				}
