 ...
 */
@Named
public class PlannedStudiesValidator extends BaseValidator {

  private final GlobalUnitManager crpManager;
  private final PowbSynthesisManager powbSynthesisManager;


  public PlannedStudiesValidator(GlobalUnitManager crpManager, PowbSynthesisManager powbSynthesisManager) {
    super();
  ...
  private Path getAutoSaveFilePath(PowbSynthesis powbSynthesis, long crpID, BaseAction baseAction) {
    ...
    String actionFile = PowbSynthesis2019SectionStatusEnum.PLANNED_STUDIES.getStatus().replace("/", "_");
  ...
  public void validate(BaseAction action, PowbSynthesis powbSynthesis, boolean saving) {
    ...
    if (powbSynthesis != null) {
      ...
      this.saveMissingFields(powbSynthesis, action.getActualPhase().getDescription(), action.getActualPhase().getYear(),
        action.getActualPhase().getUpkeep(), PowbSynthesis2019SectionStatusEnum.PLANNED_STUDIES.getStatus(), action);
    }
  }
