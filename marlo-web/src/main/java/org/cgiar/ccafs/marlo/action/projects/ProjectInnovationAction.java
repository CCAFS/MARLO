...
public class ProjectInnovationAction extends BaseAction {
	...
	public ProjectInnovationAction(APConfig config, GlobalUnitManager globalUnitManager,
			...
			RepIndInnovationTypeManager repIndInnovationTypeManager, RepIndRegionManager repIndRegionManager,
	...
	/**
	 * this is not functional, but is added to prevent a strange exception.
	 * Taken from DeliverableAction.
	 */
	@Override
	...
	public void deleteLocElements(ProjectInnovation innovation, Phase phase, boolean isCountry) {
		if (isCountry) {
			if (innovation.getProjectInnovationCountries() != null
					&& innovation.getProjectInnovationCountries().size() > 0) {
			...
			if (innovation.getProjectInnovationRegions() != null
					&& innovation.getProjectInnovationRegions().size() > 0) {

				List<ProjectInnovationRegion> regionPrev = new ArrayList<>(innovation.getProjectInnovationRegions()
						.stream().filter(nu -> nu.isActive() && nu.getPhase().getId().equals(phase.getId()))
						.collect(Collectors.toList()));
		...
		if (this.getRequest().getParameter(APConstants.TRANSACTION_ID) != null) {
			...
			if (innovation.getProjectInnovationInfo() != null) {
				...
				if (innovation.getProjectInnovationOrganizations() != null) {
					for (ProjectInnovationOrganization projectOrganization : innovation
							.getProjectInnovationOrganizations()) {

						if (projectOrganization.getRepIndOrganizationType() != null
								&& projectOrganization.getRepIndOrganizationType().getId() != null) {

							if (repIndOrganizationTypeManager.getRepIndOrganizationTypeById(
									projectOrganization.getRepIndOrganizationType().getId()) != null) {
								repIndOrganizationTypeManager.getRepIndOrganizationTypeById(
										projectOrganization.getRepIndOrganizationType().getId());
							}
						}
					}
				}
				...
				if (innovation.getProjectInnovationInfo().getRepIndStageInnovation() != null
					...
					innovation.getProjectInnovationInfo()
							.setRepIndStageInnovation(repIndStageInnovationManager.getRepIndStageInnovationById(
									innovation.getProjectInnovationInfo().getRepIndStageInnovation().getId()));
				}
				...
				if (innovation.getProjectInnovationInfo().getRepIndRegion() != null
					...
					innovation.getProjectInnovationInfo().setRepIndRegion(repIndRegionManager
							.getRepIndRegionById(innovation.getProjectInnovationInfo().getRepIndRegion().getId()));
				}
				...
				if (innovation.getProjectInnovationInfo().getRepIndInnovationType() != null
					...
					innovation.getProjectInnovationInfo()
							.setRepIndInnovationType(repIndInnovationTypeManager.getRepIndInnovationTypeById(
				...
				if (innovation.getProjectInnovationInfo().getRepIndDegreeInnovation() != null
					...
					innovation.getProjectInnovationInfo()
							.setRepIndDegreeInnovation(repIndDegreeInnovationManager.getRepIndDegreeInnovationById(
				...
				if (innovation.getProjectInnovationInfo().getLeadOrganization() != null
					...
					innovation.getProjectInnovationInfo().setLeadOrganization(institutionManager
				...
				if (innovation.getProjectInnovationDeliverables() != null
					...
					for (ProjectInnovationDeliverable projectInnovationDeliverable : innovation
						...
						if (projectInnovationDeliverable.getDeliverable() != null
							...
							if (deliverableManager.getDeliverableById(
									projectInnovationDeliverable.getDeliverable().getId()) != null) {
				...
				if (innovation.getProjectInnovationContributingOrganization() != null
					...
					for (ProjectInnovationContributingOrganization projectInnovationContributingOrganization : innovation
						...
						if (projectInnovationContributingOrganization.getInstitution() != null
							...
							if (institutionManager.getInstitutionById(
									projectInnovationContributingOrganization.getInstitution().getId()) != null) {
								Institution institution = institutionManager.getInstitutionById(
										projectInnovationContributingOrganization.getInstitution().getId());
		...
		if (innovation != null) {
			...
			if (path.toFile().exists() && this.getCurrentUser().isAutoSave()) {
				...
				if (innovation.getGeographicScopes() != null) {
					for (ProjectInnovationGeographicScope projectInnovationGeographicScope : innovation
							.getGeographicScopes()) {
				...
				if (haveRegions) {
					...
					if (innovation.getRegions() != null) {
						for (ProjectInnovationRegion projectPolicyRegion : innovation.getRegions()) {
							projectPolicyRegion.setLocElement(
									locElementManager.getLocElementById(projectPolicyRegion.getLocElement().getId()));
						}
					}
				}
				...
				if (haveCountries) {
					...
					if (innovation.getCountriesIdsText() != null) {
						String[] countriesText = innovation.getCountriesIdsText().replace("[", "").replace("]", "")
								.split(",");
				...
				if (innovation.getOrganizations() != null) {
					for (ProjectInnovationOrganization projectInnovationOrganization : innovation.getOrganizations()) {
						projectInnovationOrganization
								.setRepIndOrganizationType(repIndOrganizationTypeManager.getRepIndOrganizationTypeById(
										projectInnovationOrganization.getRepIndOrganizationType().getId()));
					}
				}
				...
				if (innovation.getDeliverables() != null) {
					for (ProjectInnovationDeliverable projectInnovationDeliverable : innovation.getDeliverables()) {
						projectInnovationDeliverable.setDeliverable(deriverableManager
								.getDeliverableById(projectInnovationDeliverable.getDeliverable().getId()));
					}
				}
				...
				if (innovation.getContributingOrganizations() != null
						&& !innovation.getContributingOrganizations().isEmpty()) {
					for (ProjectInnovationContributingOrganization projectInnovationContributingOrganization : innovation
							.getContributingOrganizations()) {
						projectInnovationContributingOrganization.setInstitution(institutionManager.getInstitutionById(
								projectInnovationContributingOrganization.getInstitution().getId()));
					}
				}
				...
				if (innovation.getCrps() != null) {
					for (ProjectInnovationCrp projectInnovationCrp : innovation.getCrps()) {
						projectInnovationCrp.setGlobalUnit(
								globalUnitManager.getGlobalUnitById(projectInnovationCrp.getGlobalUnit().getId()));
					}
				}
				...
				if (this.innovation.getStudies() != null) {
					for (ProjectExpectedStudyInnovation projectExpectedStudyInnovation : this.innovation.getStudies()) {
						projectExpectedStudyInnovation
								.setProjectExpectedStudy(this.projectExpectedStudyManager.getProjectExpectedStudyById(
										projectExpectedStudyInnovation.getProjectExpectedStudy().getId()));
					}
				}
				...
				if (this.innovation.getSharedInnovations() != null) {
					for (ProjectInnovationShared projectInnovationShared : this.innovation.getSharedInnovations()) {
						projectInnovationShared.setProject(
								this.projectManager.getProjectById(projectInnovationShared.getProject().getId()));
					}
				}
				...
				if (innovation.getProjectInnovationGeographicScopes() != null) {
					innovation.setGeographicScopes(new ArrayList<>(innovation.getProjectInnovationGeographicScopes()
				...
				if (innovation.getProjectInnovationDeliverables() != null) {
					innovation.setDeliverables(new ArrayList<>(innovation.getProjectInnovationDeliverables().stream()
							.filter(d -> d.isActive() && d.getPhase().getId() == phase.getId())
							.collect(Collectors.toList())));
				}

				// Innovation Contributing organizations List
				if (innovation.getProjectInnovationContributingOrganization() != null) {
					innovation.setContributingOrganizations(
							new ArrayList<>(innovation.getProjectInnovationContributingOrganization().stream()
									.filter(d -> d.getPhase().getId().equals(phase.getId()))
									.sorted((o1, o2) -> o1.getInstitution().getComposedName()
				...
				if (innovation.getProjectInnovationCrps() != null) {
					innovation.setCrps(new ArrayList<>(innovation.getProjectInnovationCrps().stream()
							.filter(c -> c.isActive() && c.getPhase().getId().equals(phase.getId()))
							.collect(Collectors.toList())));
				}
				...
				if (innovation.getProjectInnovationEvidenceLinks() != null) {
					innovation.setInnovationLinks(new ArrayList<>(innovation.getProjectInnovationEvidenceLinks()
							.stream().filter(c -> c.isActive() && c.getPhase().getId().equals(phase.getId()))
							.collect(Collectors.toList())));
				}
				...
				if (this.innovation.getProjectExpectedStudyInnovations() != null) {
					this.innovation.setStudies(new ArrayList<>(this.innovation.getProjectExpectedStudyInnovations()
							.stream().filter(o -> o.isActive() && o.getPhase().getId().equals(phase.getId()))
							.collect(Collectors.toList())));
					// Get the ID of the principal Sub IDO if exist
					if (innovation.getMilestones() != null) {
			...
			regions = locElementManager.findAll().stream().filter(
				...
				if (projectPartner.getInstitution() != null && projectPartner.getInstitution().getId() != null) {
					...
					if (institution != null
							&& (institution.isPPA(this.getActualPhase().getCrp().getId(), this.getActualPhase())
									|| institution.getInstitutionType().getId()
											.longValue() == APConstants.INSTITUTION_CGIAR_CENTER_TYPE)) {
			...
			// Load Shared studies
			List<ExpectedStudyProject> expectedStudyProject = new ArrayList<>(project.getExpectedStudyProjects()
					.stream()
			...
			if (allProjectStudies != null && allProjectStudies.size() > 0) {
				...
				expectedStudyList = allProjectStudies.stream()
						.filter(ex -> ex.isActive() && ex.getProjectExpectedStudyInfo(phase) != null
								...
								&& ex.getProjectExpectedStudyInfo().getStudyType().getId().intValue() == 1
								&& ex.getProject() != null)
						.collect(Collectors.toList());
			}
			...
			List<ProjectExpectedStudy> evidences = projectExpectedStudyManager.findAll().stream()
					.filter(s -> s != null && s.getProject() == null
							&& s.getProjectExpectedStudyInfo(this.getActualPhase()) != null
						...
						if (!deliverableList.contains(deliverableS.getDeliverable())) {
							if (deliverableS.getDeliverable().getProject() != null
								...
								deliverableS.getDeliverable()
										.setTagTitle("<span class=\"label label-info\">From P"
												+ deliverableS.getDeliverable().getProject().getId() + "</span> "
												+ deliverableS.getDeliverable().getComposedName());
							} else {
								deliverableS.getDeliverable()
			...
			// Update crp list - Delete the actual crp from the list except if
			// this crp was

			if (innovation.getCrps() != null && innovation.getCrps().stream().filter(
	...
	public String save() {
		if (this.hasPermission("canEdit")) {
				...
				if (innovation.getCountriesIds() != null || !innovation.getCountriesIds().isEmpty()) {
					...
					for (ProjectInnovationCountry projectInnovationCountry : countries) {
						if (!countriesSave.contains(projectInnovationCountry)) {
							projectInnovationCountryManager
									.deleteProjectInnovationCountry(projectInnovationCountry.getId());
						}
					}
				}
			} else {
			...
			// If innovation type is different to genetic, the value of the
			// field -number of
			// innovation- is set to null
			// Dperez 2021-07-28 Change all innovations with type different to
			// genetic the default value is 1

			if (innovation.getProjectInnovationInfo().getRepIndInnovationType() != null
					&& innovation.getProjectInnovationInfo().getRepIndInnovationType().getId() != null) {
				if (innovation.getProjectInnovationInfo().getRepIndInnovationType().getId() != 1) {
					innovation.getProjectInnovationInfo().setInnovationNumber(1L);
				} else {
					if (innovation.getProjectInnovationInfo().getRepIndStageInnovation() != null
							&& innovation.getProjectInnovationInfo().getRepIndStageInnovation().getId() != null
							&& (innovation.getProjectInnovationInfo().getRepIndStageInnovation().getId() == 3L
									|| innovation.getProjectInnovationInfo().getRepIndStageInnovation()
											.getId() == 4L)) {
						innovation.getProjectInnovationInfo().setInnovationNumber(1L);
					}
				}
			...
			// NOTE -> FOR SOME REASON "CLEAR LEAD" MEANS "NOT A CLEAR LEAD", SO
			// WE HAVE TO REVERSE THE CONDITIONAL
			 ...
			 * the @ProjectInnovation if we want a row created in the auditlog
			 * table.
			 */
	...
	public void saveCenters(ProjectInnovation projectInnovation, Phase phase) {
		...
		if (projectInnovation.getProjectInnovationCenters() != null
			...
			List<ProjectInnovationCenter> centerPrev = new ArrayList<>(projectInnovation.getProjectInnovationCenters()
					.stream().filter(nu -> nu.isActive() && nu.getPhase().getId().equals(phase.getId()))
					.collect(Collectors.toList()));
		...
		if (innovation.getCenters() != null) {
			for (ProjectInnovationCenter innovationCenter : innovation.getCenters()) {
				if (innovationCenter.getId() == null) {
					...
					Institution institution = institutionManager
							.getInstitutionById(innovationCenter.getInstitution().getId());
					...
					// This is to add innovationCenterSave to generate correct
					// auditlog.
		...
		if (innovation.getContributingOrganizations() != null) {
			for (ProjectInnovationContributingOrganization innovationOrganization : innovation
				...
				if (innovationOrganization.getId() == null) {
					ProjectInnovationContributingOrganization innovationOrganizationSave = new ProjectInnovationContributingOrganization();
					innovationOrganizationSave.setProjectInnovation(projectInnovation);
					innovationOrganizationSave.setPhase(phase);

					Institution institution = institutionManager
							.getInstitutionById(innovationOrganization.getInstitution().getId());
		...
		if (projectInnovation.getProjectInnovationDeliverables() != null
			...
			for (ProjectInnovationDeliverable innovationDeliverable : deliverablePrev) {
				if (innovation != null && innovationDeliverable != null && innovation.getDeliverables() != null
					...
					projectInnovationDeliverableManager
							.deleteProjectInnovationDeliverable(innovationDeliverable.getId());
				}
			}
		}
		...
		if (innovation.getDeliverables() != null) {
			for (ProjectInnovationDeliverable innovationDeliverable : innovation.getDeliverables()) {
				if (innovationDeliverable.getId() == null) {
					...
					// This is to add innovationDeliverableSave to generate
					// correct auditlog.
		...
		if (projectInnovation.getProjectInnovationGeographicScopes() != null
			...
			for (ProjectInnovationGeographicScope innovationScope : scopePrev) {
				if (innovation.getGeographicScopes() == null
					...
					projectInnovationGeographicScopeManager
							.deleteProjectInnovationGeographicScope(innovationScope.getId());
				}
			}
		}
	...
	public void saveLink(ProjectInnovation projectInnovation, Phase phase) {
		...
		if (this.isNotEmpty(projectInnovation.getProjectInnovationEvidenceLinks())) {
			List<ProjectInnovationEvidenceLink> linkPrev = new ArrayList<>(
					projectInnovation.getProjectInnovationEvidenceLinks().stream()
							.filter(nu -> nu.isActive() && nu.getPhase().getId().equals(phase.getId()))
							.collect(Collectors.toList()));
			...
			for (ProjectInnovationEvidenceLink innovationLink : linkPrev) {
				if (this.innovation.getInnovationLinks() == null
					...
					this.projectInnovationEvidenceLinkManager
							.deleteProjectInnovationEvidenceLink(innovationLink.getId());
				}
			}
		}
		...
		if (this.innovation.getInnovationLinks() != null) {
			for (ProjectInnovationEvidenceLink studyLink : this.innovation.getInnovationLinks()) {
				if (studyLink.getId() == null) {
					...
					ProjectInnovationEvidenceLink studyLinkSave = this.projectInnovationEvidenceLinkManager
							.getProjectInnovationEvidenceLinkById(studyLink.getId());
					studyLinkSave.setProjectInnovation(projectInnovation);
					studyLinkSave.setPhase(phase);
					studyLinkSave.setLink(studyLink.getLink());
	...
	public void saveMilestones(ProjectInnovation projectInnovation, Phase phase) {
		...
		// Save policy milestones only if boolean 'has milestones' selection is
		// true
			...
			if (innovation.getMilestones() != null) {
				for (ProjectInnovationMilestone innovationMilestone : innovation.getMilestones()) {
					if (innovationMilestone.getId() == null) {
						...
						// This is to add innovationCenterSave to generate
						// correct auditlog.
				...
				for (ProjectInnovationMilestone innovationMilestone : innovation.getMilestones()) {
					try {
						...
						if (milestone != null) {
							...
							// This is to add innovationCenterSave to generate
							// correct auditlog.
							innovation.getProjectInnovationMilestones().remove(
									projectInnovationMilestoneManager.getProjectInnovationMilestoneById(innovationID));
						}
					} catch (Exception e) {
	...
	public void saveOrganizations(ProjectInnovation projectInnovation, Phase phase) {
		...
		if (projectInnovation.getProjectInnovationOrganizations() != null
			...
			for (ProjectInnovationOrganization innovationOrganization : organizationPrev) {
				if (innovation.getOrganizations() == null
						|| !innovation.getOrganizations().contains(innovationOrganization)) {
					projectInnovationOrganizationManager
							.deleteProjectInnovationOrganization(innovationOrganization.getId());
				}
			}
			...
			if (projectInnovation.getProjectInnovationInfo(this.getActualPhase()) != null
					&& projectInnovation.getProjectInnovationInfo(this.getActualPhase())
	...
	public void saveProjects(ProjectInnovation projectInnovation, Phase phase) {
		...
		if (projectInnovation.getProjectInnovationShareds() != null
			...
			List<ProjectInnovationShared> projectPrev = new ArrayList<>(projectInnovation.getProjectInnovationShareds()
					.stream().filter(nu -> nu.isActive() && nu.getPhase().getId().equals(phase.getId()))
					.collect(Collectors.toList()));
			...
			for (ProjectInnovationRegion innovationRegion : innovation.getRegions()) {
				if (innovationRegion.getId() == null) {
					...
					LocElement locElement = locElementManager
							.getLocElementById(innovationRegion.getLocElement().getId());
	...
	public void saveStudies(ProjectInnovation projectInnovation, Phase phase) {
		...
		// For Stage different to 4, delete previous expect studies milestones
		// information
		if (projectInnovation.getProjectInnovationInfo() != null
			...
			for (ProjectExpectedStudyInnovation studies : projectInnovation.getStudies()) {
				if (studies != null && studies.getId() != null) {
					if (projectExpectedStudyInnovationManager
							.getProjectExpectedStudyInnovationById(studies.getId()) != null) {
	...
	public void saveSubIdos(ProjectInnovation projectInnovation, Phase phase) {
		...
		if (projectInnovation.getProjectInnovationSubIdos() != null
			...
			List<ProjectInnovationSubIdo> subIdoPrev = new ArrayList<>(projectInnovation.getProjectInnovationSubIdos()
					.stream().filter(nu -> nu.isActive() && nu.getPhase().getId().equals(phase.getId()))
					.collect(Collectors.toList()));
			...
			for (ProjectInnovationSubIdo innovationSubIdo : innovation.getSubIdos()) {
				if (innovationSubIdo.getId() == null) {
					...
					if (innovationSubIdo.getSrfSubIdo() != null && innovationSubIdo.getSrfSubIdo().getId() != null) {
						SrfSubIdo srfSubIdo = srfSubIdoManager
								.getSrfSubIdoById(innovationSubIdo.getSrfSubIdo().getId());
						innovationSubIdoSave.setSrfSubIdo(srfSubIdo);
					}
