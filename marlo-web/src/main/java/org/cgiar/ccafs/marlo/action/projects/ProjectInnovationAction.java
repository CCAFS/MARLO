...
public class ProjectInnovationAction extends BaseAction {
  ...
  public void prepare() throws Exception {
    ...
    if (innovation != null) {
      ...
      institutions = institutionManager.findAll().stream().filter(c -> c.getName() != null && c.getName().length() > 0)
          .collect(Collectors.toList());
      ...
      try {
        if (innovation.getSharedInnovations() != null && !innovation.getSharedInnovations().isEmpty()) {
          ...
          List<DeliverableInfo> infos = this.deliverableInfoManager.getDeliverablesInfoByPhaseInnovationAndStatus(phase,
              innovation.getId(), 5L);
          counting = infos.size() + counting;
          for (DeliverableInfo deliverableInfo : infos) {
            Deliverable deliverable = deliverableInfo.getDeliverable();
    ...
    if (this.innovation.getReferences() != null) {
      for (final ProjectInnovationReference studyReference : this.innovation.getReferences()) {
        ...
        if (studyReference.getId() != null) {
          studyReferenceSave = this.projectInnovationReferenceManager
              .getProjectInnovationReferenceById(studyReference.getId());
        }
