...
public class ProjectExpectedStudiesAction extends BaseAction {
    ...
    public ProjectExpectedStudiesAction(APConfig config, ProjectManager projectManager, GlobalUnitManager crpManager,
	    ...
	    LocElementManager locElementManager, StudyTypeManager studyTypeManager, FileDBManager fileDBManager,
	    RepIndGeographicScopeManager geographicScopeManager, RepIndRegionManager repIndRegionManager,
	    RepIndOrganizationTypeManager organizationTypeManager, RepIndGenderYouthFocusLevelManager focusLevelManager,
	    RepIndPolicyInvestimentTypeManager investimentTypeManager, RepIndStageProcessManager stageProcessManager,
	    RepIndStageStudyManager stageStudyManager, ProjectExpectedStudyInfoManager projectExpectedStudyInfoManager,
	    ...
	    GeneralStatusManager generalStatusManager, EvidenceTagManager evidenceTagManager,
	...
	if (isCountry) {
		...
		List<ProjectExpectedStudyCountry> regionPrev = new ArrayList<>(
			this.expectedStudy.getProjectExpectedStudyCountries().stream()
		...
		List<ProjectExpectedStudyRegion> regionPrev = new ArrayList<>(
			this.expectedStudy.getProjectExpectedStudyRegions().stream()
				.filter(nu -> nu.isActive() && nu.getPhase().getId().equals(phase.getId()))
				.collect(Collectors.toList()));
	...
	String autoSaveFile = this.expectedStudy.getId() + "_" + composedClassName + "_"
	...
	this.expectedID = Integer
		.parseInt(StringUtils.trim(this.getRequest().getParameter(APConstants.EXPECTED_REQUEST_ID)));
	...
	if (this.getRequest().getParameter(APConstants.TRANSACTION_ID) != null) {
		...
		if (this.expectedStudy.getProjectExpectedStudyInfo().getRepIndOrganizationType() != null
			&& this.expectedStudy.getProjectExpectedStudyInfo().getRepIndOrganizationType()
				.getId() != null) {
		    this.expectedStudy.getProjectExpectedStudyInfo().setRepIndOrganizationType(
			    this.organizationTypeManager.getRepIndOrganizationTypeById(this.expectedStudy
				    .getProjectExpectedStudyInfo().getRepIndOrganizationType().getId()));
		}
		...
		if (this.expectedStudy.getProjectExpectedStudyInfo().getRepIndStageStudy() != null
		    ...
		    this.expectedStudy.getProjectExpectedStudyInfo()
			    .setRepIndStageStudy(this.stageStudyManager.getRepIndStageStudyById(
				    this.expectedStudy.getProjectExpectedStudyInfo().getRepIndStageStudy().getId()));
		}
		...
		if (this.expectedStudy.getProjectExpectedStudyInfo().getStatus() != null
		    ...
		    this.expectedStudy.getProjectExpectedStudyInfo()
			    .setStatus(this.generalStatusManager.getGeneralStatusById(
		...
		if (this.expectedStudy.getProjectExpectedStudyInfo().getEvidenceTag() != null
		    ...
		    this.expectedStudy.getProjectExpectedStudyInfo()
			    .setEvidenceTag(this.evidenceTagManager.getEvidenceTagById(
				    this.expectedStudy.getProjectExpectedStudyInfo().getEvidenceTag().getId()));
		}
	...
	if (this.expectedStudy != null) {
		...
		if (this.expectedStudy.getGeographicScopes() != null) {
			...
			projectExpectedStudyGeographicScope
				.setRepIndGeographicScope(this.geographicScopeManager.getRepIndGeographicScopeById(
					projectExpectedStudyGeographicScope.getRepIndGeographicScope().getId()));
		...
		if (haveRegions) {
			...
			for (ProjectExpectedStudyRegion projectExpectedStudyRegion : this.expectedStudy
				.getStudyRegions()) {
			    projectExpectedStudyRegion.setLocElement(this.locElementManager
				    .getLocElementById(projectExpectedStudyRegion.getLocElement().getId()));
			}
		    }
		}
		...
		if (haveCountries) {
			...
			String[] countriesText = this.expectedStudy.getCountriesIdsText().replace("[", "")
				.replace("]", "").split(",");
		...
		if (this.expectedStudy.getSubIdos() != null) {
			...
			projectExpectedStudySubIdo.setSrfSubIdo(this.srfSubIdoManager
				.getSrfSubIdoById(projectExpectedStudySubIdo.getSrfSubIdo().getId()));
		    }
		}

		// Expected Study Flagship List Autosave
		if (this.expectedStudy.getFlagships() != null) {
		    for (ProjectExpectedStudyFlagship projectExpectedStudyFlagship : this.expectedStudy
			    .getFlagships()) {
			projectExpectedStudyFlagship.setCrpProgram(this.crpProgramManager
				.getCrpProgramById(projectExpectedStudyFlagship.getCrpProgram().getId()));
		    }
		}
		...
		if (this.expectedStudy.getCrps() != null) {
			...
			projectExpectedStudyCrp.setGlobalUnit(
				this.crpManager.getGlobalUnitById(projectExpectedStudyCrp.getGlobalUnit().getId()));
		    }
		}
		...
		if (this.expectedStudy.getInstitutions() != null) {
		    for (ProjectExpectedStudyInstitution projectExpectedStudyInstitution : this.expectedStudy
			    .getInstitutions()) {
			projectExpectedStudyInstitution.setInstitution(this.institutionManager
				.getInstitutionById(projectExpectedStudyInstitution.getInstitution().getId()));
		    }
		}

		// Expected Study Srf Target List Autosave
		if (this.expectedStudy.getSrfTargets() != null) {
		    for (ProjectExpectedStudySrfTarget projectExpectedStudySrfTarget : this.expectedStudy
			    .getSrfTargets()) {
		...
		if (this.expectedStudy.getProjects() != null) {
			...
			expectedStudyProject.setProject(
				this.projectManager.getProjectById(expectedStudyProject.getProject().getId()));
		    }
		}

		// Expected Study Innovations List Autosave
		if (this.expectedStudy.getInnovations() != null) {
		    for (ProjectExpectedStudyInnovation projectExpectedStudyInnovation : this.expectedStudy
			    .getInnovations()) {
			projectExpectedStudyInnovation
				.setProjectInnovation(this.projectInnovationManager.getProjectInnovationById(
					projectExpectedStudyInnovation.getProjectInnovation().getId()));
		    }
		}
		...
		if (this.expectedStudy.getPolicies() != null) {
			...
			projectExpectedStudyPolicy.setProjectPolicy(this.projectPolicyManager
				.getProjectPolicyById(projectExpectedStudyPolicy.getProjectPolicy().getId()));
		    }
		}
		...
		if (this.expectedStudy.getProjectExpectedStudyInfo().getStudyType() != null) {
			...
			StudyType studyType = this.studyTypeManager.getStudyTypeById(
				this.expectedStudy.getProjectExpectedStudyInfo().getStudyType().getId());
		...
		if (this.expectedStudy.getProjectExpectedStudyGeographicScopes() != null) {
		    this.expectedStudy.setGeographicScopes(
			    new ArrayList<>(this.expectedStudy.getProjectExpectedStudyGeographicScopes().stream()
		...
		if (this.expectedStudy.getProjectExpectedStudyRegions() == null) {
		    ...
		    // Load Regions
		    this.expectedStudy.setStudyRegions(
			    geographics.stream().filter(sc -> sc.getLocElement().getLocElementType().getId() == 1)
				    .collect(Collectors.toList()));
		}
		...
		if (this.expectedStudy.getProjectExpectedStudyFlagships() != null) {
		    this.expectedStudy.setFlagships(new ArrayList<>(this.expectedStudy
			    .getProjectExpectedStudyFlagships().stream()
			    .filter(o -> o.isActive() && o.getPhase().getId().equals(phase.getId()) && o.getCrpProgram()
		...
		if (this.expectedStudy.getProjectExpectedStudyFlagships() != null) {
		    this.expectedStudy.setRegions(new ArrayList<>(this.expectedStudy.getProjectExpectedStudyFlagships()
			    .stream()
			    .filter(o -> o.isActive() && o.getPhase().getId().equals(phase.getId()) && o.getCrpProgram()
				    .getProgramType() == ProgramType.REGIONAL_PROGRAM_TYPE.getValue())
			    .collect(Collectors.toList())));
		}
		...
		if (this.expectedStudy.getProjectExpectedStudyCenters() != null) {
		    this.expectedStudy.setCenters(new ArrayList<>(this.expectedStudy.getProjectExpectedStudyCenters()
			    .stream().filter(o -> o.isActive() && o.getPhase().getId().equals(phase.getId()))
			    .collect(Collectors.toList())));
		}
		...
		if (this.expectedStudy.getProjectExpectedStudyMilestones() != null) {
		    this.expectedStudy.setMilestones(new ArrayList<>(this.expectedStudy
			    .getProjectExpectedStudyMilestones().stream()
		...
		if (this.expectedStudy.getProjectExpectedStudyInstitutions() != null) {
		    this.expectedStudy
			    .setInstitutions(new ArrayList<>(this.expectedStudy.getProjectExpectedStudyInstitutions()
				    .stream().filter(o -> o.isActive() && o.getPhase().getId().equals(phase.getId()))
				    .collect(Collectors.toList())));
		}
		...
		if (this.expectedStudy.getProjectExpectedStudySrfTargets() != null) {
		    this.expectedStudy
			    .setSrfTargets(new ArrayList<>(this.expectedStudy.getProjectExpectedStudySrfTargets()
				    .stream().filter(o -> o.isActive() && o.getPhase().getId().equals(phase.getId()))
				    .collect(Collectors.toList())));
		}
		...
		if (this.expectedStudy.getExpectedStudyProjects() != null) {
		    this.expectedStudy.setProjects(new ArrayList<>(this.expectedStudy.getExpectedStudyProjects()
			    .stream().filter(o -> o.isActive() && o.getPhase().getId().equals(phase.getId()))
			    .collect(Collectors.toList())));
		}
		...
		if (this.expectedStudy.getProjectExpectedStudyLinks() != null) {
		    this.expectedStudy.setLinks(new ArrayList<>(this.expectedStudy.getProjectExpectedStudyLinks()
			    .stream().filter(o -> o.isActive() && o.getPhase().getId().equals(phase.getId()))
			    .collect(Collectors.toList())));
		}
		...
		if (this.expectedStudy.getProjectExpectedStudyPolicies() != null) {
		    this.expectedStudy.setPolicies(new ArrayList<>(this.expectedStudy.getProjectExpectedStudyPolicies()
			    .stream().filter(o -> o.isActive() && o.getPhase().getId().equals(phase.getId()))
			    .collect(Collectors.toList())));
		}
		...
		if (this.expectedStudy.getProjectExpectedStudyQuantifications() != null) {
		    this.expectedStudy.setQuantifications(
			    new ArrayList<>(this.expectedStudy.getProjectExpectedStudyQuantifications().stream()
				    .filter(o -> o.isActive() && o.getPhase().getId().equals(phase.getId()))
				    .collect(Collectors.toList())));
		}
	    ...
	    this.countries = this.locElementManager.findAll().stream()
		    .filter(c -> c.getLocElementType().getId().intValue() == 2 && c.isActive())
		    .collect(Collectors.toList());

	    this.geographicScopes = this.geographicScopeManager.findAll();
	    this.regions = this.locElementManager.findAll().stream().filter(
		    c -> c.getLocElementType().getId().intValue() == 1 && c.isActive() && c.getIsoNumeric() != null)
		    .collect(Collectors.toList());
	    ...
	    // Expected Study Projects List
	    if (this.expectedStudy.getExpectedStudyProjects() != null) {
		List<ExpectedStudyProject> expectedStudyProjects = new ArrayList<>(
			this.expectedStudy.getExpectedStudyProjects().stream()
				.filter(o -> o.isActive() && o.getPhase().getId().equals(phase.getId()))
				.collect(Collectors.toList()));
		...
		for (ExpectedStudyProject expectedStudyProject : expectedStudyProjects) {
		    ...
		    List<ProjectInnovation> sharedInnovations = sharedProject.getProjectInnovations().stream()
		    ...
		    List<ProjectPolicy> sharedPolicies = sharedProject.getProjectPolicies().stream()
			    .filter(c -> c.isActive()).collect(Collectors.toList());
	    ...
	    if (expectedStudy.getCrps() != null && expectedStudy.getCrps().stream().filter(
		    x -> x != null && x.getGlobalUnit().getId().equals(this.getCurrentGlobalUnit().getId())) != null) {
	    ...
	    this.institutions = this.institutionManager.findAll().stream().filter(i -> i.isActive())
		    .collect(Collectors.toList());
		...
		List<ProjectInnovation> innovations = projectL.getProjectInnovations().stream()
			.filter(c -> c.isActive()).collect(Collectors.toList());
		...
		List<ProjectPolicy> policies = projectL.getProjectPolicies().stream().filter(c -> c.isActive())
			.collect(Collectors.toList());
		 ...
		 */
		List<ProjectInnovationShared> innovationShareds = new ArrayList<>(this.project
			.getProjectInnovationShareds().stream()
	...
	if (this.hasPermission("canEdit") || user.getId().equals(this.expectedStudyDB.getCreatedBy().getId())) {
		...
		RepIndGenderYouthFocusLevel focusLevel = this.focusLevelManager.getRepIndGenderYouthFocusLevelById(
			this.expectedStudy.getProjectExpectedStudyInfo().getYouthLevel().getId());
	...
	if (projectExpectedStudy.getProjectExpectedStudyCenters() != null
	    ...
	    List<ProjectExpectedStudyCenter> centerPrev = new ArrayList<>(
		    projectExpectedStudy.getProjectExpectedStudyCenters().stream()
			    .filter(nu -> nu.isActive() && nu.getPhase().getId().equals(phase.getId()))
			    .collect(Collectors.toList()));
	...
	if (this.expectedStudy.getCenters() != null) {
		...
		if (studyCenter.getId() == null) {
		    ...
		    Institution institution = this.institutionManager
			    .getInstitutionById(studyCenter.getInstitution().getId());
	...
	if (projectExpectedStudy.getProjectExpectedStudyFlagships() != null
	    ...
	    List<ProjectExpectedStudyFlagship> flagshipPrev = new ArrayList<>(projectExpectedStudy
		    .getProjectExpectedStudyFlagships().stream()
		    .filter(nu -> nu.isActive() && nu.getPhase().getId().equals(phase.getId())
		...
		if (this.expectedStudy.getFlagships() == null
			|| !this.expectedStudy.getFlagships().contains(studyFlagship)) {
	...
	if (this.expectedStudy.getFlagships() != null) {
		...
		if (studyFlagship.getId() == null) {
		    ...
		    CrpProgram crpProgram = this.crpProgramManager
			    .getCrpProgramById(studyFlagship.getCrpProgram().getId());
	...
	if (projectExpectedStudy.getProjectExpectedStudyGeographicScopes() != null
	    ...
	    List<ProjectExpectedStudyGeographicScope> scopePrev = new ArrayList<>(
		...
		if (this.expectedStudy.getGeographicScopes() == null
		    ...
		    this.projectExpectedStudyGeographicScopeManager
			    .deleteProjectExpectedStudyGeographicScope(studyScope.getId());
		}
	    }
	}
	...
	if (this.expectedStudy.getGeographicScopes() != null) {
		...
		if (studyScope.getId() == null) {
		    ...
		    RepIndGeographicScope geoScope = this.geographicScopeManager
			    .getRepIndGeographicScopeById(studyScope.getRepIndGeographicScope().getId());

		    studyScopeSave.setRepIndGeographicScope(geoScope);

		    this.projectExpectedStudyGeographicScopeManager
			    .saveProjectExpectedStudyGeographicScope(studyScopeSave);
	...
	if (projectExpectedStudy.getProjectExpectedStudyInnovations() != null
	    ...
	    List<ProjectExpectedStudyInnovation> innovationPrev = new ArrayList<>(
		...
		if (this.expectedStudy.getInnovations() == null
		    ...
		    this.projectExpectedStudyInnovationManager
			    .deleteProjectExpectedStudyInnovation(studyInnovation.getId());
		}
	    }
	}
	...
	if (this.expectedStudy.getInnovations() != null) {
		...
		if (studyInnovation.getId() == null) {
		    ...
		    ProjectInnovation projectInnovation = this.projectInnovationManager
			    .getProjectInnovationById(studyInnovation.getProjectInnovation().getId());
	...
	if (projectExpectedStudy.getProjectExpectedStudyInstitutions() != null
	    ...
	    List<ProjectExpectedStudyInstitution> institutionPrev = new ArrayList<>(
		...
		if (this.expectedStudy.getInstitutions() == null
		    ...
		    this.projectExpectedStudyInstitutionManager
			    .deleteProjectExpectedStudyInstitution(studyInstitution.getId());
		}
	    }
	}
	...
	if (this.expectedStudy.getInstitutions() != null) {
		...
		if (studyInstitution.getId() == null) {
		    ...
		    Institution institution = this.institutionManager
			    .getInstitutionById(studyInstitution.getInstitution().getId());

		    studyInstitutionSave.setInstitution(institution);

		    this.projectExpectedStudyInstitutionManager
			    .saveProjectExpectedStudyInstitution(studyInstitutionSave);
	...
	if (projectExpectedStudy.getProjectExpectedStudyLinks() != null
	    ...
	    List<ProjectExpectedStudyLink> linkPrev = new ArrayList<>(
	...
	if (this.expectedStudy.getLinks() != null) {
		...
		if (studyLink.getId() == null) {
		    ...
		    ProjectExpectedStudyLink studyLinkSave = this.projectExpectedStudyLinkManager
			    .getProjectExpectedStudyLinkById(studyLink.getId());
		    studyLinkSave.setProjectExpectedStudy(projectExpectedStudy);
		    studyLinkSave.setPhase(phase);
		    studyLinkSave.setLink(studyLink.getLink());
	...
	if (projectExpectedStudy.getProjectExpectedStudyMilestones() != null
	    ...
	    List<ProjectExpectedStudyMilestone> milestonePrev = new ArrayList<>(
		...
		if (this.expectedStudy.getMilestones() == null
		    ...
		    this.projectExpectedStudyMilestoneManager
			    .deleteProjectExpectedStudyMilestone(studyMilestone.getId());
		}
	    }
	}
	...
	if (this.expectedStudy.getProjectExpectedStudyInfo().getHasMilestones() != null
		...
		for (ProjectExpectedStudyMilestone studyMilestone : this.expectedStudy.getMilestones()) {
			...
			if (studyMilestone.getCrpMilestone() != null
				&& studyMilestone.getCrpMilestone().getId() != null) {
			    ...
			    if ((milestonePrimaryId != 0 || crpMilestonePrimary != 0)
				    && studyMilestone.getCrpMilestone() != null) {
			...
			if ((milestonePrimaryId != 0 || crpMilestonePrimary != 0)
				&& studyMilestone.getCrpMilestone() != null) {
			    ProjectExpectedStudyMilestone studyMilestoneSave = new ProjectExpectedStudyMilestone();
			    studyMilestoneSave = projectExpectedStudyMilestoneManager
				    .getProjectExpectedStudyMilestoneById(studyMilestone.getId());
	...
	if (projectExpectedStudy.getProjectExpectedStudyPolicies() != null
	    ...
	    List<ProjectExpectedStudyPolicy> policyPrev = new ArrayList<>(
		...
		if (this.expectedStudy.getPolicies() == null
			|| !this.expectedStudy.getPolicies().contains(studyPolicy)) {
	...
	if (this.expectedStudy.getPolicies() != null) {
		...
		if (studyPolicy.getId() == null) {
		    ...
		    ProjectPolicy projectPolicy = this.projectPolicyManager
			    .getProjectPolicyById(studyPolicy.getProjectPolicy().getId());
	...
	if (projectExpectedStudy.getExpectedStudyProjects() != null
	    ...
	    List<ExpectedStudyProject> projectPrev = new ArrayList<>(projectExpectedStudy.getExpectedStudyProjects()
		    .stream().filter(nu -> nu.isActive() && nu.getPhase().getId().equals(phase.getId()))
		    .collect(Collectors.toList()));

	    for (ExpectedStudyProject studyProject : projectPrev) {
		if (this.expectedStudy.getProjects() == null
			|| !this.expectedStudy.getProjects().contains(studyProject)) {
	...
	if (projectExpectedStudy.getProjectExpectedStudyQuantifications() != null
	    ...
	    List<ProjectExpectedStudyQuantification> quantificationPrev = new ArrayList<>(
		    projectExpectedStudy.getProjectExpectedStudyQuantifications().stream()
			    .filter(nu -> nu.isActive() && nu.getPhase().getId().equals(phase.getId()))
			    .collect(Collectors.toList()));
	...
	if (projectExpectedStudy.getProjectExpectedStudyFlagships() != null
	    ...
	    for (ProjectExpectedStudyFlagship studyFlagship : flagshipPrev) {
		if (this.expectedStudy.getRegions() == null
			|| !this.expectedStudy.getRegions().contains(studyFlagship)) {
		    this.projectExpectedStudyFlagshipManager.deleteProjectExpectedStudyFlagship(studyFlagship.getId());
		}
	    }
	}
	...
	if (projectExpectedStudy.getProjectExpectedStudySrfTargets() != null
	    ...
	    List<ProjectExpectedStudySrfTarget> targetPrev = new ArrayList<>(
		...
		if (this.expectedStudy.getSrfTargets() == null
			|| !this.expectedStudy.getSrfTargets().contains(studytarget)) {
	...
	if (this.expectedStudy.getSrfTargets() != null) {
		...
		if (studytarget.getId() == null) {
		    ...
		    SrfSloIndicator sloIndicator = this.srfSloIndicatorManager
			    .getSrfSloIndicatorById(studytarget.getSrfSloIndicator().getId());
	...
	if (this.expectedStudy.getStudyRegions() != null) {
		...
		if (studyRegion.getId() == null) {
		    ...
		    LocElement locElement = this.locElementManager
			    .getLocElementById(studyRegion.getLocElement().getId());
	...
	if (projectExpectedStudy.getProjectExpectedStudySubIdos() != null
	    ...
	    List<ProjectExpectedStudySubIdo> subIdoPrev = new ArrayList<>(
	...
	if (this.expectedStudy.getSubIdos() != null) {
		...
		if (studySubIdo.getId() == null) {
		    ...
		    if ((subIdoPrimaryId != 0 || srfSubIdoPrimary != 0) && studySubIdo.getSrfSubIdo() != null) {
			ProjectExpectedStudySubIdo studySubIdoSave = new ProjectExpectedStudySubIdo();
			studySubIdoSave = projectExpectedStudySubIdoManager
				.getProjectExpectedStudySubIdoById(studySubIdo.getId());
