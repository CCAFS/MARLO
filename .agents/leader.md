...
## 🏗️ MARLO Directory Boundaries (injected by `/akili-constitution`)

This is what you judge **task independence** against when deciding whether two tasks may run in
parallel (see *Delegation Thresholds* → "Multiple writers at once").

### Module map

| Module | Contents | Java files |
|---|---|---|
| `marlo-data` | JPA entities, DAOs, Managers, `ManagerImpl` | ~2,400 |
| `marlo-web` | Struts actions, Spring MVC REST, validators, interceptors, FTL views, JS/CSS | ~1,150 |
| `marlo-core` | Core config / initialization | 2 |
| `marlo-utils` | Utility classes | 6 |
| `marlo-parent` | Parent POM, dependency management | — |

### Task-footprint areas

| Area | Path |
|---|---|
| Struts actions | `marlo-web/src/main/java/org/cgiar/ccafs/marlo/action/<module>/` |
| REST (Spring MVC) | `marlo-web/src/main/java/org/cgiar/ccafs/marlo/rest/` |
| Validators | `marlo-web/src/main/java/org/cgiar/ccafs/marlo/validation/<module>/` |
...
| Struts routing | `marlo-web/src/main/resources/struts-<module>.xml` (29 files) |
| FTL views | `marlo-web/src/main/webapp/crp/**/*.ftl` (~300 files) |
| Static JS / CSS | `marlo-web/src/main/webapp/global/` |
| Migrations | `marlo-web/src/main/resources/database/migrations/` (~1,580 files) |
| i18n | `marlo-web/src/main/resources/global.properties`, `custom/*.properties` |

### 🔴 Shared writers — these make tasks NOT parallel-safe

Two tasks that both touch any one of these are **serial**, no matter how disjoint their intent:

| Shared file / directory | Why it collides |
|---|---|
| `marlo-data/.../config/APConstants.java` **and** `marlo-web/.../config/APConstants.java` | Every specificity adds a constant to **both**. Two tasks adding flags conflict twice |
| `marlo-web/src/main/resources/global.properties` | All i18n additions land in one file |
| `marlo-web/.../action/BaseAction.java` | Shared base for every action; a common target for cross-cutting helpers |
| `marlo-web/src/main/resources/struts.xml` | Global routing/package config (per-module `struts-<module>.xml` files are safe to parallelize **across different modules**) |
| `.../database/migrations/` | **Ordering**, not content, collides. Two tasks each adding a timestamped migration produce an apply order neither task validated |
| `marlo-parent/pom.xml` | Dependency management for all modules |

**Practical rule for this repo:** a spec that adds a specificity, an i18n string, or a migration has
a **serial spine**. Give one task ownership of the shared-file edits and let the dependent tasks run
in parallel behind it — do not hand two Implementers the same `APConstants.java`.

### ✅ Reliably parallel-safe pairs

- Two tasks in **different Struts modules** (`struts-projects.xml` vs `struts-powb.xml`) touching
  different action packages and different FTL trees.
- A `marlo-data` entity/manager task and a `marlo-web` FTL/JS task **once the data task has landed**
  (serial dependency, then parallel).
- Two FTL view tasks under different `webapp/crp/<module>/` trees.

### Exemplar-file briefing for MARLO

The repository is large and highly patterned — an exemplar is worth more here than anywhere. When
briefing an Implementer, name the closest existing file:

| Task shape | Point at |
|---|---|
| New Struts action | An existing action in the same `action/<module>/` package |
| New `ManagerImpl` save/delete chain | The nearest existing `ManagerImpl` in `marlo-data/.../manager/impl/` |
| New validator | The sibling validator in `validation/<module>/` |
| New FTL section | The nearest `.ftl` in the same `webapp/crp/<module>/` tree |
| New migration | The most recent file in `database/migrations/` (for naming and style) |
| New specificity | An existing specificity migration + its `APConstants` entries + its FTL guard |

### Skills to assign, by task shape

Drawn from the `## Skill Map` in `AGENTS.md` / `CLAUDE.md`:

| Task shape | Assign |
|---|---|
| REST endpoint under `/api/*` | `api-design-principles`, `error-handling-patterns` |
| Bug with unclear cause | `systematic-debugging` |
| Logic-heavy unit with a clean seam | `tdd` |
| FTL/CSS screen work | `ui-ux-pro-max` |
| Bedrock / RAG / agent work | `ai-agent-development` |
| Lambda / S3 / Bedrock infra touchpoints | `aws-serverless` |

### CodeGraph is initialized here

`.codegraph/` exists and the index is current (4,251 files · 92,602 nodes · 261,275 edges).
Instruct workers to resolve unfamiliar symbols through graph lookups before exploratory file reads,
and to run an impact query before changing a shared symbol — in a 3,500-class codebase this is the
difference between a scoped change and an unbounded one. Remind them the graph does **not** include
this spec's own changes.

### Verification commands to put in every brief
...
The automated suite is **three JUnit 4 classes**. Treat "`mvn test` passed" as evidence of nothing
beyond "nothing that was already tested broke" — when a task's claim is behavioral, the brief must
name what actually proves it.
