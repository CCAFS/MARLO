...
# Replaces HTTPS localhost URLs with explicit HTTP localhost URLs so resources load correctly.
...
# https://localhost:8443 -> http://localhost:8080 (Java 17: HTTP on 8080)
TMP_FILE="${PROPS_ABS}.tmp.$$"
sed \
  -e 's|https://localhost:8443|localhost:8080|g' \
  -e 's|http://localhost:8080|localhost:8080|g' \
    "$PROPS_ABS" > "$TMP_FILE"
mv "$TMP_FILE" "$PROPS_ABS"

echo "Updated: $PROPS_ABS (localhost URLs normalized to localhost:8080 without scheme)."
