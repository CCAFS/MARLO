...
| R-D4 | `/api/**` `authcBasic` breaks for CGIAR users — federated identities cannot use Basic auth | **OQ-4 — CLOSED 2026-09-07** | **NO AFFECTED CONSUMERS.** IBD confirmed no external application, script or integration authenticates to `/api/**` with Basic auth; report generation reaches MARLO through normal actions, not that surface. T00's second entry, `ClarisaPublicAccesFilter`'s configured service account, was verified from the database: `is_cgiar_user = 0`, and none of its 16 Global Units carries the Cognito flag — safe on two independent counts. **The `/api/**` → `authcBasic` mapping still exists**, so this is "nothing uses it today", not "the surface is gone"; a future integration must not walk into it. TRD §8.4's drift remains **queued**, not applied (shared-file rule) |
