...
public class CrpDAO implements ICrpDAO {
  ...
  public List<Crp> findAll() {
    String query = "from " + Crp.class.getName();
  ...
  public List<Crp> getCrpsByUserId(long userId) {
    String query = "from " + Crp.class.getName() + " where user_id=" + userId;
  ...
  @Override
  public long save(Crp crp, String actionName, List<String> relationsName) {
    if (crp.getId() == null) {
      dao.save(crp, actionName, relationsName);
    } else {
      dao.update(crp, actionName, relationsName);
    }
    return crp.getId();
  }
